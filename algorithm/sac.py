...
from hy.core import *  #13 (line num in coconut source)
from hy.core.language import *  #14 (line num in coconut source)
from hy.contrib.pprint import pp  #15 (line num in coconut source)

## Defaults (as args later)
verbose: bool = True  # Print verbose debug output  #18 (line num in coconut source)
num_episodes: int = 666  # Number of episodes to play  #19 (line num in coconut source)
num_steps: int = 100  # How many steps to take  #20 (line num in coconut source)
early_stop: float = -50.0  # Early stop criterion  #21 (line num in coconut source)
batch_size: int = 100  # size of the batches during epoch  #22 (line num in coconut source)
rng_seed: int = 666  # Random seed for reproducability  #23 (line num in coconut source)
algorithm: str = "sac"  # Name of used algorithm ∈  ./algorithm  #24 (line num in coconut source)
...
τ_soft: float = 1e-2  # Avantage Factor  #29 (line num in coconut source)
α: float = 3e-4  # Learning Rate  #30 (line num in coconut source)
β1: float = 0.9  #32 (line num in coconut source)
β2: float = 0.999  #33 (line num in coconut source)
βs: tuple[float] = (β1, β2)  # Lower Clipping  #34 (line num in coconut source)
σ_min: float = -2  # Lower Clipping  #34 (line num in coconut source)
σ_max: float = 20  # Upper Clipping  #35 (line num in coconut source)
buffer_size: int = (int)(1e7)  # Maximum size of replay buffer  #36 (line num in coconut source)
...
class ReplayBuffer(_coconut.typing.NamedTuple("ReplayBuffer", [("state", pt.Tensor), ("action", pt.Tensor), ("reward", pt.Tensor), ("next_state", pt.Tensor), ("done", pt.Tensor)])):  #61 (line num in coconut source)
    ...
    def push(self, other):  #61 (line num in coconut source)
        new = ((fmap)(_coconut.operator.itemgetter((_coconut.slice(-buffer_size, None))), (ReplayBuffer)(*other)) if len(self.state) <= 0 else (ReplayBuffer)(*(fmap)(_coconut_forward_compose(pt.vstack, _coconut.operator.itemgetter((_coconut.slice(-buffer_size, None)))), zip(self, other))))  #64 (line num in coconut source)
        return new  #65 (line num in coconut source)

    def sample(self, batch_size: int):  #65 (line num in coconut source)
        idx = (((pt.randperm)(((self.state).shape)[0]))[_coconut.slice(None, batch_size)]).tolist()  #68 (line num in coconut source)
        smpl = (ReplayBuffer)(*(e[idx, :] for e in self))  #69 (line num in coconut source)
        return smpl  #70 (line num in coconut source)

    @_coconut_tco  #70 (line num in coconut source)
    def size(self):  #70 (line num in coconut source)
        return _coconut_tail_call((float), ((self.state).shape)[0])  #70 (line num in coconut source)

## Neural Networks

class HStack(_coconut.collections.namedtuple("HStack", ()), pt.nn.Module):  #73 (line num in coconut source)
    ...
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #73 (line num in coconut source)
        return _coconut_tail_call((pt.hstack), X)  #74 (line num in coconut source)


class VStack(_coconut.collections.namedtuple("VStack", ()), pt.nn.Module):  #76 (line num in coconut source)
    _coconut_is_data = True  #76 (line num in coconut source)
    __slots__ = ()  #76 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #76 (line num in coconut source)
    def __eq__(self, other):  #76 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #76 (line num in coconut source)
    def __hash__(self):  #76 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #76 (line num in coconut source)
    __match_args__ = ()  #76 (line num in coconut source)
    @_coconut_tco  #76 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #76 (line num in coconut source)
        return _coconut_tail_call((pt.vstack), X)  #77 (line num in coconut source)

## Value Network

class ValueNet(_coconut.typing.NamedTuple("ValueNet", [("obs_dim", int)]), pt.nn.Module):  #80 (line num in coconut source)
    _coconut_is_data = True  #80 (line num in coconut source)
    __slots__ = ()  #80 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #80 (line num in coconut source)
    def __eq__(self, other):  #80 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #80 (line num in coconut source)
    def __hash__(self):  #80 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #80 (line num in coconut source)
    __match_args__ = ('obs_dim',)  #80 (line num in coconut source)
    def __init__(self, obs_dim: int):  #81 (line num in coconut source)
        super(ValueNet, self).__init__()  #82 (line num in coconut source)
        self.v_net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #83 (line num in coconut source)

    @_coconut_tco  #84 (line num in coconut source)
    def forward(self, state: pt.Tensor):  #84 (line num in coconut source)
        return _coconut_tail_call((self.v_net), state)  #87 (line num in coconut source)

## Critic Network

class CriticNet(_coconut.typing.NamedTuple("CriticNet", [("obs_dim", int), ("act_dim", int)]), pt.nn.Module):  #90 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim', 'act_dim')  #90 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #91 (line num in coconut source)
        super(CriticNet, self).__init__()  #92 (line num in coconut source)
        dim = obs_dim + act_dim  #93 (line num in coconut source)
        self.c_net = pt.nn.Sequential(HStack(), pt.nn.Linear(dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #94 (line num in coconut source)

    @_coconut_tco  #95 (line num in coconut source)
    def forward(self, state: pt.Tensor, action: pt.Tensor):  #95 (line num in coconut source)
        return _coconut_tail_call((self.c_net), (state, action))  #99 (line num in coconut source)

## Actor Network

class ActorNet(_coconut.typing.NamedTuple("ActorNet", [("obs_dim", int), ("act_dim", int), ("σ_min", float), ("σ_max", float)]), pt.nn.Module):  #102 (line num in coconut source)
    _coconut_is_data = True  #102 (line num in coconut source)
    __slots__ = ()  #102 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #102 (line num in coconut source)
    def __eq__(self, other):  #102 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #102 (line num in coconut source)
    def __hash__(self):  #102 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #102 (line num in coconut source)
    __match_args__ = ('obs_dim', 'act_dim', 'σ_min', 'σ_max')  #102 (line num in coconut source)
    def __new__(_coconut_cls, obs_dim, act_dim, σ_min=-2.0, σ_max=20.0):  #102 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (obs_dim, act_dim, σ_min, σ_max))  #102 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #104 (line num in coconut source)
        super(ActorNet, self).__init__()  #105 (line num in coconut source)
        self.lin_1 = pt.nn.Linear(obs_dim, 256)  #106 (line num in coconut source)
        self.lin_2 = pt.nn.Linear(256, 128)  #107 (line num in coconut source)
        self.lin_3 = pt.nn.Linear(128, 64)  #108 (line num in coconut source)
        self.lin_μ = pt.nn.Linear(64, act_dim)  #109 (line num in coconut source)
        self.lin_σ = pt.nn.Linear(64, act_dim)  #110 (line num in coconut source)
        self.relu = pt.nn.functional.relu  #111 (line num in coconut source)
        self.min_σ = σ_min  #112 (line num in coconut source)
        self.max_σ = σ_max  #113 (line num in coconut source)
        self.lin_σ.weight.data.uniform_(-3e-3, 3e-3)  #114 (line num in coconut source)
        self.lin_σ.bias.data.uniform_(-3e-3, 3e-3)  #115 (line num in coconut source)

    def forward(self, state: pt.Tensor):  #116 (line num in coconut source)
        x = (self.lin_3)((self.relu)((self.lin_2)((self.relu)((self.lin_1)(state)))))  #117 (line num in coconut source)
        μ = (self.lin_μ)(x)  #118 (line num in coconut source)
        σ = (_coconut_partial(pt.clamp, {1: self.min_σ, 2: self.max_σ}, 3))((self.lin_σ)(x))  #119 (line num in coconut source)

        return (μ, σ)  #121 (line num in coconut source)

class Model(_coconut.typing.NamedTuple("Model", [("actor_online", pt.nn.Module), ("critic_online", pt.nn.Module), ("value_online", pt.nn.Module), ("value_target", pt.nn.Module), ("actor_optim", pt.optim.Optimizer), ("critic_optim", pt.optim.Optimizer), ("value_optim", pt.optim.Optimizer), ("critic_loss", _coconut.typing.Any), ("value_loss", _coconut.typing.Any)])):  #121 (line num in coconut source)
    _coconut_is_data = True  #121 (line num in coconut source)
    __slots__ = ()  #121 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #121 (line num in coconut source)
    def __eq__(self, other):  #121 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #121 (line num in coconut source)
    def __hash__(self):  #121 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #121 (line num in coconut source)
    __match_args__ = ('actor_online', 'critic_online', 'value_online', 'value_target', 'actor_optim', 'critic_optim', 'value_optim', 'critic_loss', 'value_loss')  #121 (line num in coconut source)
    def sync_target(self, other=None):  #121 (line num in coconut source)
        source = other if other else self.value_online  #126 (line num in coconut source)
        target = self.value_target  #127 (line num in coconut source)
        update = lambda t, o: (t.copy_)((t * (1.0 - τ_soft) + o * τ_soft))  #128 (line num in coconut source)
        _ = (list)((_coconut_partial(starmap, {0: update}, 2))((zip)(*(fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("parameters"), list, _coconut.functools.partial(fmap, _coconut.operator.attrgetter("data"))), [target, source]))))  #129 (line num in coconut source)
        return self.value_target  #130 (line num in coconut source)

    def act(self, state: pt.Tensor):  #130 (line num in coconut source)
        μ, σ_ = (self.actor_online)(state)  #132 (line num in coconut source)
        σ = (pt.exp)(σ_)  #133 (line num in coconut source)
        a = ((pt.tanh)(((pt.distributions.Normal)(*(μ, σ))).sample())).detach()  #134 (line num in coconut source)
        return a  #135 (line num in coconut source)

    def evaluate(self, state: pt.Tensor, ε: float=1e-6):  #135 (line num in coconut source)
        μ, σ_ = (self.actor_online)(state)  #137 (line num in coconut source)
        σ = (pt.exp)(σ_)  #138 (line num in coconut source)
        n = pt.distributions.Normal(μ, σ)  #139 (line num in coconut source)
        z = n.sample()  #140 (line num in coconut source)
        a = (pt.tanh)(z)  #141 (line num in coconut source)
        ε_ = pt.log(1 - a.pow(2) + ε)  #142 (line num in coconut source)
        p = ((_coconut_partial((_coconut_minus), {1: ε_}, 2))((n).log_prob(z))).sum(-1, keepdim=True)  #143 (line num in coconut source)

        return (a, p, z, μ, σ)  #145 (line num in coconut source)

def make_model(act_dim: int, obs_dim: int):  #145 (line num in coconut source)
    def init_weights(m: pt.nn.Module, init: float):  #146 (line num in coconut source)
        if isinstance(m, pt.nn.Linear) and (m.out_features == 1 or m.out_features == act_dim):  #147 (line num in coconut source)
            m.weight.data.uniform_(-init, init)  #148 (line num in coconut source)
            m.bias.data.uniform_(-init, init)  #149 (line num in coconut source)

    critic_loss = pt.nn.MSELoss()  #150 (line num in coconut source)
    value_loss = pt.nn.MSELoss()  #151 (line num in coconut source)
    actor_online = (ActorNet(obs_dim, act_dim)).to(device)  #152 (line num in coconut source)
    critic_online = (CriticNet(obs_dim, act_dim)).to(device)  #153 (line num in coconut source)
    value_online = (ValueNet(obs_dim)).to(device)  #154 (line num in coconut source)
    _ = [(n.apply)(_coconut_partial(init_weights, {1: 3e-3}, 2)) for n in [value_online, critic_online, actor_online]]  #155 (line num in coconut source)
    value_target = (ValueNet(obs_dim)).to(device)  #157 (line num in coconut source)
    params = (zip)(*(fmap)(_coconut.operator.methodcaller("parameters"), [value_target, value_online]))  #158 (line num in coconut source)
    _ = [(tp.data.copy_)((op).data) for tp, op in params]  #159 (line num in coconut source)
    actor_optim = pt.optim.Adam(actor_online.parameters(), lr=α, betas=βs)  #160 (line num in coconut source)
    critic_optim = pt.optim.Adam(critic_online.parameters(), lr=α, betas=βs)  #161 (line num in coconut source)
    value_optim = pt.optim.Adam(value_online.parameters(), lr=α, betas=βs)  #162 (line num in coconut source)
    model = Model(actor_online, critic_online, value_online, value_target, actor_optim, critic_optim, value_optim, critic_loss, value_loss)  #163 (line num in coconut source)

## Update
    return model  #168 (line num in coconut source)
...
def soft_q_update(epoch: int, model: Model, buffer: ReplayBuffer, batch_size: int, μλ: float=1e-3, σλ: float=1e-3, zλ: float=0.0, γ: float=0.99):  #168 (line num in coconut source)
    states, actions, rewards, next_states, dones = (fmap)(_coconut.operator.methodcaller("to", device), buffer.sample(batch_size))  #171 (line num in coconut source)
    v_expected = model.value_online(states)  #173 (line num in coconut source)
    q_expected = model.critic_online(states, actions)  #174 (line num in coconut source)
    new_actions, log_prob, z, μ, σ = model.evaluate(states)  #175 (line num in coconut source)
    v_target = model.value_target(next_states)  #177 (line num in coconut source)
    q_next = ((rewards + (1 - dones) * γ * v_target)).detach()  #178 (line num in coconut source)
    c_loss = model.critic_loss(q_expected, q_next)  #179 (line num in coconut source)
    q_exp_nxt = model.critic_online(states, new_actions)  #180 (line num in coconut source)
    v_next = ((q_exp_nxt - log_prob)).detach()  #181 (line num in coconut source)
    v_loss = model.value_loss(v_expected, v_next)  #182 (line num in coconut source)
    l_target = q_exp_nxt - v_expected  #183 (line num in coconut source)
    l_loss = (((log_prob * (log_prob - l_target))).detach()).mean()  #184 (line num in coconut source)
    μ_loss = (_coconut_partial((_coconut.operator.mul), {0: μλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(μ)))  #185 (line num in coconut source)
    σ_loss = (_coconut_partial((_coconut.operator.mul), {0: σλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(σ)))  #186 (line num in coconut source)
    z_loss = (_coconut_partial((_coconut.operator.mul), {0: zλ}, 2))((pt.mean)(((_coconut_partial(pt.pow, {1: 2}, 2))(z)).sum(1)))  #187 (line num in coconut source)
    a_loss = l_loss + μ_loss + σ_loss + z_loss  #188 (line num in coconut source)
    _ = model.critic_optim.zero_grad()  #189 (line num in coconut source)
    _ = c_loss.backward()  #190 (line num in coconut source)
    _ = model.critic_optim.step()  #191 (line num in coconut source)
    _ = model.value_optim.zero_grad()  #192 (line num in coconut source)
    _ = v_loss.backward()  #193 (line num in coconut source)
    _ = model.value_optim.step()  #194 (line num in coconut source)
    _ = model.actor_optim.zero_grad()  #195 (line num in coconut source)
    _ = a_loss.backward()  #196 (line num in coconut source)
    _ = model.actor_optim.step()  #197 (line num in coconut source)
    _ = model.sync_target()  #198 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Actor", a_loss, epoch)  #199 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic", c_loss, epoch)  #200 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Value", v_loss, epoch)  #201 (line num in coconut source)

## Exploration
    return (v_loss, c_loss, a_loss)  #204 (line num in coconut source)

def postprocess(observations, keys):  #204 (line num in coconut source)
    idx = [(list)((map)(lambda p: (key.index)(p), (_coconut_partial(filter, {0: lambda k: ":" not in k}, 2))(key))) for key in keys]  #205 (line num in coconut source)
    states = ((pt.vstack)((fmap)(pt.from_numpy, [o[i] for o, i in zip(observations, idx)]))).to(device)  #207 (line num in coconut source)

## Checkpoint Saving
    return states  #211 (line num in coconut source)

def save_checkpoint(model: Model, checkpoint_file: str):  #211 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [model.actor_online, model.critic_online, model.value_online, model.value_target, model.actor_optim, model.critic_optim, model.value_optim])  #212 (line num in coconut source)
    keys = ["actor_online", "critic_online", "value_online", "value_target", "actor_optim", "critic_optim", "value_optim"]  #215 (line num in coconut source)
    save_dict = (dict)((zip)(*(keys, state_dicts)))  #217 (line num in coconut source)
    res = pt.save(save_dict, checkpoint_file)  #218 (line num in coconut source)

## Training Loop
    return res  #221 (line num in coconut source)

@_coconut_mark_as_match  #221 (line num in coconut source)
def train(*_coconut_match_args, **_coconut_match_kwargs):  #221 (line num in coconut source)
    _coconut_match_check_0 = False  #221 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #221 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #221 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #221 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #221 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) == 7) and ("model" not in _coconut_match_kwargs) and ("episode" not in _coconut_match_kwargs) and ("buffer" not in _coconut_match_kwargs) and (_coconut_match_args[4] is True):  #221 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #221 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #221 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #221 (line num in coconut source)
        if not _coconut_match_kwargs:  #221 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_0  #221 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_1  #221 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_2  #221 (line num in coconut source)
            _coconut_match_check_0 = True  #221 (line num in coconut source)
    if _coconut_match_check_0:  #221 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #221 (line num in coconut source)
            model = _coconut_match_temp_0  #221 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #221 (line num in coconut source)
            episode = _coconut_match_temp_1  #221 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #221 (line num in coconut source)
            buffer = _coconut_match_temp_2  #221 (line num in coconut source)
    if not _coconut_match_check_0:  #221 (line num in coconut source)
        raise _coconut_FunctionMatchError('def train(_, model, episode, _, True, buffer, _) = model where:', _coconut_match_args)  #221 (line num in coconut source)

    total = ((buffer.reward).sum()).item()  #222 (line num in coconut source)
    _ = writer.add_scalar(f"_Reward_Total", total, episode)  #223 (line num in coconut source)
    if verbose:  #224 (line num in coconut source)
        (print)(f"Episode {episode:03} Finished | Total Reward: {total}")  #225 (line num in coconut source)
    return model  #226 (line num in coconut source)

@_coconut_addpattern(train)  #226 (line num in coconut source)
@_coconut_tco  #226 (line num in coconut source)
@_coconut_mark_as_match  #226 (line num in coconut source)
def train(*_coconut_match_args, **_coconut_match_kwargs):  #226 (line num in coconut source)
    _coconut_match_check_1 = False  #226 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #226 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #226 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #226 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #226 (line num in coconut source)
    _coconut_match_set_name_finish = _coconut_sentinel  #226 (line num in coconut source)
    ...
    if (_coconut.len(_coconut_match_args) <= 7) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "envs" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "model" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "episode" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "finish" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 5, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "states" in _coconut_match_kwargs)) == 1):  #226 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("envs")  #226 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #226 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #226 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("iteration")  #226 (line num in coconut source)
        _coconut_match_temp_7 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("finish")  #226 (line num in coconut source)
        _coconut_match_temp_8 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #226 (line num in coconut source)
        _coconut_match_temp_9 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("states")  #226 (line num in coconut source)
        if not _coconut_match_kwargs:  #226 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_3  #226 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_4  #226 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_5  #226 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_6  #226 (line num in coconut source)
            _coconut_match_set_name_finish = _coconut_match_temp_7  #226 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_8  #226 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_9  #226 (line num in coconut source)
            _coconut_match_check_1 = True  #226 (line num in coconut source)
    if _coconut_match_check_1:  #226 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #226 (line num in coconut source)
            envs = _coconut_match_temp_3  #226 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #226 (line num in coconut source)
            model = _coconut_match_temp_4  #226 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #226 (line num in coconut source)
            episode = _coconut_match_temp_5  #226 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #226 (line num in coconut source)
            iteration = _coconut_match_temp_6  #226 (line num in coconut source)
        if _coconut_match_set_name_finish is not _coconut_sentinel:  #226 (line num in coconut source)
            finish = _coconut_match_temp_7  #226 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #226 (line num in coconut source)
            buffer = _coconut_match_temp_8  #226 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #226 (line num in coconut source)
            states = _coconut_match_temp_9  #226 (line num in coconut source)
    if not _coconut_match_check_1:  #226 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def train( envs, model, episode, iteration, finish, buffer, states\n) = train( envs, model, episode, iteration_, finish_\n, buffer_, states_ ) where:', _coconut_match_args)  #226 (line num in coconut source)

    t0 = time.time()  #229 (line num in coconut source)
    actions = model.act(states)  #230 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #231 (line num in coconut source)
    states_ = postprocess(observations, [i["output-parameters"] for i in infos])  #235 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #236 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #237 (line num in coconut source)
    buffer_ = (buffer.push)((ReplayBuffer)(*(states, actions, rewards, states_, dones)))  #238 (line num in coconut source)
    t1 = time.time()  #240 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #241 (line num in coconut source)
    _ = soft_q_update(iteration, model, buffer_, batch_size)  #242 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(dones)  #243 (line num in coconut source)
    stop_ = (((pt.mean)(rewards)).item()) < early_stop  #244 (line num in coconut source)
    finish_ = done_ or stop_ or (iteration >= num_steps)  #245 (line num in coconut source)
    iteration_ = iteration + 1  #246 (line num in coconut source)
    if verbose:  #247 (line num in coconut source)
        print(f"Iteration {iteration:03} took {(t1 - t0):.3f}s | Average Reward: {rewards.mean():.3f}")  #248 (line num in coconut source)
    _ = writer.add_scalar("_Reward_Mean", rewards.mean(), iteration)  #249 (line num in coconut source)
    _ = save_checkpoint(model, model_path)  #250 (line num in coconut source)

## Episode Loop
    return _coconut_tail_call(train, envs, model, episode, iteration_, finish_, buffer_, states_)  #253 (line num in coconut source)

def run_episode(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #253 (line num in coconut source)
    obs = envs.reset()  #255 (line num in coconut source)
    states = postprocess(obs, envs.observation_keys)  #256 (line num in coconut source)
    buffer = (ReplayBuffer)(*(tuple)(_coconut_iter_getitem((repeat)(pt.empty(0)), (_coconut.slice(None, 5)))))  #257 (line num in coconut source)
    model = train(envs, model, episode, 0, False, buffer, states)  #258 (line num in coconut source)

    return model  #259 (line num in coconut source)
