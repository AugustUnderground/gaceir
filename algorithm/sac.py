...
num_steps: int = 100  # How many steps to take  #17 (line num in coconut source)
...
algorithm: str = "sac"  # Name of used algorithm ∈  ./algorithm  #21 (line num in coconut source)
...
τ_soft: float = 1e-2  # Avantage Factor  #26 (line num in coconut source)
α: float = 3e-2  # Learning Rate  #27 (line num in coconut source)
β1: float = 0.5  # 0.9  #29 (line num in coconut source)
β2: float = 0.3  # 0.999  #30 (line num in coconut source)
βs: tuple[float] = (β1, β2)  # Lower Clipping  #31 (line num in coconut source)
σ_min: float = -2  # Lower Clipping  #31 (line num in coconut source)
σ_max: float = 20  # Upper Clipping  #32 (line num in coconut source)
buffer_size: int = (int)(1e7)  # Maximum size of replay buffer  #33 (line num in coconut source)
max_time: float = 20.0  # Maximum time to cut off  #34 (line num in coconut source)

## Setup
env_id: str = f"gace:{ace_id}-{ace_backend}-v{ace_variant}"  #37 (line num in coconut source)
time_stamp: str = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")  #38 (line num in coconut source)
model_dir: str = f"./models/{time_stamp}-{env_id}-{algorithm}"  #39 (line num in coconut source)
model_path: str = f"{model_dir}/checkpoint.pt"  #40 (line num in coconut source)
log_dir: str = f"./runs/{time_stamp}-{env_id}-{algorithm}/"  #41 (line num in coconut source)
os.makedirs(model_dir, exist_ok=True)  #42 (line num in coconut source)

## Setup Globals
device = pt.device("cuda:1") if pt.cuda.is_available() else pt.device("cpu")  #45 (line num in coconut source)
writer = SummaryWriter(log_dir=log_dir, flush_secs=30)  #46 (line num in coconut source)
_ = (pt.manual_seed)(rng_seed)  #47 (line num in coconut source)

## Utility
def write_performance(env: gym.Env, step: int):  #50 (line num in coconut source)
    target = env.target  #51 (line num in coconut source)
    performance = (ac.current_performance)((env).ace)  #52 (line num in coconut source)
    for k in target.keys():  #53 (line num in coconut source)
        writer.add_scalars(k, {"performance": performance[k], "target": target[k]}, step)  #54 (line num in coconut source)

## Replay Buffer
    return performance  #59 (line num in coconut source)

class ReplayBuffer(_coconut.typing.NamedTuple("ReplayBuffer", [("state", pt.Tensor), ("action", pt.Tensor), ("reward", pt.Tensor), ("next_state", pt.Tensor), ("done", pt.Tensor)])):  #59 (line num in coconut source)
    _coconut_is_data = True  #59 (line num in coconut source)
    __slots__ = ()  #59 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #59 (line num in coconut source)
    def __eq__(self, other):  #59 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #59 (line num in coconut source)
    def __hash__(self):  #59 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #59 (line num in coconut source)
    __match_args__ = ('state', 'action', 'reward', 'next_state', 'done')  #59 (line num in coconut source)
    def push(self, other):  #59 (line num in coconut source)
        new = ((fmap)(_coconut.operator.itemgetter((_coconut.slice(-buffer_size, None))), (ReplayBuffer)(*other)) if len(self.state) <= 0 else (ReplayBuffer)(*(fmap)(_coconut_forward_compose(pt.vstack, _coconut.operator.itemgetter((_coconut.slice(-buffer_size, None)))), zip(self, other))))  #62 (line num in coconut source)
        return new  #63 (line num in coconut source)

    def sample(self, batch_size: int):  #63 (line num in coconut source)
        idx = (((pt.randperm)(self.size()))[_coconut.slice(None, batch_size)]).tolist()  #66 (line num in coconut source)
        smpl = (ReplayBuffer)(*(tuple)((m[idx, :] for m in self)))  #67 (line num in coconut source)
        return smpl  #68 (line num in coconut source)

    @_coconut_tco  #68 (line num in coconut source)
    def size(self):  #68 (line num in coconut source)
        return _coconut_tail_call((int), ((self.state).shape)[0])  #68 (line num in coconut source)

## Neural Networks

class HStack(_coconut.collections.namedtuple("HStack", ()), pt.nn.Module):  #71 (line num in coconut source)
    _coconut_is_data = True  #71 (line num in coconut source)
    __slots__ = ()  #71 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #71 (line num in coconut source)
    def __eq__(self, other):  #71 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #71 (line num in coconut source)
    def __hash__(self):  #71 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #71 (line num in coconut source)
    __match_args__ = ()  #71 (line num in coconut source)
    @_coconut_tco  #71 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #71 (line num in coconut source)
        return _coconut_tail_call((pt.hstack), X)  #72 (line num in coconut source)


class VStack(_coconut.collections.namedtuple("VStack", ()), pt.nn.Module):  #74 (line num in coconut source)
    _coconut_is_data = True  #74 (line num in coconut source)
    __slots__ = ()  #74 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #74 (line num in coconut source)
    def __eq__(self, other):  #74 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #74 (line num in coconut source)
    def __hash__(self):  #74 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #74 (line num in coconut source)
    __match_args__ = ()  #74 (line num in coconut source)
    @_coconut_tco  #74 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #74 (line num in coconut source)
        return _coconut_tail_call((pt.vstack), X)  #75 (line num in coconut source)

## Value Network

class ValueNet(_coconut.typing.NamedTuple("ValueNet", [("obs_dim", int)]), pt.nn.Module):  #78 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim',)  #78 (line num in coconut source)
    def __init__(self, obs_dim: int):  #79 (line num in coconut source)
        super(ValueNet, self).__init__()  #80 (line num in coconut source)
        self.v_net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #81 (line num in coconut source)

    @_coconut_tco  #82 (line num in coconut source)
    def forward(self, state: pt.Tensor):  #82 (line num in coconut source)
        return _coconut_tail_call((self.v_net), state)  #85 (line num in coconut source)

## Critic Network

class CriticNet(_coconut.typing.NamedTuple("CriticNet", [("obs_dim", int), ("act_dim", int)]), pt.nn.Module):  #88 (line num in coconut source)
    _coconut_is_data = True  #88 (line num in coconut source)
    __slots__ = ()  #88 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #88 (line num in coconut source)
    def __eq__(self, other):  #88 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #88 (line num in coconut source)
    def __hash__(self):  #88 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #88 (line num in coconut source)
    __match_args__ = ('obs_dim', 'act_dim')  #88 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #89 (line num in coconut source)
        super(CriticNet, self).__init__()  #90 (line num in coconut source)
        dim = obs_dim + act_dim  #91 (line num in coconut source)
        self.c_net = pt.nn.Sequential(HStack(), pt.nn.Linear(dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #92 (line num in coconut source)

    @_coconut_tco  #93 (line num in coconut source)
    def forward(self, state: pt.Tensor, action: pt.Tensor):  #93 (line num in coconut source)
        return _coconut_tail_call((self.c_net), (state, action))  #97 (line num in coconut source)

## Actor Network

class ActorNet(_coconut.typing.NamedTuple("ActorNet", [("obs_dim", int), ("act_dim", int), ("σ_min", float), ("σ_max", float)]), pt.nn.Module):  #100 (line num in coconut source)
    _coconut_is_data = True  #100 (line num in coconut source)
    __slots__ = ()  #100 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #100 (line num in coconut source)
    def __eq__(self, other):  #100 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #100 (line num in coconut source)
    def __hash__(self):  #100 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #100 (line num in coconut source)
    __match_args__ = ('obs_dim', 'act_dim', 'σ_min', 'σ_max')  #100 (line num in coconut source)
    def __new__(_coconut_cls, obs_dim, act_dim, σ_min=-2.0, σ_max=20.0):  #100 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (obs_dim, act_dim, σ_min, σ_max))  #100 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #102 (line num in coconut source)
        super(ActorNet, self).__init__()  #103 (line num in coconut source)
        self.lin_1 = pt.nn.Linear(obs_dim, 256)  #104 (line num in coconut source)
        self.lin_2 = pt.nn.Linear(256, 128)  #105 (line num in coconut source)
        self.lin_3 = pt.nn.Linear(128, 64)  #106 (line num in coconut source)
        self.lin_μ = pt.nn.Linear(64, act_dim)  #107 (line num in coconut source)
        self.lin_σ = pt.nn.Linear(64, act_dim)  #108 (line num in coconut source)
        self.relu = pt.nn.functional.relu  #109 (line num in coconut source)
        self.min_σ = σ_min  #110 (line num in coconut source)
        self.max_σ = σ_max  #111 (line num in coconut source)
        self.lin_σ.weight.data.uniform_(-3e-3, 3e-3)  #112 (line num in coconut source)
        self.lin_σ.bias.data.uniform_(-3e-3, 3e-3)  #113 (line num in coconut source)

    def forward(self, state: pt.Tensor):  #114 (line num in coconut source)
        x = (self.lin_3)((self.relu)((self.lin_2)((self.relu)((self.lin_1)(state)))))  #115 (line num in coconut source)
        μ = (self.lin_μ)(x)  #116 (line num in coconut source)
        σ = (_coconut_partial(pt.clamp, {1: self.min_σ, 2: self.max_σ}, 3))((self.lin_σ)(x))  #117 (line num in coconut source)

        return (μ, σ)  #119 (line num in coconut source)

class Model(_coconut.typing.NamedTuple("Model", [("actor_online", pt.nn.Module), ("critic_online", pt.nn.Module), ("value_online", pt.nn.Module), ("value_target", pt.nn.Module), ("actor_optim", pt.optim.Optimizer), ("critic_optim", pt.optim.Optimizer), ("value_optim", pt.optim.Optimizer), ("critic_loss", _coconut.typing.Any), ("value_loss", _coconut.typing.Any)])):  #119 (line num in coconut source)
    ...
    __match_args__ = ('actor_online', 'critic_online', 'value_online', 'value_target', 'actor_optim', 'critic_optim', 'value_optim', 'critic_loss', 'value_loss')  #119 (line num in coconut source)
    def sync_target(self, other=None):  #119 (line num in coconut source)
        source = other if other else self.value_online  #124 (line num in coconut source)
        target = self.value_target  #125 (line num in coconut source)
        update = lambda t, o: (t.copy_)((t * (1.0 - τ_soft) + o * τ_soft))  #126 (line num in coconut source)
        _ = (list)((_coconut_partial(starmap, {0: update}, 2))((zip)(*(fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("parameters"), list, _coconut.functools.partial(fmap, _coconut.operator.attrgetter("data"))), [target, source]))))  #127 (line num in coconut source)
        return self.value_target  #128 (line num in coconut source)

    def act(self, state: pt.Tensor):  #128 (line num in coconut source)
        μ, σ_ = (self.actor_online)(state)  #130 (line num in coconut source)
        σ = (pt.exp)(σ_)  #131 (line num in coconut source)
        a = ((pt.tanh)(((pt.distributions.Normal)(*(μ, σ))).sample())).detach()  #132 (line num in coconut source)
        return a  #133 (line num in coconut source)

    def evaluate(self, state: pt.Tensor, ε: float=1e-6):  #133 (line num in coconut source)
        μ, σ_ = (self.actor_online)(state)  #135 (line num in coconut source)
        σ = (pt.exp)(σ_)  #136 (line num in coconut source)
        n = pt.distributions.Normal(μ, σ)  #137 (line num in coconut source)
        z = n.sample()  #138 (line num in coconut source)
        a = (pt.tanh)(z)  #139 (line num in coconut source)
        ε_ = pt.log(1 - a.pow(2) + ε)  #140 (line num in coconut source)
        p = ((_coconut_partial((_coconut_minus), {1: ε_}, 2))((n).log_prob(z))).sum(-1, keepdim=True)  #141 (line num in coconut source)

        return (a, p, z, μ, σ)  #143 (line num in coconut source)

def make_model(act_dim: int, obs_dim: int):  #143 (line num in coconut source)
    def init_weights(m: pt.nn.Module, init: float):  #144 (line num in coconut source)
        if isinstance(m, pt.nn.Linear) and (m.out_features == 1 or m.out_features == act_dim):  #145 (line num in coconut source)
            m.weight.data.uniform_(-init, init)  #146 (line num in coconut source)
            m.bias.data.uniform_(-init, init)  #147 (line num in coconut source)

    critic_loss = pt.nn.MSELoss()  #148 (line num in coconut source)
    value_loss = pt.nn.MSELoss()  #149 (line num in coconut source)
    actor_online = (ActorNet(obs_dim, act_dim)).to(device)  #150 (line num in coconut source)
    critic_online = (CriticNet(obs_dim, act_dim)).to(device)  #151 (line num in coconut source)
    value_online = (ValueNet(obs_dim)).to(device)  #152 (line num in coconut source)
    _ = [(n.apply)(_coconut_partial(init_weights, {1: 3e-3}, 2)) for n in [value_online, critic_online, actor_online]]  #153 (line num in coconut source)
    value_target = (ValueNet(obs_dim)).to(device)  #155 (line num in coconut source)
    params = (zip)(*(fmap)(_coconut.operator.methodcaller("parameters"), [value_target, value_online]))  #156 (line num in coconut source)
    _ = [(tp.data.copy_)((op).data) for tp, op in params]  #157 (line num in coconut source)
    actor_optim = pt.optim.Adam(actor_online.parameters(), lr=α, betas=βs)  #158 (line num in coconut source)
    critic_optim = pt.optim.Adam(critic_online.parameters(), lr=α, betas=βs)  #159 (line num in coconut source)
    value_optim = pt.optim.Adam(value_online.parameters(), lr=α, betas=βs)  #160 (line num in coconut source)
    model = Model(actor_online, critic_online, value_online, value_target, actor_optim, critic_optim, value_optim, critic_loss, value_loss)  #161 (line num in coconut source)

## Update
    return model  #166 (line num in coconut source)

def soft_q_update(epoch: int, model: Model, buffer: ReplayBuffer, batch_size: int, μλ: float=1e-3, σλ: float=1e-3, zλ: float=0.0, γ: float=0.99):  #166 (line num in coconut source)
    states, actions, rewards, next_states, dones = (fmap)(_coconut.operator.methodcaller("to", device), buffer.sample(batch_size))  #169 (line num in coconut source)
    v_expected = model.value_online(states)  #171 (line num in coconut source)
    q_expected = model.critic_online(states, actions)  #172 (line num in coconut source)
    new_actions, log_prob, z, μ, σ = model.evaluate(states)  #173 (line num in coconut source)
    v_target = model.value_target(next_states)  #175 (line num in coconut source)
    q_next = ((rewards + (1 - dones) * γ * v_target)).detach()  #176 (line num in coconut source)
    c_loss = model.critic_loss(q_expected, q_next)  #177 (line num in coconut source)
    q_exp_nxt = model.critic_online(states, new_actions)  #178 (line num in coconut source)
    v_next = ((q_exp_nxt - log_prob)).detach()  #179 (line num in coconut source)
    v_loss = model.value_loss(v_expected, v_next)  #180 (line num in coconut source)
    l_target = q_exp_nxt - v_expected  #181 (line num in coconut source)
    l_loss = (((log_prob * (log_prob - l_target))).detach()).mean()  #182 (line num in coconut source)
    μ_loss = (_coconut_partial((_coconut.operator.mul), {0: μλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(μ)))  #183 (line num in coconut source)
    σ_loss = (_coconut_partial((_coconut.operator.mul), {0: σλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(σ)))  #184 (line num in coconut source)
    z_loss = (_coconut_partial((_coconut.operator.mul), {0: zλ}, 2))((pt.mean)(((_coconut_partial(pt.pow, {1: 2}, 2))(z)).sum(1)))  #185 (line num in coconut source)
    a_loss = l_loss + μ_loss + σ_loss + z_loss  #186 (line num in coconut source)
    _ = model.critic_optim.zero_grad()  #187 (line num in coconut source)
    _ = c_loss.backward()  #188 (line num in coconut source)
    _ = model.critic_optim.step()  #189 (line num in coconut source)
    _ = model.value_optim.zero_grad()  #190 (line num in coconut source)
    _ = v_loss.backward()  #191 (line num in coconut source)
    _ = model.value_optim.step()  #192 (line num in coconut source)
    _ = model.actor_optim.zero_grad()  #193 (line num in coconut source)
    _ = a_loss.backward()  #194 (line num in coconut source)
    _ = model.actor_optim.step()  #195 (line num in coconut source)
    _ = model.sync_target()  #196 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Actor", a_loss, epoch)  #197 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic", c_loss, epoch)  #198 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Value", v_loss, epoch)  #199 (line num in coconut source)

## Exploration
    return (v_loss, c_loss, a_loss)  #202 (line num in coconut source)

def postprocess(observations, keys: dict[str, list[str]]):  #202 (line num in coconut source)
    pf = lambda k: ":" not in k and "/" not in k and (k[0]).islower()  #203 (line num in coconut source)
    lf = lambda k: k in ["ugbw", "cof", "sr_f", "sr_r"] or k.endswith("fug")  #204 (line num in coconut source)
    p_idx = (list)((fmap)(keys["observations"].index, (filter)(pf, keys["observations"])))  #205 (line num in coconut source)
    i_idx = [(keys["observations"].index)(k) for k in keys["actions"]]  #208 (line num in coconut source)
    idx = (sorted)((p_idx + i_idx))  #209 (line num in coconut source)
    l_msk = ((pt.Tensor)((list)((_coconut_partial(map, {0: lf}, 2))(keys["observations"])))).to(device)  #210 (line num in coconut source)
    of = lambda o: (pt.where(o > 0, pt.log10(o), o) * l_msk) + (o * (1 - l_msk))  #211 (line num in coconut source)
    states = (pt.vstack)([((of)(((pt.from_numpy)(obs)).to(device)))[idx] for obs in observations])  #212 (line num in coconut source)
#states  = (pt.log10(states_) *  l_msk) + (states_ * (~l_msk))
#states = [ obs |> pt.from_numpy for obs in observations
#         ] |> pt.vstack |> .to(device)

## Checkpoint Saving
    return states  #219 (line num in coconut source)

def save_checkpoint(model: Model, checkpoint_file: str):  #219 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [model.actor_online, model.critic_online, model.value_online, model.value_target, model.actor_optim, model.critic_optim, model.value_optim])  #220 (line num in coconut source)
    keys = ["actor_online", "critic_online", "value_online", "value_target", "actor_optim", "critic_optim", "value_optim"]  #223 (line num in coconut source)
    save_dict = (dict)((zip)(*(keys, state_dicts)))  #225 (line num in coconut source)
    res = pt.save(save_dict, checkpoint_file)  #226 (line num in coconut source)

## Training Loop
    return res  #229 (line num in coconut source)

@_coconut_mark_as_match  #229 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #229 (line num in coconut source)
    _coconut_match_check_0 = False  #229 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #229 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #229 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #229 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #229 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) == 7) and ("model" not in _coconut_match_kwargs) and ("episode" not in _coconut_match_kwargs) and ("buffer" not in _coconut_match_kwargs) and (_coconut_match_args[4] is True):  #229 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #229 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #229 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #229 (line num in coconut source)
        if not _coconut_match_kwargs:  #229 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_0  #229 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_1  #229 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_2  #229 (line num in coconut source)
            _coconut_match_check_0 = True  #229 (line num in coconut source)
    if _coconut_match_check_0:  #229 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #229 (line num in coconut source)
            model = _coconut_match_temp_0  #229 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #229 (line num in coconut source)
            episode = _coconut_match_temp_1  #229 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #229 (line num in coconut source)
            buffer = _coconut_match_temp_2  #229 (line num in coconut source)
    if not _coconut_match_check_0:  #229 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, model, episode, _, True, buffer, _) = model where:', _coconut_match_args)  #229 (line num in coconut source)

    total = ((buffer.reward).sum()).item()  #230 (line num in coconut source)
    _ = writer.add_scalar(f"_Reward_Total", total, episode)  #231 (line num in coconut source)
    if verbose:  #232 (line num in coconut source)
        (print)(f"Episode {episode:03} Finished | Total Reward: {total}")  #233 (line num in coconut source)
    return model  #234 (line num in coconut source)

@_coconut_addpattern(run_episode)  #234 (line num in coconut source)
@_coconut_tco  #234 (line num in coconut source)
@_coconut_mark_as_match  #234 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #234 (line num in coconut source)
    _coconut_match_check_1 = False  #234 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #234 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #234 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #234 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #234 (line num in coconut source)
    _coconut_match_set_name_finish = _coconut_sentinel  #234 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #234 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #234 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #234 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) <= 7) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "envs" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "model" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "episode" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "finish" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 5, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "states" in _coconut_match_kwargs)) == 1):  #234 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("envs")  #234 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #234 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #234 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("iteration")  #234 (line num in coconut source)
        _coconut_match_temp_7 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("finish")  #234 (line num in coconut source)
        _coconut_match_temp_8 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #234 (line num in coconut source)
        _coconut_match_temp_9 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("states")  #234 (line num in coconut source)
        if not _coconut_match_kwargs:  #234 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_3  #234 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_4  #234 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_5  #234 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_6  #234 (line num in coconut source)
            _coconut_match_set_name_finish = _coconut_match_temp_7  #234 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_8  #234 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_9  #234 (line num in coconut source)
            _coconut_match_check_1 = True  #234 (line num in coconut source)
    if _coconut_match_check_1:  #234 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #234 (line num in coconut source)
            envs = _coconut_match_temp_3  #234 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #234 (line num in coconut source)
            model = _coconut_match_temp_4  #234 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #234 (line num in coconut source)
            episode = _coconut_match_temp_5  #234 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #234 (line num in coconut source)
            iteration = _coconut_match_temp_6  #234 (line num in coconut source)
        if _coconut_match_set_name_finish is not _coconut_sentinel:  #234 (line num in coconut source)
            finish = _coconut_match_temp_7  #234 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #234 (line num in coconut source)
            buffer = _coconut_match_temp_8  #234 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #234 (line num in coconut source)
            states = _coconut_match_temp_9  #234 (line num in coconut source)
    if not _coconut_match_check_1:  #234 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode( envs, model, episode, iteration, finish, buffer, states\n) = run_episode( envs, model, episode, iteration_\n, finish_, buffer_, states_ ) where:', _coconut_match_args)  #234 (line num in coconut source)

    t0 = time.time()  #237 (line num in coconut source)
    actions = (model.act)(states)  #238 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #239 (line num in coconut source)
    states_ = postprocess(observations, envs.info[0])  #243 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #244 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #245 (line num in coconut source)
    buffer_ = (buffer.push)((ReplayBuffer)(*(states, actions, rewards, states_, dones)))  #246 (line num in coconut source)
    t1 = time.time()  #248 (line num in coconut source)
    dt = t1 - t0  #249 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #250 (line num in coconut source)
    _ = soft_q_update(iteration, model, buffer_, batch_size)  #251 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(dones)  #252 (line num in coconut source)
    stop_ = (((pt.mean)(rewards)).item()) < early_stop  #253 (line num in coconut source)
    finish_ = done_ or stop_ or (iteration >= num_steps) or (dt > max_time)  #254 (line num in coconut source)
    iteration_ = iteration + 1  #255 (line num in coconut source)
    if verbose:  #256 (line num in coconut source)
        (print)(f"Iteration {iteration:03} took {dt:.3f}s | Average Reward: {rewards.mean():.3f}")  #257 (line num in coconut source)
    _ = writer.add_scalar("_Reward_Mean", rewards.mean(), iteration)  #258 (line num in coconut source)
    _ = save_checkpoint(model, model_path)  #259 (line num in coconut source)

## Episode Loop
    return _coconut_tail_call(run_episode, envs, model, episode, iteration_, finish_, buffer_, states_)  #262 (line num in coconut source)

def run_episodes(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #262 (line num in coconut source)
    obs = envs.reset()  #264 (line num in coconut source)
    states = postprocess(obs, envs.info[0])  #265 (line num in coconut source)
    buffer = (ReplayBuffer)(*(tuple)(_coconut_iter_getitem((repeat)(pt.empty(0)), (_coconut.slice(None, 5)))))  #266 (line num in coconut source)
    model = run_episode(envs, model, episode, 0, False, buffer, states)  #267 (line num in coconut source)

    return model  #268 (line num in coconut source)
