...
def postprocess(observations, keys: [str,]):  #202 (line num in coconut source)
    states = ((pt.vstack)([(pt.from_numpy)(obs) for obs in observations])).to(device)  #215 (line num in coconut source)
...
#states_     = postprocess(observations, [i["output-parameters"] for i in infos])
    states_ = postprocess(observations, envs.info[0]["observations"])  #244 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #245 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #246 (line num in coconut source)
    buffer_ = (buffer.push)((ReplayBuffer)(*(states, actions, rewards, states_, dones)))  #247 (line num in coconut source)
    t1 = time.time()  #249 (line num in coconut source)
    dt = t1 - t0  #250 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #251 (line num in coconut source)
    _ = soft_q_update(iteration, model, buffer_, batch_size)  #252 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(dones)  #253 (line num in coconut source)
    stop_ = (((pt.mean)(rewards)).item()) < early_stop  #254 (line num in coconut source)
    finish_ = done_ or stop_ or (iteration >= num_steps) or (dt > max_time)  #255 (line num in coconut source)
    iteration_ = iteration + 1  #256 (line num in coconut source)
    if verbose:  #257 (line num in coconut source)
        (print)(f"Iteration {iteration:03} took {dt:.3f}s | Average Reward: {rewards.mean():.3f}")  #258 (line num in coconut source)
    _ = writer.add_scalar("_Reward_Mean", rewards.mean(), iteration)  #259 (line num in coconut source)
    _ = save_checkpoint(model, model_path)  #260 (line num in coconut source)

## Episode Loop
    return _coconut_tail_call(run_episode, envs, model, episode, iteration_, finish_, buffer_, states_)  #263 (line num in coconut source)

def run_episodes(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #263 (line num in coconut source)
    obs = envs.reset()  #265 (line num in coconut source)
#states  = postprocess(obs, [i["output-parameters"] for i in envs.info])
    states = postprocess(obs, envs.info[0]["observations"])  #267 (line num in coconut source)
    buffer = (ReplayBuffer)(*(tuple)(_coconut_iter_getitem((repeat)(pt.empty(0)), (_coconut.slice(None, 5)))))  #268 (line num in coconut source)
    model = run_episode(envs, model, episode, 0, False, buffer, states)  #269 (line num in coconut source)

    return model  #270 (line num in coconut source)
