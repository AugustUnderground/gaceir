...
ReplayBuffer = namedtuple("ReplayBuffer", "state action reward next_state done")  #72 (line num in coconut source)

def push(b: ReplayBuffer, t: tuple):  #74 (line num in coconut source)
    b_ = ((fmap)(_coconut.operator.itemgetter((_coconut.slice(-buffer_size, None))), (ReplayBuffer)(*t)) if len(b.state) <= 0 else (ReplayBuffer)(*(fmap)(_coconut_forward_compose(pt.vstack, _coconut.operator.itemgetter((_coconut.slice(-buffer_size, None)))), zip(b, t))))  #75 (line num in coconut source)

    return b_  #79 (line num in coconut source)

def sample(b: ReplayBuffer, bs: int):  #79 (line num in coconut source)
    i = (((pt.randperm)(((b.state).shape)[0]))[_coconut.slice(None, bs)]).tolist()  #80 (line num in coconut source)
    s = (ReplayBuffer)(*(e[i, :] for e in b))  #81 (line num in coconut source)

## Neural Networks
    return s  #84 (line num in coconut source)

class HStack(_coconut.collections.namedtuple("HStack", ()), pt.nn.Module):  #84 (line num in coconut source)
    ...
    __match_args__ = ()  #84 (line num in coconut source)
    @_coconut_tco  #84 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #84 (line num in coconut source)
        return _coconut_tail_call((pt.hstack), X)  #85 (line num in coconut source)


class VStack(_coconut.collections.namedtuple("VStack", ()), pt.nn.Module):  #87 (line num in coconut source)
    ...
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #87 (line num in coconut source)
        return _coconut_tail_call((pt.vstack), X)  #88 (line num in coconut source)

## Critic

def critic_net(obs_dim: int, act_dim: int):  #91 (line num in coconut source)
    dim = obs_dim + act_dim  #92 (line num in coconut source)
    crt_net = pt.nn.Sequential(HStack(), pt.nn.Linear(dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #93 (line num in coconut source)

## Actor
    return crt_net  #100 (line num in coconut source)

def actor_net(obs_dim: int, act_dim: int):  #100 (line num in coconut source)
    act_net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 128), pt.nn.ReLU(), pt.nn.Linear(128, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, act_dim), pt.nn.Tanh())  #101 (line num in coconut source)

    return act_net  #107 (line num in coconut source)

def soft_update(source, target, τ=1e-2):  #107 (line num in coconut source)
    for param, target_param in zip(source.parameters(), target.parameters()):  #108 (line num in coconut source)
        target_param.data.copy_(τ * param.data + (1 - τ) * target_param.data)  #109 (line num in coconut source)

## Current Networks
    return target  #112 (line num in coconut source)

Q1 = (critic_net(obs_dim, act_dim)).to(device)  #112 (line num in coconut source)
Q2 = (critic_net(obs_dim, act_dim)).to(device)  #113 (line num in coconut source)
π = (actor_net(obs_dim, act_dim)).to(device)  #114 (line num in coconut source)

## Target Networks
θ1 = (critic_net(obs_dim, act_dim)).to(device)  #117 (line num in coconut source)
θ2 = (critic_net(obs_dim, act_dim)).to(device)  #118 (line num in coconut source)
φ = (actor_net(obs_dim, act_dim)).to(device)  #119 (line num in coconut source)

## Initial Syncronization
_ = soft_update(Q1, θ1, τ=1.0)  #122 (line num in coconut source)
_ = soft_update(Q2, θ2, τ=1.0)  #123 (line num in coconut source)
_ = soft_update(π, φ, τ=1.0)  #124 (line num in coconut source)

## Optimizers
q1_optim = pt.optim.Adam(Q1.parameters(), lr=α, betas=βs)  #127 (line num in coconut source)
q2_optim = pt.optim.Adam(Q2.parameters(), lr=α, betas=βs)  #128 (line num in coconut source)
π_optim = pt.optim.Adam(π.parameters(), lr=α, betas=βs)  #129 (line num in coconut source)

## Loss
q_loss = pt.nn.MSELoss()  #132 (line num in coconut source)
...
def random_action(envs):  #140 (line num in coconut source)
    action = (((pt.vstack)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("sample"), pt.from_numpy), envs.action_space))).detach()).to(device)  #141 (line num in coconut source)
    if verbose:  #143 (line num in coconut source)
        print(f"WARMUP: Selecting Random Action.")  #144 (line num in coconut source)

    return action  #146 (line num in coconut source)

def select_action(net, state, step):  #146 (line num in coconut source)
    with pt.no_grad():  #147 (line num in coconut source)
        action = ((_coconut_partial(add_noise, {}, 1, t=step))((net)(state))).detach()  #148 (line num in coconut source)
    if verbose:  #149 (line num in coconut source)
        print(f"Selecting Action From Model.")  #150 (line num in coconut source)

    return action  #152 (line num in coconut source)

def policy_update(epoch, states):  #152 (line num in coconut source)
    π_loss = ((_coconut_minus))(((Q1)((_coconut_partial((_coconut_comma_op), {0: states}, 2))((π)(states)))).mean())  #153 (line num in coconut source)
    _ = π_optim.zero_grad()  #154 (line num in coconut source)
    _ = π_loss.backward()  #155 (line num in coconut source)
    _ = π_optim.step()  #156 (line num in coconut source)
    _ = soft_update(Q1, θ1, τ=τ_soft)  #157 (line num in coconut source)
    _ = soft_update(Q2, θ2, τ=τ_soft)  #158 (line num in coconut source)
    _ = soft_update(π, φ, τ=τ_soft)  #159 (line num in coconut source)
    _ = writer.add_scalar("π_loss", π_loss, epoch)  #160 (line num in coconut source)
#if verbose:
#    print(f"Actor Update  {epoch:03}/{num_epochs} | π Loss: {π_loss.item():.3f}")

    return π_loss  #164 (line num in coconut source)

def update(epoch, buffer):  #164 (line num in coconut source)
    states, actions, rewards, next_states, dones = (fmap)(_coconut.operator.methodcaller("to", device), sample(buffer, batch_size))  #165 (line num in coconut source)
    noise = ((_coconut_partial(pt.clamp, {1: -clp_noise, 2: clp_noise}, 3))((_coconut_partial(pt.normal, {1: pol_noise}, 2))((pt.zeros)(actions.shape)))).to(device)  #167 (line num in coconut source)
    next_actions = (_coconut_partial(pt.clamp, {1: -1.0, 2: 1.0}, 3))((_coconut_partial((_coconut.operator.add), {1: noise}, 2))((φ)(states)))  #170 (line num in coconut source)
    q1_target = (θ1)((next_states, next_actions))  #171 (line num in coconut source)
    q2_target = (θ2)((next_states, next_actions))  #172 (line num in coconut source)
    q_target = pt.min(q1_target, q2_target)  #173 (line num in coconut source)
    q_expected = ((rewards + (1.0 - dones) * γ * q_target)).detach()  #174 (line num in coconut source)
    q1 = (Q1)((states, actions))  #175 (line num in coconut source)
    q2 = (Q2)((states, actions))  #176 (line num in coconut source)
    q1_loss = q_loss(q1, q_expected)  #177 (line num in coconut source)
    q2_loss = q_loss(q2, q_expected)  #178 (line num in coconut source)
    _ = q1_optim.zero_grad()  #179 (line num in coconut source)
    _ = q1_loss.backward()  #180 (line num in coconut source)
    _ = q1_optim.step()  #181 (line num in coconut source)
    _ = q2_optim.zero_grad()  #182 (line num in coconut source)
    _ = q2_loss.backward()  #183 (line num in coconut source)
    _ = q2_optim.step()  #184 (line num in coconut source)
    π_loss = (policy_update(epoch, states) if epoch in count(0, update_interval) else ((pt.tensor)(pt.nan)).to(device))  #185 (line num in coconut source)
    _ = writer.add_scalar("q1_loss", q1_loss, epoch)  #188 (line num in coconut source)
    _ = writer.add_scalar("q2_loss", q2_loss, epoch)  #189 (line num in coconut source)
#if verbose:
#    print(f"Critic Update {epoch:03}/{num_epochs} | Q1 Loss: {q1_loss.item():.3f}")
#    print(f"                      | Q2 Loss: {q2_loss.item():.3f}")

## Exploration
    return (q1_loss, q2_loss, π_loss)  #195 (line num in coconut source)

def postprocess(observations, infos):  #195 (line num in coconut source)
    states = (((pt.vstack)((list)((starmap)(_coconut_forward_compose((lambda i, o: o[i]), pt.from_numpy), (_coconut_partial(zip, {1: observations}, 2))([[i["output-parameters"].index(p) for p in i["output-parameters"] if (any)((_coconut_partial(fmap, {0: p.startswith}, 2))(["performance", "target", "distance"]))] for i in infos]))))).to(device)).detach()  #200 (line num in coconut source)
#states = observations |> fmap$pt.from_numpy |> pt.vstack |> .to(device) |> .detach() 

    return states  #207 (line num in coconut source)

def explore(envs, step, states, buffer):  #207 (line num in coconut source)
    actions = (random_action(envs) if (step * num_envs) < warmup_periode else select_action(π, states, step))  #209 (line num in coconut source)
    t0 = time.time()  #212 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #213 (line num in coconut source)
    next_states = postprocess(observations, infos)  #217 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #218 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #219 (line num in coconut source)
    new_buffer = push(buffer, (states, actions, rewards, next_states, dones))  #220 (line num in coconut source)
    t1 = time.time()  #221 (line num in coconut source)
    _ = writer.add_scalar("Total_Reward", rewards.sum().item(), step)  #222 (line num in coconut source)
    _ = writer.add_scalar("Average_Reward", rewards.mean().item(), step)  #223 (line num in coconut source)
    if verbose:  #224 (line num in coconut source)
        print(f"Step {step:03} took {(t1 - t0):.3f}s  | Average Reward: {rewards.mean():.3f}")  #225 (line num in coconut source)

## Run Episode until done
    return (next_states, new_buffer)  #228 (line num in coconut source)
...
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #228 (line num in coconut source)
    ...
    if (_coconut.len(_coconut_match_args) == 4) and ("buffer" not in _coconut_match_kwargs) and (_coconut_match_args[3] is True):  #228 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("buffer")  #228 (line num in coconut source)
        if not _coconut_match_kwargs:  #228 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_0  #228 (line num in coconut source)
    ...
    if _coconut_match_check_0:  #228 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #228 (line num in coconut source)
            buffer = _coconut_match_temp_0  #228 (line num in coconut source)
    if not _coconut_match_check_0:  #228 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, _, buffer, True) = buffer', _coconut_match_args)  #228 (line num in coconut source)

    return buffer  #228 (line num in coconut source)
...
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #229 (line num in coconut source)
    ...
    if (_coconut.len(_coconut_match_args) <= 4) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "states" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "done" in _coconut_match_kwargs)) == 1):  #229 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("iteration")  #229 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("states")  #229 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("buffer")  #229 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("done")  #229 (line num in coconut source)
        if not _coconut_match_kwargs:  #229 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_1  #229 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_2  #229 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_3  #229 (line num in coconut source)
            _coconut_match_set_name_done = _coconut_match_temp_4  #229 (line num in coconut source)
    ...
    _ = (fmap)(_coconut.operator.methodcaller("eval"), [π, Q1, Q2, φ, θ1, θ2])  #233 (line num in coconut source)
    expl = lambda sb, e: (_coconut_partial(explore, {0: envs, 1: e}, 3))(*sb)  #234 (line num in coconut source)
    ...
    stop_ = new_buffer.reward[-num_envs:].mean().item() < early_stop  #239 (line num in coconut source)
    finish = done_ or stop_ or (iteration >= 200)  #240 (line num in coconut source)
    _ = (fmap)(_coconut.operator.methodcaller("train"), [π, Q1, Q2, φ, θ1, θ2])  #241 (line num in coconut source)
    q1_loss, q2_loss, π_loss_ = (tuple)((_coconut.functools.partial(_coconut.functools.partial, fmap)(pt.hstack))((zip)(*[update(epoch, new_buffer) for epoch in (range)(num_epochs)])))  #242 (line num in coconut source)
    π_loss = π_loss_[~π_loss_.isnan()]  #245 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #246 (line num in coconut source)

    return _coconut_tail_call(run_episode, iteration + 1, next_states, new_buffer, finish)  #248 (line num in coconut source)

def run_episodes(episode: int):  #248 (line num in coconut source)
    states = ((pt.vstack)((fmap)(_coconut_forward_compose(pt.from_numpy, _coconut.operator.itemgetter((_coconut.slice(None, obs_dim)))), envs.reset()))).to(device)  #249 (line num in coconut source)
    buffer = (ReplayBuffer)(*(list)(_coconut_iter_getitem((repeat)(pt.empty(0)), (_coconut.slice(None, 5)))))  #250 (line num in coconut source)
    memory = run_episode(0, states, buffer, False)  #251 (line num in coconut source)


    return (φ, θ1, θ2)  #253 (line num in coconut source)
