...
a: float = 0.6  # Replay Buffer stuff  #37 (line num in coconut source)
...
## Utility
def write_performance(env: gym.Env, step: int):  #53 (line num in coconut source)
    ...
    for k in target.keys():  #56 (line num in coconut source)
        writer.add_scalars(k, {"performance": performance[k], "target": target[k]}, step)  #57 (line num in coconut source)
...
## Prioritized Experience Replay Buffer
    return performance  #73 (line num in coconut source)

class PERBuffer(_coconut.typing.NamedTuple("PERBuffer", [("priority", pt.Tensor), ("state", pt.Tensor), ("action", pt.Tensor), ("reward", pt.Tensor), ("next_state", pt.Tensor), ("done", pt.Tensor)])):  #73 (line num in coconut source)
    ...
    __match_args__ = ('priority', 'state', 'action', 'reward', 'next_state', 'done')  #73 (line num in coconut source)
    @_coconut_tco  #73 (line num in coconut source)
    def __len__(self):  #73 (line num in coconut source)
        return _coconut_tail_call((int), ((self.state).shape)[0])  #75 (line num in coconut source)

    def β_frame(self, idx: int, β_start: float=0.4, β_frames: int=(int)(1e5)):  #76 (line num in coconut source)
        fβ = min(1.0, (β_start + idx * (1.0 - β_start) / β_frames))  #78 (line num in coconut source)

        return fβ  #80 (line num in coconut source)

def make_per_buffer(state: pt.Tensor=pt.empty(0), action: pt.Tensor=pt.empty(0), reward: pt.Tensor=pt.empty(0), next_state: pt.Tensor=pt.empty(0), done: pt.Tensor=pt.empty(0)):  #80 (line num in coconut source)
    priority = (pt.ones_like)(reward)  #83 (line num in coconut source)
    buffer = (PERBuffer)(*(map)(_coconut.operator.methodcaller("to", device), (priority, state, action, reward, next_state, done)))  #84 (line num in coconut source)

    return buffer  #87 (line num in coconut source)

def push_per_buffer(buffer: PERBuffer, other: PERBuffer):  #87 (line num in coconut source)
    prio = (((pt.ones_like)(other.priority)).to(device)) * ((pt.max)(buffer.priority) if ((len)(buffer)) > 0 else 1.0)  #88 (line num in coconut source)
    buf_ = (PERBuffer)(*(map)(pt.cat, (_coconut_partial(zip, {0: buffer}, 2))((PERBuffer)(prio, *(map)(_coconut.operator.methodcaller("to", device), other[1:])))))  #92 (line num in coconut source)
    idx = (((_coconut_partial(pt.argsort, {}, 1, descending=True))((pt.squeeze)(buf_.priority)))[_coconut.slice(None, buffer_size)]).tolist()  #94 (line num in coconut source)
    buf = (PERBuffer)(*(map)(_coconut.operator.itemgetter((idx)), buf_))  #96 (line num in coconut source)

    return buf  #98 (line num in coconut source)

def update_per_buffer(buffer: PERBuffer, idx: list[int], priority: pt.Tensor):  #98 (line num in coconut source)
    _, state, action, reward, next_state, done = map(_coconut.operator.itemgetter((idx)), buffer)  #100 (line num in coconut source)
    prio = ((priority).to(device)).reshape(-1, 1)  #102 (line num in coconut source)
    buf_ = PERBuffer(prio, state, action, reward, next_state, done)  #103 (line num in coconut source)
    xdi = (list)(filter(lambda i: i not in idx, (range)((len)(buffer))))  #104 (line num in coconut source)
    buf = (PERBuffer)(*(map)(_coconut.operator.itemgetter((xdi)), buffer))  #105 (line num in coconut source)
    fsi = ((_coconut_partial(pt.argsort, {}, 1, descending=True))((pt.cat)((tuple)((map)(_coconut_forward_compose(_coconut.operator.attrgetter("priority"), pt.squeeze), (buf, buf_)))))).tolist()  #106 (line num in coconut source)
    buffer_ = (PERBuffer)(*(map)(_coconut_forward_compose(pt.cat, _coconut.operator.itemgetter((fsi))), zip(buf, buf_)))  #108 (line num in coconut source)

    return buffer_  #110 (line num in coconut source)

def sample_per_buffer(buffer: PERBuffer, batch_size: int, frame: int, α: float=0.6):  #110 (line num in coconut source)
    n = (len)(buffer)  #113 (line num in coconut source)
    probs = (_coconut_partial(pt.pow, {1: α}, 2))(buffer.priority)  #114 (line num in coconut source)
    p = (pt.squeeze)(probs / pt.sum(probs))  #115 (line num in coconut source)
    indices = ((_coconut.functools.partial(_coconut_iter_getitem, pt.arange(0, n)))(p.multinomial(num_samples=batch_size, replacement=False))).tolist()  #116 (line num in coconut source)
    smpl = (PERBuffer)(*(map)(_coconut.operator.itemgetter((indices)), buffer))  #118 (line num in coconut source)
    β = buffer.β_frame(frame)  #119 (line num in coconut source)
    w = pt.pow(n * p[indices], -β)  #120 (line num in coconut source)
    weights = w / pt.max(w)  #121 (line num in coconut source)

## Neural Networks
    return (smpl, indices, weights)  #124 (line num in coconut source)

class HStack(_coconut.collections.namedtuple("HStack", ()), pt.nn.Module):  #124 (line num in coconut source)
    _coconut_is_data = True  #124 (line num in coconut source)
    __slots__ = ()  #124 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #124 (line num in coconut source)
    def __eq__(self, other):  #124 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #124 (line num in coconut source)
    def __hash__(self):  #124 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #124 (line num in coconut source)
    __match_args__ = ()  #124 (line num in coconut source)
    @_coconut_tco  #124 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #124 (line num in coconut source)
        return _coconut_tail_call((pt.hstack), X)  #125 (line num in coconut source)


class VStack(_coconut.collections.namedtuple("VStack", ()), pt.nn.Module):  #127 (line num in coconut source)
    _coconut_is_data = True  #127 (line num in coconut source)
    __slots__ = ()  #127 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #127 (line num in coconut source)
    def __eq__(self, other):  #127 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #127 (line num in coconut source)
    def __hash__(self):  #127 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #127 (line num in coconut source)
    __match_args__ = ()  #127 (line num in coconut source)
    @_coconut_tco  #127 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #127 (line num in coconut source)
        return _coconut_tail_call((pt.vstack), X)  #128 (line num in coconut source)

## Actor Network

class ActorNet(_coconut.typing.NamedTuple("ActorNet", [("obs_dim", int), ("act_dim", int), ("σ_min", float), ("σ_max", float)]), pt.nn.Module):  #131 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim', 'act_dim', 'σ_min', 'σ_max')  #131 (line num in coconut source)
    def __new__(_coconut_cls, obs_dim, act_dim, σ_min=-2.0, σ_max=20.0):  #131 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (obs_dim, act_dim, σ_min, σ_max))  #131 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #133 (line num in coconut source)
        super(ActorNet, self).__init__()  #134 (line num in coconut source)
        self.lin_1 = pt.nn.Linear(obs_dim, 256)  #135 (line num in coconut source)
        self.lin_2 = pt.nn.Linear(256, 128)  #136 (line num in coconut source)
        self.lin_3 = pt.nn.Linear(128, 64)  #137 (line num in coconut source)
        self.lin_μ = pt.nn.Linear(64, act_dim)  #138 (line num in coconut source)
        self.lin_σ = pt.nn.Linear(64, act_dim)  #139 (line num in coconut source)
        self.relu = pt.nn.functional.relu  #140 (line num in coconut source)
        self.min_σ = σ_min  #141 (line num in coconut source)
        self.max_σ = σ_max  #142 (line num in coconut source)
        self.lin_σ.weight.data.uniform_(-3e-3, 3e-3)  #143 (line num in coconut source)
        self.lin_σ.bias.data.uniform_(-3e-3, 3e-3)  #144 (line num in coconut source)
...
#critic_loss   = pt.nn.MSELoss()
    ...
    _ = [(n.apply)(_coconut_partial(init_weights, {1: 3e-3}, 2)) for n in [actor, critic_1, critic_2, value_online]]  #211 (line num in coconut source)
    ...
    return model  #228 (line num in coconut source)

def scale_rewards(reward: pt.Tensor, ρ: float=1e-3):  #228 (line num in coconut source)
    scaled_reward = (reward - pt.mean(reward)) / (pt.std(reward) + ρ)  #229 (line num in coconut source)

## Update
    return scaled_reward  #232 (line num in coconut source)

def soft_q_update(epoch: int, model: Model, buffer: PERBuffer, batch_size: int, μλ: float=1e-3, σλ: float=1e-3, zλ: float=0.0, γ: float=0.99):  #232 (line num in coconut source)
    buffer_sample, idx, weights = (_coconut_partial(sample_per_buffer, {1: batch_size, 2: epoch}, 3))(buffer)  #235 (line num in coconut source)
    _, states, actions, rewards_, next_states, dones = (map)(_coconut.operator.methodcaller("to", device), buffer_sample)  #237 (line num in coconut source)
    rewards = (scale_rewards)(rewards_)  #239 (line num in coconut source)
#rewards     = rewards_
    new_actions, log_prob, z, μ, log_σ = (model.evaluate)(states)  #241 (line num in coconut source)
    q_exp_1 = (model.critic_1)(*(states, actions))  #243 (line num in coconut source)
    q_exp_2 = (model.critic_2)(*(states, actions))  #244 (line num in coconut source)
    v_expected = (model.value_online)(states)  #245 (line num in coconut source)
    v_target = (model.value_target)(next_states)  #246 (line num in coconut source)
    q_next = ((rewards + (1 - dones) * γ * v_target)).detach()  #247 (line num in coconut source)
#c1_loss     = (q_exp_1, q_next) |*> model.critic_loss
#c2_loss     = (q_exp_2, q_next) |*> model.critic_loss
    c1_loss = (model.critic_loss)(*(q_exp_1, q_next, weights))  #250 (line num in coconut source)
    c2_loss = (model.critic_loss)(*(q_exp_2, q_next, weights))  #251 (line num in coconut source)
    q_exp_1_nxt = (model.critic_1)(*(states, new_actions))  #252 (line num in coconut source)
    q_exp_2_nxt = (model.critic_2)(*(states, new_actions))  #253 (line num in coconut source)
    q_exp_nxt = (pt.min)(*(q_exp_1_nxt, q_exp_2_nxt))  #254 (line num in coconut source)
    v_next = ((q_exp_nxt - log_prob)).detach()  #255 (line num in coconut source)
    v_loss = (model.value_loss)(*(v_expected, v_next))  #256 (line num in coconut source)
    l_target = q_exp_nxt - v_expected  #257 (line num in coconut source)
    l_loss = (((log_prob * (log_prob - l_target))).detach()).mean()  #258 (line num in coconut source)
    μ_loss = (_coconut_partial((_coconut.operator.mul), {0: μλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(μ)))  #259 (line num in coconut source)
    σ_loss = (_coconut_partial((_coconut.operator.mul), {0: σλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(log_σ)))  #260 (line num in coconut source)
    z_loss = (_coconut_partial((_coconut.operator.mul), {0: zλ}, 2))((pt.mean)(((_coconut_partial(pt.pow, {1: 2}, 2))(z)).sum(1)))  #261 (line num in coconut source)
    a_loss = l_loss + μ_loss + σ_loss + z_loss  #262 (line num in coconut source)
    _ = model.critic_1_opt.zero_grad()  #263 (line num in coconut source)
    _ = c1_loss.backward()  #264 (line num in coconut source)
    _ = model.critic_1_opt.step()  #265 (line num in coconut source)
    _ = model.critic_2_opt.zero_grad()  #266 (line num in coconut source)
    _ = c2_loss.backward()  #267 (line num in coconut source)
    _ = model.critic_2_opt.step()  #268 (line num in coconut source)
    _ = model.value_opt.zero_grad()  #269 (line num in coconut source)
    _ = v_loss.backward()  #270 (line num in coconut source)
    _ = model.value_opt.step()  #271 (line num in coconut source)
    _ = model.actor_opt.zero_grad()  #272 (line num in coconut source)
    _ = a_loss.backward()  #273 (line num in coconut source)
    _ = model.actor_opt.step()  #274 (line num in coconut source)
    _ = model.soft_sync()  #275 (line num in coconut source)
    td_1_err = ((q_next - q_exp_1)).detach()  #276 (line num in coconut source)
    td_2_err = ((q_next - q_exp_2)).detach()  #277 (line num in coconut source)
    priorities = (pt.squeeze)((pt.abs)((((td_1_err + td_2_err) / 2.0) + 1e-5)))  #278 (line num in coconut source)
    buffer_ = (_coconut_partial(update_per_buffer, {1: idx, 2: priorities}, 3))(buffer)  #279 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Actor", a_loss, epoch)  #280 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic_1", c1_loss, epoch)  #281 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic_2", c2_loss, epoch)  #282 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Value", v_loss, epoch)  #283 (line num in coconut source)

## Exploration
    return buffer_  #286 (line num in coconut source)

def postprocess(observations, keys: dict[str, list[str]]):  #286 (line num in coconut source)
    pf = lambda k: ":" not in k and "/" not in k and (k[0]).islower()  #287 (line num in coconut source)
    lf = lambda k: k in ["ugbw", "cof", "sr_f", "sr_r"] or k.endswith("fug")  #288 (line num in coconut source)
    p_idx = (list)((fmap)(keys["observations"].index, (filter)(pf, keys["observations"])))  #289 (line num in coconut source)
    i_idx = [(keys["observations"].index)(k) for k in keys["actions"]]  #292 (line num in coconut source)
    idx = (sorted)((p_idx + i_idx))  #293 (line num in coconut source)
    l_msk = ((pt.Tensor)((list)((_coconut_partial(map, {0: lf}, 2))(keys["observations"])))).to(device)  #294 (line num in coconut source)
    of = lambda o: (pt.where(o > 0, pt.log10(o), o) * l_msk) + (o * (1 - l_msk))  #295 (line num in coconut source)
    states = (pt.vstack)([((of)(((pt.from_numpy)(obs)).to(device)))[idx] for obs in observations])  #296 (line num in coconut source)
#states  = (pt.log10(states_) *  l_msk) + (states_ * (~l_msk))
#states = [ obs |> pt.from_numpy for obs in observations
#         ] |> pt.vstack |> .to(device)

## Checkpoint Saving
    return states  #303 (line num in coconut source)

def save_checkpoint(model: Model, checkpoint_file: str):  #303 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [model.actor, model.critic_1, model.critic_2, model.value_online, model.value_target, model.actor_opt, model.critic_1_opt, model.critic_2_opt, model.value_opt])  #304 (line num in coconut source)
    keys = ["actor", "critic_1", "critic_2", "value_online", "value_target", "actor_opt", "critic_1_opt", "critic_2_opt", "value_opt"]  #308 (line num in coconut source)
    save_dict = (dict)((zip)(*(keys, state_dicts)))  #311 (line num in coconut source)
    res = (pt.save)(*(save_dict, checkpoint_file))  #312 (line num in coconut source)

## Training Loop
    return res  #315 (line num in coconut source)

@_coconut_mark_as_match  #315 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #315 (line num in coconut source)
    _coconut_match_check_0 = False  #315 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #315 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #315 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #315 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #315 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) == 7) and ("model" not in _coconut_match_kwargs) and ("episode" not in _coconut_match_kwargs) and ("buffer" not in _coconut_match_kwargs) and (_coconut_match_args[4] is True):  #315 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #315 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #315 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #315 (line num in coconut source)
        if not _coconut_match_kwargs:  #315 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_0  #315 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_1  #315 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_2  #315 (line num in coconut source)
            _coconut_match_check_0 = True  #315 (line num in coconut source)
    if _coconut_match_check_0:  #315 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #315 (line num in coconut source)
            model = _coconut_match_temp_0  #315 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #315 (line num in coconut source)
            episode = _coconut_match_temp_1  #315 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #315 (line num in coconut source)
            buffer = _coconut_match_temp_2  #315 (line num in coconut source)
    if not _coconut_match_check_0:  #315 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, model, episode, _, True, buffer,_) = model where:', _coconut_match_args)  #315 (line num in coconut source)

    total = ((buffer.reward).sum()).item()  #316 (line num in coconut source)
    _ = writer.add_scalar(f"_Reward_Total", total, episode)  #317 (line num in coconut source)
    if verbose:  #318 (line num in coconut source)
        (print)(f"Episode {episode:03} Finished | Total Reward: {total}")  #319 (line num in coconut source)
    return model  #320 (line num in coconut source)

@_coconut_addpattern(run_episode)  #320 (line num in coconut source)
@_coconut_tco  #320 (line num in coconut source)
...
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #320 (line num in coconut source)
    _coconut_match_check_1 = False  #320 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #320 (line num in coconut source)
    ...
    _coconut_match_set_name_iteration = _coconut_sentinel  #320 (line num in coconut source)
    _coconut_match_set_name_finish = _coconut_sentinel  #320 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #320 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #320 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #320 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) <= 7) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "envs" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "model" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "episode" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "finish" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 5, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "states" in _coconut_match_kwargs)) == 1):  #320 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("envs")  #320 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #320 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #320 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("iteration")  #320 (line num in coconut source)
        _coconut_match_temp_7 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("finish")  #320 (line num in coconut source)
        _coconut_match_temp_8 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #320 (line num in coconut source)
        _coconut_match_temp_9 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("states")  #320 (line num in coconut source)
        if not _coconut_match_kwargs:  #320 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_3  #320 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_4  #320 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_5  #320 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_6  #320 (line num in coconut source)
            _coconut_match_set_name_finish = _coconut_match_temp_7  #320 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_8  #320 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_9  #320 (line num in coconut source)
            _coconut_match_check_1 = True  #320 (line num in coconut source)
    if _coconut_match_check_1:  #320 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #320 (line num in coconut source)
            envs = _coconut_match_temp_3  #320 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #320 (line num in coconut source)
            model = _coconut_match_temp_4  #320 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #320 (line num in coconut source)
            episode = _coconut_match_temp_5  #320 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #320 (line num in coconut source)
            iteration = _coconut_match_temp_6  #320 (line num in coconut source)
        if _coconut_match_set_name_finish is not _coconut_sentinel:  #320 (line num in coconut source)
            finish = _coconut_match_temp_7  #320 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #320 (line num in coconut source)
            buffer = _coconut_match_temp_8  #320 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #320 (line num in coconut source)
            states = _coconut_match_temp_9  #320 (line num in coconut source)
    if not _coconut_match_check_1:  #320 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode( envs, model, episode, iteration, finish, buffer, states\n) = run_episode( envs, model, episode, iteration_\n, finish_, buffer_, states_ ) where:', _coconut_match_args)  #320 (line num in coconut source)

    t0 = time.time()  #323 (line num in coconut source)
    actions = (model.act)(states)  #324 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #325 (line num in coconut source)
    states_ = postprocess(observations, envs.info[0])  #329 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #330 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #331 (line num in coconut source)
    buf = (_coconut_partial(push_per_buffer, {0: buffer}, 2))((make_per_buffer)(*(states, actions, rewards, states_, dones)))  #332 (line num in coconut source)
    t1 = time.time()  #334 (line num in coconut source)
    dt = t1 - t0  #335 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #336 (line num in coconut source)
    buffer_ = (soft_q_update(iteration, model, buf, batch_size) if len(buf) > batch_size else buf)  #337 (line num in coconut source)
    done_ = (_coconut_forward_compose(pt.squeeze, _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(dones)  #340 (line num in coconut source)
    stop_ = (((pt.mean)(rewards)).item()) < early_stop  #341 (line num in coconut source)
    finish_ = done_ or stop_ or (iteration >= num_steps)  # or (dt > max_time)  #342 (line num in coconut source)
    iteration_ = iteration + 1  #343 (line num in coconut source)
    if verbose:  #344 (line num in coconut source)
        (print)(f"Iteration {iteration:03} took {dt:.3f}s | Average Reward: {rewards.mean():.3f}")  #345 (line num in coconut source)
    _ = writer.add_scalar("_Reward_Mean", rewards.mean(), iteration)  #346 (line num in coconut source)
    _ = save_checkpoint(model, model_path)  #347 (line num in coconut source)

## Episode Loop
    return _coconut_tail_call(run_episode, envs, model, episode, iteration_, finish_, buffer_, states_)  #350 (line num in coconut source)

def run_episodes(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #350 (line num in coconut source)
    obs = envs.reset()  #352 (line num in coconut source)
    states = postprocess(obs, envs.info[0])  #353 (line num in coconut source)
#buffer  = pt.empty(0) |> .to(device) |> repeat |> .$[:5] |> tuple |*> ReplayBuffer
    buffer = make_per_buffer()  #355 (line num in coconut source)
    model = run_episode(envs, model, episode, 0, False, buffer, states)  #356 (line num in coconut source)

    return model  #357 (line num in coconut source)
