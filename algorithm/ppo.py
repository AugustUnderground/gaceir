...
def update_policy(model: Model, states: pt.Tensor, actions: pt.Tensor, logprobs: pt.Tensor, returns: pt.Tensor, advantages: pt.Tensor):  #114 (line num in coconut source)
    ...
    return losses  #130 (line num in coconut source)

def postprocess(observations, keys):  #130 (line num in coconut source)
    idx = [(list)((map)(lambda p: (key.index)(p), (_coconut_partial(filter, {0: lambda k: ":" not in k}, 2))(key))) for key in keys]  #131 (line num in coconut source)
    states = ((pt.vstack)((fmap)(pt.from_numpy, [o[i] for o, i in zip(observations, idx)]))).to(device)  #133 (line num in coconut source)

    return states  #136 (line num in coconut source)

def trajectories(model: Model, envs: gace.envs.vec.VecACE, states: pt.Tensor):  #136 (line num in coconut source)
    dist, values = (model.act_cont)(states)  #138 (line num in coconut source)
    actions = ((dist).sample()).detach()  #139 (line num in coconut source)
    logprobs = (dist.log_prob)(actions)  #140 (line num in coconut source)
    observations, rewards_, dones, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #141 (line num in coconut source)
    next_states = postprocess(observations, [i["output-parameters"] for i in infos])  #145 (line num in coconut source)
    masks = (_coconut_partial((_coconut_minus), {0: 1}, 2))(((pt.Tensor)(dones)).to(device))  #146 (line num in coconut source)
    rewards = ((pt.Tensor)(rewards_)).to(device)  #147 (line num in coconut source)
    memories = Memory(states, actions, logprobs, rewards, values, masks)  #148 (line num in coconut source)

    return (memories, next_states)  #150 (line num in coconut source)

def gae(r: pt.Tensor, v: pt.Tensor, m: pt.Tensor, v_: pt.Tensor, γ: float=0.99, τ: float=0.95):  #150 (line num in coconut source)
    δ = r + γ * v_ * m - v  #152 (line num in coconut source)
    l = (reversed)((range)(((δ).shape)[0]))  #153 (line num in coconut source)
    i = pt.Tensor([0,]).to(device)  #154 (line num in coconut source)
    g = (reduce(lambda g_, i_: (pt.hstack)((_coconut_partial((_coconut_comma_op), {1: g_}, 2))(δ[i_] + γ * τ * m[i_] * g_[0])), l, i))[_coconut.slice(None, -1)]  #155 (line num in coconut source)
    a = v + g  #157 (line num in coconut source)

    return a  #159 (line num in coconut source)

def save_checkpoint(model: Model, checkpoint_file: str):  #159 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [model.actor, model.critic, model.optim])  #160 (line num in coconut source)
    state_keys = ["actor", "critic", "optim"]  #161 (line num in coconut source)
    save_dict = (dict)((zip)(*(state_keys, state_dicts)))  #162 (line num in coconut source)
    res = pt.save(save_dict, checkpoint_file)  #163 (line num in coconut source)

    return res  #165 (line num in coconut source)

@_coconut_mark_as_match  #165 (line num in coconut source)
def traject(*_coconut_match_args, **_coconut_match_kwargs):  #165 (line num in coconut source)
    _coconut_match_check_0 = False  #165 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #165 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #165 (line num in coconut source)
    _coconut_match_set_name_memories = _coconut_sentinel  #165 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #165 (line num in coconut source)
    _coconut_match_set_name_step = _coconut_sentinel  #165 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #165 (line num in coconut source)
    if (3 <= _coconut.len(_coconut_match_args) <= 4) and ("model" not in _coconut_match_kwargs) and ("envs" not in _coconut_match_kwargs) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "step" in _coconut_match_kwargs)) == 1) and (_coconut.isinstance(_coconut_match_args[2], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_args[2]) == 2):  #165 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("model")  #165 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("envs")  #165 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("step")  #165 (line num in coconut source)
        _coconut_match_set_name_memories = _coconut_match_args[2][0]  #165 (line num in coconut source)
        _coconut_match_set_name_states = _coconut_match_args[2][1]  #165 (line num in coconut source)
        if not _coconut_match_kwargs:  #165 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_0  #165 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_1  #165 (line num in coconut source)
            _coconut_match_set_name_step = _coconut_match_temp_2  #165 (line num in coconut source)
            _coconut_match_check_0 = True  #165 (line num in coconut source)
    if _coconut_match_check_0:  #165 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #165 (line num in coconut source)
            model = _coconut_match_temp_0  #165 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #165 (line num in coconut source)
            envs = _coconut_match_temp_1  #165 (line num in coconut source)
        if _coconut_match_set_name_memories is not _coconut_sentinel:  #165 (line num in coconut source)
            memories = _coconut_match_args[2][0]  #165 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #165 (line num in coconut source)
            states = _coconut_match_args[2][1]  #165 (line num in coconut source)
        if _coconut_match_set_name_step is not _coconut_sentinel:  #165 (line num in coconut source)
            step = _coconut_match_temp_2  #165 (line num in coconut source)
    if not _coconut_match_check_0:  #165 (line num in coconut source)
        raise _coconut_FunctionMatchError('def traject(model, envs, (memories,states), step) = (memories_, states_) where:', _coconut_match_args)  #165 (line num in coconut source)

    t0 = time.time()  #166 (line num in coconut source)
    memory, states_ = trajectories(model, envs, states)  #167 (line num in coconut source)
    memories_ = memory + memories  #168 (line num in coconut source)
    t1 = time.time()  #169 (line num in coconut source)
    if verbose:  #170 (line num in coconut source)
        r = memory.rewards.mean()  #171 (line num in coconut source)
        print(f"Step {step:03} took {(t1 - t0):.3f}s | Average Reward: {r:.3f}")  #172 (line num in coconut source)
    writer.add_scalar("_Reward_Mean", memory.rewards.mean(), step)  #173 (line num in coconut source)

    return (memories_, states_)  #175 (line num in coconut source)

@_coconut_tco  #175 (line num in coconut source)
def train(model, epoch, loader):  #175 (line num in coconut source)
    t0 = time.time()  #176 (line num in coconut source)
    loss = (pt.cat)((list)(starmap(_coconut.functools.partial(update_policy, model), loader)))  #177 (line num in coconut source)
    t1 = time.time()  #178 (line num in coconut source)
    writer.add_scalar("_Loss_Mean", loss.mean().item(), epoch)  #179 (line num in coconut source)

    return _coconut_tail_call((pt.mean), loss)  #181 (line num in coconut source)

@_coconut_mark_as_match  #181 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #181 (line num in coconut source)
    _coconut_match_check_1 = False  #181 (line num in coconut source)
    _coconut_match_set_name_loss = _coconut_sentinel  #181 (line num in coconut source)
    _coconut_match_set_name_reward = _coconut_sentinel  #181 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #181 (line num in coconut source)
    if (6 <= _coconut.len(_coconut_match_args) <= 8) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "loss" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 7, "reward" in _coconut_match_kwargs)) == 1) and (_coconut_match_args[5] is True):  #181 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("loss")  #181 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[7] if _coconut.len(_coconut_match_args) > 7 else _coconut_match_kwargs.pop("reward")  #181 (line num in coconut source)
        if not _coconut_match_kwargs:  #181 (line num in coconut source)
            _coconut_match_set_name_loss = _coconut_match_temp_3  #181 (line num in coconut source)
            _coconut_match_set_name_reward = _coconut_match_temp_4  #181 (line num in coconut source)
            _coconut_match_check_1 = True  #181 (line num in coconut source)
    if _coconut_match_check_1:  #181 (line num in coconut source)
        if _coconut_match_set_name_loss is not _coconut_sentinel:  #181 (line num in coconut source)
            loss = _coconut_match_temp_3  #181 (line num in coconut source)
        if _coconut_match_set_name_reward is not _coconut_sentinel:  #181 (line num in coconut source)
            reward = _coconut_match_temp_4  #181 (line num in coconut source)
    if not _coconut_match_check_1:  #181 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, _, _, _, _, True, loss, reward) = model where:', _coconut_match_args)  #181 (line num in coconut source)

    total = ((reward).sum()).item()  #182 (line num in coconut source)
    _ = writer.add_scalar(f"_Reward_Total", total, episode)  #183 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Sum", loss.sum().item(), epoch)  #184 (line num in coconut source)
    if verbose:  #185 (line num in coconut source)
        (print)(f"Episode {episode:03} Finished | Total Reward: {total}")  #186 (line num in coconut source)
    return model  #187 (line num in coconut source)

@_coconut_addpattern(run_episode)  #187 (line num in coconut source)
@_coconut_tco  #187 (line num in coconut source)
@_coconut_mark_as_match  #187 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #187 (line num in coconut source)
    _coconut_match_check_2 = False  #187 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #187 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #187 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #187 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #187 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #187 (line num in coconut source)
    _coconut_match_set_name_loss = _coconut_sentinel  #187 (line num in coconut source)
    _coconut_match_set_name_reward = _coconut_sentinel  #187 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #187 (line num in coconut source)
    if (6 <= _coconut.len(_coconut_match_args) <= 8) and ("envs" not in _coconut_match_kwargs) and ("model" not in _coconut_match_kwargs) and ("episode" not in _coconut_match_kwargs) and ("iteration" not in _coconut_match_kwargs) and ("states" not in _coconut_match_kwargs) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "loss" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 7, "reward" in _coconut_match_kwargs)) == 1):  #187 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("envs")  #187 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #187 (line num in coconut source)
        _coconut_match_temp_7 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #187 (line num in coconut source)
        _coconut_match_temp_8 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("iteration")  #187 (line num in coconut source)
        _coconut_match_temp_9 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("states")  #187 (line num in coconut source)
        _coconut_match_temp_10 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("loss")  #187 (line num in coconut source)
        _coconut_match_temp_11 = _coconut_match_args[7] if _coconut.len(_coconut_match_args) > 7 else _coconut_match_kwargs.pop("reward")  #187 (line num in coconut source)
        if not _coconut_match_kwargs:  #187 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_5  #187 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_6  #187 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_7  #187 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_8  #187 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_9  #187 (line num in coconut source)
            _coconut_match_set_name_loss = _coconut_match_temp_10  #187 (line num in coconut source)
            _coconut_match_set_name_reward = _coconut_match_temp_11  #187 (line num in coconut source)
            _coconut_match_check_2 = True  #187 (line num in coconut source)
    if _coconut_match_check_2:  #187 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #187 (line num in coconut source)
            envs = _coconut_match_temp_5  #187 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #187 (line num in coconut source)
            model = _coconut_match_temp_6  #187 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #187 (line num in coconut source)
            episode = _coconut_match_temp_7  #187 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #187 (line num in coconut source)
            iteration = _coconut_match_temp_8  #187 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #187 (line num in coconut source)
            states = _coconut_match_temp_9  #187 (line num in coconut source)
        if _coconut_match_set_name_loss is not _coconut_sentinel:  #187 (line num in coconut source)
            loss = _coconut_match_temp_10  #187 (line num in coconut source)
        if _coconut_match_set_name_reward is not _coconut_sentinel:  #187 (line num in coconut source)
            reward = _coconut_match_temp_11  #187 (line num in coconut source)
    if not _coconut_match_check_2:  #187 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode(envs, model, episode, iteration, states, _, loss, reward\n) = run_episode( envs, model, episode, iteration_\n, next_states, done, loss_, reward_\n) where:', _coconut_match_args)  #187 (line num in coconut source)

    _ = model.eval_mode()  #191 (line num in coconut source)
    with pt.no_grad():  #192 (line num in coconut source)
        memories_, next_states = reduce(_coconut.functools.partial(traject, model, envs), range(num_steps), ((Memory)(*_coconut_iter_getitem((repeat)((pt.empty(0)).to(device)), (_coconut.slice(None, 6)))), states))  #193 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(((envs).gace_envs)[0])  #197 (line num in coconut source)
    _ = model.train_mode()  #198 (line num in coconut source)
    loader = data_loader(memories_)  #199 (line num in coconut source)
    losses = ((pt.stack)([train(model, epoch, loader) for epoch in (range)(num_epochs)])).detach()  #200 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))((1 - memories_.masks))  #202 (line num in coconut source)
    stop_ = (((pt.mean)(memories_.rewards)).item()) < early_stop  #203 (line num in coconut source)
    done = done_ or stop_ or (iteration >= max_iter)  #204 (line num in coconut source)
    loss_ = pt.cat((loss, losses))  #205 (line num in coconut source)
    reward_ = pt.cat((reward, memories_.rewards))  #206 (line num in coconut source)
    iteration_ = iteration + 1  #207 (line num in coconut source)
    if verbose:  #208 (line num in coconut source)
        r = memories_.rewards.mean().item()  #209 (line num in coconut source)
        print(f"Epsiode {episode:03}, Iteration {iteration:03} | Average Reward {r:.3f}")  #210 (line num in coconut source)
    _ = save_checkpoint(model, model_path)  #211 (line num in coconut source)

    return _coconut_tail_call(run_episode, envs, model, episode, iteration_, next_states, done, loss_, reward_)  #213 (line num in coconut source)

def run_episodes(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #213 (line num in coconut source)
    obs = envs.reset()  #215 (line num in coconut source)
    states = postprocess(obs, envs.observation_keys)  #216 (line num in coconut source)
    l = r = (pt.empty(0)).to(device=device)  #217 (line num in coconut source)
    losses, rewards = run_episode(envs, model, episode, 0, states, False, l, r)  #218 (line num in coconut source)

    return model  #219 (line num in coconut source)
