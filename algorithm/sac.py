...
class ReplayBuffer(_coconut.typing.NamedTuple("ReplayBuffer", [("state", pt.Tensor), ("action", pt.Tensor), ("reward", pt.Tensor), ("next_state", pt.Tensor), ("done", pt.Tensor)])):  #59 (line num in coconut source)
    ...
    def __len__(self):  #59 (line num in coconut source)
        return _coconut_tail_call((int), ((self.state).shape)[0])  #61 (line num in coconut source)

    def __add__(self, other: ReplayBuffer):  #62 (line num in coconut source)
        buf = (ReplayBuffer)(*(map)(_coconut_forward_compose(pt.cat, _coconut.operator.itemgetter((_coconut.slice(-buffer_size, None)))), (_coconut_partial(zip, {0: self}, 2))((map)(_coconut.operator.methodcaller("to", device), other))))  #63 (line num in coconut source)

        return buf  #66 (line num in coconut source)

def sample_buffer(buffer: ReplayBuffer, batch_size: int):  #66 (line num in coconut source)
    idx = (((pt.randperm)((len)(buffer)))[_coconut.slice(None, batch_size)]).tolist()  #67 (line num in coconut source)
    smpl = (ReplayBuffer)(*(map)(_coconut.operator.itemgetter((idx, _coconut.slice(None, None))), buffer))  #68 (line num in coconut source)


## Neural Networks
    return smpl  #72 (line num in coconut source)

class HStack(_coconut.collections.namedtuple("HStack", ()), pt.nn.Module):  #72 (line num in coconut source)
    _coconut_is_data = True  #72 (line num in coconut source)
    __slots__ = ()  #72 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #72 (line num in coconut source)
    def __eq__(self, other):  #72 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #72 (line num in coconut source)
    def __hash__(self):  #72 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #72 (line num in coconut source)
    __match_args__ = ()  #72 (line num in coconut source)
    @_coconut_tco  #72 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #72 (line num in coconut source)
        return _coconut_tail_call((pt.hstack), X)  #73 (line num in coconut source)


class VStack(_coconut.collections.namedtuple("VStack", ()), pt.nn.Module):  #75 (line num in coconut source)
    ...
    __match_args__ = ()  #75 (line num in coconut source)
    @_coconut_tco  #75 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #75 (line num in coconut source)
        return _coconut_tail_call((pt.vstack), X)  #76 (line num in coconut source)

## Value Network

class ValueNet(_coconut.typing.NamedTuple("ValueNet", [("obs_dim", int)]), pt.nn.Module):  #79 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim',)  #79 (line num in coconut source)
    def __init__(self, obs_dim: int):  #80 (line num in coconut source)
        super(ValueNet, self).__init__()  #81 (line num in coconut source)
        self.v_net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #82 (line num in coconut source)

    @_coconut_tco  #83 (line num in coconut source)
    def forward(self, state: pt.Tensor):  #83 (line num in coconut source)
        return _coconut_tail_call((self.v_net), state)  #86 (line num in coconut source)

## Critic Network

class CriticNet(_coconut.typing.NamedTuple("CriticNet", [("obs_dim", int), ("act_dim", int)]), pt.nn.Module):  #89 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim', 'act_dim')  #89 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #90 (line num in coconut source)
        super(CriticNet, self).__init__()  #91 (line num in coconut source)
        dim = obs_dim + act_dim  #92 (line num in coconut source)
        self.c_net = pt.nn.Sequential(HStack(), pt.nn.Linear(dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #93 (line num in coconut source)

    @_coconut_tco  #94 (line num in coconut source)
    def forward(self, state: pt.Tensor, action: pt.Tensor):  #94 (line num in coconut source)
        return _coconut_tail_call((self.c_net), (state, action))  #98 (line num in coconut source)

## Actor Network

class ActorNet(_coconut.typing.NamedTuple("ActorNet", [("obs_dim", int), ("act_dim", int), ("σ_min", float), ("σ_max", float)]), pt.nn.Module):  #101 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim', 'act_dim', 'σ_min', 'σ_max')  #101 (line num in coconut source)
    def __new__(_coconut_cls, obs_dim, act_dim, σ_min=-2.0, σ_max=20.0):  #101 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (obs_dim, act_dim, σ_min, σ_max))  #101 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #103 (line num in coconut source)
        super(ActorNet, self).__init__()  #104 (line num in coconut source)
        self.lin_1 = pt.nn.Linear(obs_dim, 256)  #105 (line num in coconut source)
        self.lin_2 = pt.nn.Linear(256, 128)  #106 (line num in coconut source)
        self.lin_3 = pt.nn.Linear(128, 64)  #107 (line num in coconut source)
        self.lin_μ = pt.nn.Linear(64, act_dim)  #108 (line num in coconut source)
        self.lin_σ = pt.nn.Linear(64, act_dim)  #109 (line num in coconut source)
        self.relu = pt.nn.functional.relu  #110 (line num in coconut source)
        self.min_σ = σ_min  #111 (line num in coconut source)
        self.max_σ = σ_max  #112 (line num in coconut source)
        self.lin_σ.weight.data.uniform_(-3e-3, 3e-3)  #113 (line num in coconut source)
        self.lin_σ.bias.data.uniform_(-3e-3, 3e-3)  #114 (line num in coconut source)

    def forward(self, state: pt.Tensor):  #115 (line num in coconut source)
        x = (self.lin_3)((self.relu)((self.lin_2)((self.relu)((self.lin_1)(state)))))  #116 (line num in coconut source)
        μ = (self.lin_μ)(x)  #117 (line num in coconut source)
        σ = (_coconut_partial(pt.clamp, {1: self.min_σ, 2: self.max_σ}, 3))((self.lin_σ)(x))  #118 (line num in coconut source)

        return (μ, σ)  #120 (line num in coconut source)

class Model(_coconut.typing.NamedTuple("Model", [("actor_online", pt.nn.Module), ("critic_online", pt.nn.Module), ("value_online", pt.nn.Module), ("value_target", pt.nn.Module), ("actor_optim", pt.optim.Optimizer), ("critic_optim", pt.optim.Optimizer), ("value_optim", pt.optim.Optimizer), ("critic_loss", _coconut.typing.Any), ("value_loss", _coconut.typing.Any)])):  #120 (line num in coconut source)
    _coconut_is_data = True  #120 (line num in coconut source)
    __slots__ = ()  #120 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #120 (line num in coconut source)
    def __eq__(self, other):  #120 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #120 (line num in coconut source)
    def __hash__(self):  #120 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #120 (line num in coconut source)
    __match_args__ = ('actor_online', 'critic_online', 'value_online', 'value_target', 'actor_optim', 'critic_optim', 'value_optim', 'critic_loss', 'value_loss')  #120 (line num in coconut source)
    def sync_target(self, other=None):  #120 (line num in coconut source)
        source = other if other else self.value_online  #125 (line num in coconut source)
        target = self.value_target  #126 (line num in coconut source)
        update = lambda t, o: (t.copy_)((t * (1.0 - τ_soft) + o * τ_soft))  #127 (line num in coconut source)
        _ = (list)((_coconut_partial(starmap, {0: update}, 2))((zip)(*(fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("parameters"), list, _coconut.functools.partial(fmap, _coconut.operator.attrgetter("data"))), [target, source]))))  #128 (line num in coconut source)
        return self.value_target  #129 (line num in coconut source)

    def act(self, state: pt.Tensor):  #129 (line num in coconut source)
        μ, σ_ = (self.actor_online)(state)  #131 (line num in coconut source)
        σ = (pt.exp)(σ_)  #132 (line num in coconut source)
        a = ((pt.tanh)(((pt.distributions.Normal)(*(μ, σ))).sample())).detach()  #133 (line num in coconut source)
        return a  #134 (line num in coconut source)

    def evaluate(self, state: pt.Tensor, ε: float=1e-6):  #134 (line num in coconut source)
        μ, σ_ = (self.actor_online)(state)  #136 (line num in coconut source)
        σ = (pt.exp)(σ_)  #137 (line num in coconut source)
        n = pt.distributions.Normal(μ, σ)  #138 (line num in coconut source)
        z = n.sample()  #139 (line num in coconut source)
        a = (pt.tanh)(z)  #140 (line num in coconut source)
        ε_ = pt.log(1 - a.pow(2) + ε)  #141 (line num in coconut source)
        p = ((_coconut_partial((_coconut_minus), {1: ε_}, 2))((n).log_prob(z))).sum(-1, keepdim=True)  #142 (line num in coconut source)

        return (a, p, z, μ, σ)  #144 (line num in coconut source)

def make_model(act_dim: int, obs_dim: int):  #144 (line num in coconut source)
    def init_weights(m: pt.nn.Module, init: float):  #145 (line num in coconut source)
        if isinstance(m, pt.nn.Linear) and (m.out_features == 1 or m.out_features == act_dim):  #146 (line num in coconut source)
            m.weight.data.uniform_(-init, init)  #147 (line num in coconut source)
            m.bias.data.uniform_(-init, init)  #148 (line num in coconut source)

    critic_loss = pt.nn.MSELoss()  #149 (line num in coconut source)
    value_loss = pt.nn.MSELoss()  #150 (line num in coconut source)
    actor_online = (ActorNet(obs_dim, act_dim)).to(device)  #151 (line num in coconut source)
    critic_online = (CriticNet(obs_dim, act_dim)).to(device)  #152 (line num in coconut source)
    value_online = (ValueNet(obs_dim)).to(device)  #153 (line num in coconut source)
    _ = [(n.apply)(_coconut_partial(init_weights, {1: 3e-3}, 2)) for n in [value_online, critic_online, actor_online]]  #154 (line num in coconut source)
    value_target = (ValueNet(obs_dim)).to(device)  #156 (line num in coconut source)
    params = (zip)(*(fmap)(_coconut.operator.methodcaller("parameters"), [value_target, value_online]))  #157 (line num in coconut source)
    _ = [(tp.data.copy_)((op).data) for tp, op in params]  #158 (line num in coconut source)
    actor_optim = pt.optim.Adam(actor_online.parameters(), lr=α, betas=βs)  #159 (line num in coconut source)
    critic_optim = pt.optim.Adam(critic_online.parameters(), lr=α, betas=βs)  #160 (line num in coconut source)
    value_optim = pt.optim.Adam(value_online.parameters(), lr=α, betas=βs)  #161 (line num in coconut source)
    model = Model(actor_online, critic_online, value_online, value_target, actor_optim, critic_optim, value_optim, critic_loss, value_loss)  #162 (line num in coconut source)

    return model  #166 (line num in coconut source)

def scale_rewards(reward: pt.Tensor, ρ: float=1e-3):  #166 (line num in coconut source)
    scaled_reward = (reward - pt.mean(reward)) / (pt.std(reward) + ρ)  #167 (line num in coconut source)

## Update
    return scaled_reward  #170 (line num in coconut source)

def soft_q_update(epoch: int, model: Model, buffer: ReplayBuffer, batch_size: int, μλ: float=1e-3, σλ: float=1e-3, zλ: float=0.0, γ: float=0.99):  #170 (line num in coconut source)
    states, actions, rewards_, next_states, dones = (map)(_coconut.operator.methodcaller("to", device), (_coconut_partial(sample_buffer, {1: batch_size}, 2))(buffer))  #173 (line num in coconut source)
    rewards = (scale_rewards)(rewards_)  #175 (line num in coconut source)
    v_expected = model.value_online(states)  #176 (line num in coconut source)
    q_expected = model.critic_online(states, actions)  #177 (line num in coconut source)
    new_actions, log_prob, z, μ, σ = model.evaluate(states)  #178 (line num in coconut source)
    v_target = model.value_target(next_states)  #180 (line num in coconut source)
    q_next = ((rewards + (1 - dones) * γ * v_target)).detach()  #181 (line num in coconut source)
    c_loss = model.critic_loss(q_expected, q_next)  #182 (line num in coconut source)
    q_exp_nxt = model.critic_online(states, new_actions)  #183 (line num in coconut source)
    v_next = ((q_exp_nxt - log_prob)).detach()  #184 (line num in coconut source)
    v_loss = model.value_loss(v_expected, v_next)  #185 (line num in coconut source)
    l_target = q_exp_nxt - v_expected  #186 (line num in coconut source)
    l_loss = (((log_prob * (log_prob - l_target))).detach()).mean()  #187 (line num in coconut source)
    μ_loss = (_coconut_partial((_coconut.operator.mul), {0: μλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(μ)))  #188 (line num in coconut source)
    σ_loss = (_coconut_partial((_coconut.operator.mul), {0: σλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(σ)))  #189 (line num in coconut source)
    z_loss = (_coconut_partial((_coconut.operator.mul), {0: zλ}, 2))((pt.mean)(((_coconut_partial(pt.pow, {1: 2}, 2))(z)).sum(1)))  #190 (line num in coconut source)
    a_loss = l_loss + μ_loss + σ_loss + z_loss  #191 (line num in coconut source)
    _ = model.critic_optim.zero_grad()  #192 (line num in coconut source)
    _ = c_loss.backward()  #193 (line num in coconut source)
    _ = model.critic_optim.step()  #194 (line num in coconut source)
    _ = model.value_optim.zero_grad()  #195 (line num in coconut source)
    _ = v_loss.backward()  #196 (line num in coconut source)
    _ = model.value_optim.step()  #197 (line num in coconut source)
    _ = model.actor_optim.zero_grad()  #198 (line num in coconut source)
    _ = a_loss.backward()  #199 (line num in coconut source)
    _ = model.actor_optim.step()  #200 (line num in coconut source)
    _ = model.sync_target()  #201 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Actor", a_loss, epoch)  #202 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic", c_loss, epoch)  #203 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Value", v_loss, epoch)  #204 (line num in coconut source)

## Exploration
    return (v_loss, c_loss, a_loss)  #207 (line num in coconut source)

def postprocess(observations, keys: dict[str, list[str]]):  #207 (line num in coconut source)
    pf = lambda k: ":" not in k and "/" not in k and (k[0]).islower()  #208 (line num in coconut source)
    lf = lambda k: k in ["ugbw", "cof", "sr_f", "sr_r"] or k.endswith("fug")  #209 (line num in coconut source)
    p_idx = (list)((fmap)(keys["observations"].index, (filter)(pf, keys["observations"])))  #210 (line num in coconut source)
    i_idx = [(keys["observations"].index)(k) for k in keys["actions"]]  #213 (line num in coconut source)
    idx = (sorted)((p_idx + i_idx))  #214 (line num in coconut source)
    l_msk = ((pt.Tensor)((list)((_coconut_partial(map, {0: lf}, 2))(keys["observations"])))).to(device)  #215 (line num in coconut source)
    of = lambda o: (pt.where(o > 0, pt.log10(o), o) * l_msk) + (o * (1 - l_msk))  #216 (line num in coconut source)
    states = (pt.vstack)([((of)(((pt.from_numpy)(obs)).to(device)))[idx] for obs in observations])  #217 (line num in coconut source)
#states  = (pt.log10(states_) *  l_msk) + (states_ * (~l_msk))
#states = [ obs |> pt.from_numpy for obs in observations
#         ] |> pt.vstack |> .to(device)

## Checkpoint Saving
    return states  #224 (line num in coconut source)

def save_checkpoint(model: Model, checkpoint_file: str):  #224 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [model.actor_online, model.critic_online, model.value_online, model.value_target, model.actor_optim, model.critic_optim, model.value_optim])  #225 (line num in coconut source)
    keys = ["actor_online", "critic_online", "value_online", "value_target", "actor_optim", "critic_optim", "value_optim"]  #228 (line num in coconut source)
    save_dict = (dict)((zip)(*(keys, state_dicts)))  #230 (line num in coconut source)
    res = pt.save(save_dict, checkpoint_file)  #231 (line num in coconut source)

## Training Loop
    return res  #234 (line num in coconut source)
...
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #234 (line num in coconut source)
    _coconut_match_check_0 = False  #234 (line num in coconut source)
    ...
    if (_coconut.len(_coconut_match_args) == 7) and ("model" not in _coconut_match_kwargs) and ("episode" not in _coconut_match_kwargs) and ("buffer" not in _coconut_match_kwargs) and (_coconut_match_args[4] is True):  #234 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #234 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #234 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #234 (line num in coconut source)
        if not _coconut_match_kwargs:  #234 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_0  #234 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_1  #234 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_2  #234 (line num in coconut source)
            _coconut_match_check_0 = True  #234 (line num in coconut source)
    if _coconut_match_check_0:  #234 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #234 (line num in coconut source)
            model = _coconut_match_temp_0  #234 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #234 (line num in coconut source)
            episode = _coconut_match_temp_1  #234 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #234 (line num in coconut source)
            buffer = _coconut_match_temp_2  #234 (line num in coconut source)
    if not _coconut_match_check_0:  #234 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, model, episode, _, True, buffer, _) = model where:', _coconut_match_args)  #234 (line num in coconut source)

    total = ((buffer.reward).sum()).item()  #235 (line num in coconut source)
    _ = writer.add_scalar(f"_Reward_Total", total, episode)  #236 (line num in coconut source)
    if verbose:  #237 (line num in coconut source)
        (print)(f"Episode {episode:03} Finished | Total Reward: {total}")  #238 (line num in coconut source)
    return model  #239 (line num in coconut source)

@_coconut_addpattern(run_episode)  #239 (line num in coconut source)
@_coconut_tco  #239 (line num in coconut source)
@_coconut_mark_as_match  #239 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #239 (line num in coconut source)
    _coconut_match_check_1 = False  #239 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #239 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #239 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #239 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #239 (line num in coconut source)
    _coconut_match_set_name_finish = _coconut_sentinel  #239 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #239 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #239 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #239 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) <= 7) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "envs" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "model" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "episode" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "finish" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 5, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "states" in _coconut_match_kwargs)) == 1):  #239 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("envs")  #239 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #239 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #239 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("iteration")  #239 (line num in coconut source)
        _coconut_match_temp_7 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("finish")  #239 (line num in coconut source)
        _coconut_match_temp_8 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #239 (line num in coconut source)
        _coconut_match_temp_9 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("states")  #239 (line num in coconut source)
        if not _coconut_match_kwargs:  #239 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_3  #239 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_4  #239 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_5  #239 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_6  #239 (line num in coconut source)
            _coconut_match_set_name_finish = _coconut_match_temp_7  #239 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_8  #239 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_9  #239 (line num in coconut source)
            _coconut_match_check_1 = True  #239 (line num in coconut source)
    if _coconut_match_check_1:  #239 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #239 (line num in coconut source)
            envs = _coconut_match_temp_3  #239 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #239 (line num in coconut source)
            model = _coconut_match_temp_4  #239 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #239 (line num in coconut source)
            episode = _coconut_match_temp_5  #239 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #239 (line num in coconut source)
            iteration = _coconut_match_temp_6  #239 (line num in coconut source)
        if _coconut_match_set_name_finish is not _coconut_sentinel:  #239 (line num in coconut source)
            finish = _coconut_match_temp_7  #239 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #239 (line num in coconut source)
            buffer = _coconut_match_temp_8  #239 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #239 (line num in coconut source)
            states = _coconut_match_temp_9  #239 (line num in coconut source)
    if not _coconut_match_check_1:  #239 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode( envs, model, episode, iteration, finish, buffer, states\n) = run_episode( envs, model, episode, iteration_\n, finish_, buffer_, states_ ) where:', _coconut_match_args)  #239 (line num in coconut source)

    t0 = time.time()  #242 (line num in coconut source)
    actions = (model.act)(states)  #243 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #244 (line num in coconut source)
    states_ = postprocess(observations, envs.info[0])  #248 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #249 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #250 (line num in coconut source)
    buffer_ = buffer + ReplayBuffer(states, actions, rewards, states_, dones)  #251 (line num in coconut source)
    t1 = time.time()  #252 (line num in coconut source)
    dt = t1 - t0  #253 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #254 (line num in coconut source)
    if len(buffer_) > batch_size:  #255 (line num in coconut source)
        soft_q_update(iteration, model, buffer_, batch_size)  #256 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(dones)  #257 (line num in coconut source)
    stop_ = (((pt.mean)(rewards)).item()) < early_stop  #258 (line num in coconut source)
    finish_ = done_ or stop_ or (iteration >= num_steps)  # or (dt > max_time)  #259 (line num in coconut source)
    iteration_ = iteration + 1  #260 (line num in coconut source)
    if verbose:  #261 (line num in coconut source)
        (print)(f"Iteration {iteration:03} took {dt:.3f}s | Average Reward: {rewards.mean():.3f}")  #262 (line num in coconut source)
    _ = writer.add_scalar("_Reward_Mean", rewards.mean(), iteration)  #263 (line num in coconut source)
    _ = save_checkpoint(model, model_path)  #264 (line num in coconut source)

## Episode Loop
    return _coconut_tail_call(run_episode, envs, model, episode, iteration_, finish_, buffer_, states_)  #267 (line num in coconut source)

def run_episodes(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #267 (line num in coconut source)
    obs = envs.reset()  #269 (line num in coconut source)
    states = postprocess(obs, envs.info[0])  #270 (line num in coconut source)
    buffer = (ReplayBuffer)(*(tuple)(_coconut_iter_getitem((repeat)((pt.empty(0)).to(device)), (_coconut.slice(None, 5)))))  #271 (line num in coconut source)
    model = run_episode(envs, model, episode, 0, False, buffer, states)  #272 (line num in coconut source)

    return model  #273 (line num in coconut source)
