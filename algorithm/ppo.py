...
num_envs: int = 50  #15 (line num in coconut source)
...
batch_size: int = 100  # size of the batches during epoch  #20 (line num in coconut source)
...
def act_net(obs_dim: int, act_dim: int):  #57 (line num in coconut source)
    net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 128), pt.nn.Tanh(), pt.nn.Linear(128, 256), pt.nn.Tanh(), pt.nn.Linear(256, 128), pt.nn.Tanh(), pt.nn.Linear(128, 64), pt.nn.Tanh(), pt.nn.Linear(64, act_dim), pt.nn.Tanh())  #58 (line num in coconut source)

    return net  #64 (line num in coconut source)

def crt_net(obs_dim: int):  #64 (line num in coconut source)
    net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 128), pt.nn.Tanh(), pt.nn.Linear(128, 256), pt.nn.Tanh(), pt.nn.Linear(256, 128), pt.nn.Tanh(), pt.nn.Linear(128, 64), pt.nn.Tanh(), pt.nn.Linear(64, 1))  #65 (line num in coconut source)

    return net  #71 (line num in coconut source)

class Memory(_coconut.typing.NamedTuple("Memory", [("states", pt.Tensor), ("actions", pt.Tensor), ("logprobs", pt.Tensor), ("rewards", pt.Tensor), ("values", pt.Tensor), ("masks", pt.Tensor)])):  #71 (line num in coconut source)
    ...
    __match_args__ = ('states', 'actions', 'logprobs', 'rewards', 'values', 'masks')  #71 (line num in coconut source)
    @_coconut_tco  #71 (line num in coconut source)
    def __add__(self, other):  #71 (line num in coconut source)
        return _coconut_tail_call((Memory), *(fmap)(pt.cat, (zip)(*(self, other))))  #73 (line num in coconut source)

    def data_loader():  #74 (line num in coconut source)
        states, actions, logprobs, rewards, values, masks = (Memory)(*(fmap)(_coconut.operator.itemgetter((_coconut.slice(None, -1))), self))  #75 (line num in coconut source)
        values_ = self.values[1:]  #77 (line num in coconut source)
        returns = gae(rewards, values, masks, values_)  #78 (line num in coconut source)
        advantages = returns - values  #79 (line num in coconut source)
        loader = (_coconut_partial(DataLoader, {}, 1, batch_size=batch_size, shuffle=True))((TensorDataset)(*(states, actions, logprobs, returns, advantages)))  #80 (line num in coconut source)


def std_deviation(act_dim: int, act_std: float=0.0):  #84 (line num in coconut source)
    std = (_coconut_partial((_coconut.operator.mul), {1: act_std}, 2))((_coconut_partial(pt.ones, {0: 1}, 2))(act_dim))  #85 (line num in coconut source)

    return std  #87 (line num in coconut source)

def continuous_actor_critic(actor: pt.nn.Module, critic: pt.nn.Module, std: float, states: pt.Tensor):  #87 (line num in coconut source)
    μ = actor(states)  #90 (line num in coconut source)
#σ = std |> .expand_as(μ) |> pt.diag_embed |> .to(device)
    σ = ((_coconut_partial((_coconut.operator.mul), {1: std}, 2))((pt.eye)(((μ).shape)[1]))).to(device)  #92 (line num in coconut source)
    π = pt.distributions.MultivariateNormal(μ, σ)  #93 (line num in coconut source)
    v = (critic(states)).squeeze()  #94 (line num in coconut source)

    return (π, v)  #96 (line num in coconut source)

def discrete_actor_critic(actor, critic, states):  #96 (line num in coconut source)
    μ = actor(states)  #97 (line num in coconut source)
    π = pt.distributions.Categorical(μ)  #98 (line num in coconut source)
    v = (critic(states)).squeeze()  #99 (line num in coconut source)

    return (π, v)  #101 (line num in coconut source)

@_coconut_tco  #101 (line num in coconut source)
def update_policy(actor_critic: function, optimizer: pt.optim, states: pt.Tensor, actions: pt.Tensor, logprobs: pt.Tensor, returns: pt.Tensor, advantages: pt.Tensor):  #101 (line num in coconut source)
    dist, values = actor_critic(states)  #105 (line num in coconut source)
    entropy = ((dist).entropy()).mean()  #106 (line num in coconut source)
    logprobs_ = dist.log_prob(actions)  #107 (line num in coconut source)
    ratios = (pt.exp)((logprobs_ - logprobs))  #108 (line num in coconut source)
    surr_1 = ratios * advantages  #109 (line num in coconut source)
    surr_2 = pt.clamp(ratios, 1.0 - ε, 1.0 + ε) * advantages  #110 (line num in coconut source)
    loss_act = ((_coconut_minus))((pt.min)(*(surr_1, surr_2)))  #111 (line num in coconut source)
    loss_crt = (pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))((returns - values)))  #112 (line num in coconut source)
    losses = 0.5 * loss_crt + loss_act - η * entropy  #113 (line num in coconut source)
    _ = (optimizer).zero_grad()  #114 (line num in coconut source)
    _ = ((pt.mean)(losses)).backward()  #115 (line num in coconut source)
    _ = (optimizer).step()  #116 (line num in coconut source)

    return _coconut_tail_call(losses.detach)  #118 (line num in coconut source)

def subset(obs, inf, pre):  #118 (line num in coconut source)
    sub = (((pt.vstack)((list)((starmap)(_coconut_forward_compose((lambda i, o: o[i]), pt.from_numpy), (_coconut_partial(zip, {1: obs}, 2))([[i["output-parameters"].index(p) for p in i["output-parameters"] if p.startswith(pre)] for i in inf]))))).to(device)).detach()  #119 (line num in coconut source)

    return sub  #125 (line num in coconut source)

def postprocess(observations, infos):  #125 (line num in coconut source)
    states = (((pt.vstack)((list)((starmap)(_coconut_forward_compose((lambda i, o: o[i]), pt.from_numpy), (_coconut_partial(zip, {1: observations}, 2))([[i["output-parameters"].index(p) for p in i["output-parameters"] if (any)((_coconut_partial(fmap, {0: p.startswith}, 2))(["performance", "target", "distance"]))] for i in infos]))))).to(device)).detach()  #130 (line num in coconut source)
#states = observations |> fmap$pt.from_numpy |> pt.vstack |> .to(device) |> .detach() 

    return states  #137 (line num in coconut source)

def trajectories(actor_critic: function, envs: gace.envs.vec.VecACE, states: pt.Tensor):  #137 (line num in coconut source)
    dist, values = actor_critic(states)  #139 (line num in coconut source)
    ...
    next_states = postprocess(observations, infos)  #146 (line num in coconut source)
    ...
    return (memories, next_states)  #151 (line num in coconut source)

def gae(r: pt.Tensor, v: pt.Tensor, m: pt.Tensor, v_: pt.Tensor, γ: float=0.99, τ: float=0.95):  #151 (line num in coconut source)
    δ = r + γ * v_ * m - v  #153 (line num in coconut source)
    l = (reversed)((range)(((δ).shape)[0]))  #154 (line num in coconut source)
    i = pt.Tensor([0,]).to(device)  #155 (line num in coconut source)
    g = (reduce(lambda g_, i_: (pt.hstack)((_coconut_partial((_coconut_comma_op), {1: g_}, 2))(δ[i_] + γ * τ * m[i_] * g_[0])), l, i))[_coconut.slice(None, -1)]  #156 (line num in coconut source)
    a = v + g  #158 (line num in coconut source)

    return a  #160 (line num in coconut source)

def save_checkpoint(actor: pt.nn.Module, critic: pt.nn.Module, optimizer: pt.optim, checkpoint_file: str):  #160 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [actor, critic, optimizer])  #163 (line num in coconut source)
    save_dict = (dict)((zip)(*(_coconut_partial((_coconut_comma_op), {0: ["actor", "critic", "optimizer"]}, 2))(state_dicts)))  #164 (line num in coconut source)
    res = pt.save(save_dict, checkpoint_file)  #166 (line num in coconut source)

    return res  #168 (line num in coconut source)

def write_performance(env: gym.Env, step: int, episode: int):  #168 (line num in coconut source)
    target = env.target  #169 (line num in coconut source)
    performance = (ac.current_performance)((env).ace)  #170 (line num in coconut source)
    for k in target.keys():  #171 (line num in coconut source)
        writer.add_scalars(k, {"Episode_{episode}": performance[k]}, step)  #172 (line num in coconut source)

    return performance  #175 (line num in coconut source)

@_coconut_mark_as_match  #175 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #175 (line num in coconut source)
    _coconut_match_check_0 = False  #175 (line num in coconut source)
    _coconut_match_set_name_loss = _coconut_sentinel  #175 (line num in coconut source)
    _coconut_match_set_name_reward = _coconut_sentinel  #175 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #175 (line num in coconut source)
    if (3 <= _coconut.len(_coconut_match_args) <= 5) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "loss" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "reward" in _coconut_match_kwargs)) == 1) and (_coconut_match_args[2] is True):  #175 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("loss")  #175 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("reward")  #175 (line num in coconut source)
        if not _coconut_match_kwargs:  #175 (line num in coconut source)
            _coconut_match_set_name_loss = _coconut_match_temp_0  #175 (line num in coconut source)
            _coconut_match_set_name_reward = _coconut_match_temp_1  #175 (line num in coconut source)
            _coconut_match_check_0 = True  #175 (line num in coconut source)
    if _coconut_match_check_0:  #175 (line num in coconut source)
        if _coconut_match_set_name_loss is not _coconut_sentinel:  #175 (line num in coconut source)
            loss = _coconut_match_temp_0  #175 (line num in coconut source)
        if _coconut_match_set_name_reward is not _coconut_sentinel:  #175 (line num in coconut source)
            reward = _coconut_match_temp_1  #175 (line num in coconut source)
    if not _coconut_match_check_0:  #175 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, _, True, loss, reward) = (loss, reward)', _coconut_match_args)  #175 (line num in coconut source)

    return (loss, reward)  #175 (line num in coconut source)

@_coconut_addpattern(run_episode)  #176 (line num in coconut source)
@_coconut_tco  #176 (line num in coconut source)
@_coconut_mark_as_match  #176 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #176 (line num in coconut source)
    _coconut_match_check_2 = False  #176 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #176 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #176 (line num in coconut source)
    _coconut_match_set_name_loss = _coconut_sentinel  #176 (line num in coconut source)
    _coconut_match_set_name_reward = _coconut_sentinel  #176 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #176 (line num in coconut source)
    if (3 <= _coconut.len(_coconut_match_args) <= 5) and ("episode" not in _coconut_match_kwargs) and ("states" not in _coconut_match_kwargs) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "loss" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "reward" in _coconut_match_kwargs)) == 1):  #176 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("episode")  #176 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("states")  #176 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("loss")  #176 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("reward")  #176 (line num in coconut source)
        if not _coconut_match_kwargs:  #176 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_3  #176 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_4  #176 (line num in coconut source)
            _coconut_match_set_name_loss = _coconut_match_temp_5  #176 (line num in coconut source)
            _coconut_match_set_name_reward = _coconut_match_temp_6  #176 (line num in coconut source)
            _coconut_match_check_2 = True  #176 (line num in coconut source)
    if _coconut_match_check_2:  #176 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #176 (line num in coconut source)
            episode = _coconut_match_temp_3  #176 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #176 (line num in coconut source)
            states = _coconut_match_temp_4  #176 (line num in coconut source)
        if _coconut_match_set_name_loss is not _coconut_sentinel:  #176 (line num in coconut source)
            loss = _coconut_match_temp_5  #176 (line num in coconut source)
        if _coconut_match_set_name_reward is not _coconut_sentinel:  #176 (line num in coconut source)
            reward = _coconut_match_temp_6  #176 (line num in coconut source)
    if not _coconut_match_check_2:  #176 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode( episode, states, _, loss, reward\n    ) = run_episode( episode, next_states, done, loss_, reward_\n    )  where:', _coconut_match_args)  #176 (line num in coconut source)

    @_coconut_mark_as_match  #176 (line num in coconut source)
    def traject(*_coconut_match_args, **_coconut_match_kwargs):  #176 (line num in coconut source)
        _coconut_match_check_1 = False  #176 (line num in coconut source)
        _coconut_match_set_name_memories = _coconut_sentinel  #176 (line num in coconut source)
        _coconut_match_set_name_states = _coconut_sentinel  #176 (line num in coconut source)
        _coconut_match_set_name_step = _coconut_sentinel  #176 (line num in coconut source)
        _coconut_FunctionMatchError = _coconut_get_function_match_error()  #176 (line num in coconut source)
        if (1 <= _coconut.len(_coconut_match_args) <= 2) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "step" in _coconut_match_kwargs)) == 1) and (_coconut.isinstance(_coconut_match_args[0], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_args[0]) == 2):  #176 (line num in coconut source)
            _coconut_match_temp_2 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("step")  #176 (line num in coconut source)
            _coconut_match_set_name_memories = _coconut_match_args[0][0]  #176 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_args[0][1]  #176 (line num in coconut source)
            if not _coconut_match_kwargs:  #176 (line num in coconut source)
                _coconut_match_set_name_step = _coconut_match_temp_2  #176 (line num in coconut source)
                _coconut_match_check_1 = True  #176 (line num in coconut source)
        if _coconut_match_check_1:  #176 (line num in coconut source)
            if _coconut_match_set_name_memories is not _coconut_sentinel:  #176 (line num in coconut source)
                memories = _coconut_match_args[0][0]  #176 (line num in coconut source)
            if _coconut_match_set_name_states is not _coconut_sentinel:  #176 (line num in coconut source)
                states = _coconut_match_args[0][1]  #176 (line num in coconut source)
            if _coconut_match_set_name_step is not _coconut_sentinel:  #176 (line num in coconut source)
                step = _coconut_match_temp_2  #176 (line num in coconut source)
        if not _coconut_match_check_1:  #176 (line num in coconut source)
            raise _coconut_FunctionMatchError('def traject((memories,states), step) = (memories_, states_) where:', _coconut_match_args)  #176 (line num in coconut source)

        t0 = time.time()  #180 (line num in coconut source)
        memory, states_ = trajectories(actor_critic, envs, states)  #181 (line num in coconut source)
        memories_ = Memory + memories  #182 (line num in coconut source)
        t1 = time.time()  #183 (line num in coconut source)
        if verbose:  #184 (line num in coconut source)
            print(f"Step {step:03}/{num_steps} took {(t1 - t0):.3f}s | Average Reward: {memory.reward.mean():.3f}")  #185 (line num in coconut source)
        writer.add_scalars("_Mean_Step_Reward", {f"Episode_{episode}": memory.reward.mean()}, step)  #186 (line num in coconut source)
        _ = (_coconut_partial(write_performance, {1: step, 2: episode}, 3))(((envs).gace_envs)[0])  #187 (line num in coconut source)
        return (memories_, states_)  #188 (line num in coconut source)

    @_coconut_tco  #188 (line num in coconut source)
    def train(epoch, loader):  #188 (line num in coconut source)
        t0 = time.time()  #189 (line num in coconut source)
        loss = (pt.stack)((list)(starmap(update, loader)))  #190 (line num in coconut source)
        t1 = time.time()  #191 (line num in coconut source)
        if verbose:  #192 (line num in coconut source)
            print(f"Epoch {epoch:03}/{num_epochs} took {(t1 - t0):.3f}s | Average Loss: {loss.mean():.3f}")  #193 (line num in coconut source)
        writer.add_scalars("_Mean_Epoch_Loss", {f"Episode_{episode}": loss.mean().item()}, epoch)  #194 (line num in coconut source)
        writer.add_scalars("_Sum_Epoch_Loss", {f"Episode_{episode}": loss.sum().item()}, epoch)  #195 (line num in coconut source)
        return _coconut_tail_call((pt.mean), loss)  #196 (line num in coconut source)

    _ = policy.eval()  #196 (line num in coconut source)
    with pt.no_grad():  #197 (line num in coconut source)
        memories_, next_states = reduce(traject, range(num_steps), ([], states))  #198 (line num in coconut source)
    _ = policy.train()  #200 (line num in coconut source)
    loader = data_loader(memories_)  #201 (line num in coconut source)
    losses = ((pt.stack)([train(epoch, loader) for epoch in (range)(num_epochs)])).detach()  #202 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))((1 - masks))  #204 (line num in coconut source)
    stop_ = rewards.mean().item() < early_stop  #205 (line num in coconut source)
    done = done_ or stop_  #206 (line num in coconut source)
    loss_ = pt.cat((loss, losses))  #207 (line num in coconut source)
    reward_ = pt.cat((reward, rewards))  #208 (line num in coconut source)
    if verbose:  #209 (line num in coconut source)
        if done:  #210 (line num in coconut source)
            print(f"All Environments Done")  #211 (line num in coconut source)
        else:  #212 (line num in coconut source)
            print(f"Episode {episode:03}/{num_episodes} | Average Reward {rewards.mean().item():.3f}")  #213 (line num in coconut source)
    _ = save_checkpoint(actor, critic, optimizer)  #214 (line num in coconut source)

    return _coconut_tail_call(run_episode, episode, next_states, done, loss_, reward_)  #216 (line num in coconut source)

def run_episodes(episode: int):  #216 (line num in coconut source)
    if verbose:  #217 (line num in coconut source)
        print(f"Starting Episode {episode:03}")  #218 (line num in coconut source)
    states = ((pt.vstack)((fmap)(_coconut_forward_compose(pt.from_numpy, _coconut.operator.itemgetter((_coconut.slice(None, obs_dim)))), envs.reset()))).to(device)  #219 (line num in coconut source)
    l = r = (pt.empty(0)).to(device=device)  #220 (line num in coconut source)
    losses, rewards = run_episode(episode, states, False, l, r)  #221 (line num in coconut source)
    writer.add_scalar(f"_Mean_Epsiode_Loss", losses.mean().item(), episode)  #222 (line num in coconut source)
    writer.add_scalar(f"_Total_Episode_Reward", rewards.sum().item(), episode)  #223 (line num in coconut source)
    for i in (range)((len)(losses)):  #224 (line num in coconut source)
        writer.add_scalars(f"Epsiode_Loss", {f"Episode_{episode}": losses[i]}, i)  #225 (line num in coconut source)
        writer.add_scalars(f"Epsiode_Reward", {f"Episode_{episode}": rewards[i]}, i)  #226 (line num in coconut source)
    if verbose:  #227 (line num in coconut source)
        print(f"Episode {episode:03} | Total Reward: {rewards.sum():.3f}")  #228 (line num in coconut source)
        print(f"Episode {episode:03} | Average Loss: {losses.mean():.3f}")  #229 (line num in coconut source)
        print(f"Finished Episode {episode:03}")  #230 (line num in coconut source)
    act = (_coconut_forward_compose(_coconut.operator.methodcaller("eval"), _coconut.operator.methodcaller("cpu")))(actor)  #231 (line num in coconut source)
    crt = (_coconut_forward_compose(_coconut.operator.methodcaller("eval"), _coconut.operator.methodcaller("cpu")))(critic)  #232 (line num in coconut source)

## Actor Critic Model
    return (act, crt)  #235 (line num in coconut source)

actor: pt.nn.Module = (act_net(obs_dim, act_dim)).to(device)  #235 (line num in coconut source)
critic: pt.nn.Module = (crt_net(obs_dim)).to(device)  #236 (line num in coconut source)
#deviation: pt.nn.Parameter = std_deviation(act_dim, act_std) |> .to(device)
policy: pt.nn.ModuleList = pt.nn.ModuleList([actor, critic])  #238 (line num in coconut source)

actor_critic: function = _coconut.functools.partial(continuous_actor_critic, actor, critic, act_std)  #240 (line num in coconut source)
...
optimizer: pt.optim = pt.optim.Adam(policy.parameters(), lr=α, betas=βs)  #244 (line num in coconut source)
...
update: function = _coconut.functools.partial(update_policy, actor_critic, optimizer)  #249 (line num in coconut source)
