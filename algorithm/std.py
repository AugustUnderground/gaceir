...
num_steps: int = 250  # How many steps to take  #17 (line num in coconut source)
...
σ_min: float = -2  # Lower Clipping  #33 (line num in coconut source)
σ_max: float = 20  # Upper Clipping  #34 (line num in coconut source)
buffer_size: int = (int)(1e7)  # Maximum size of replay buffer  #35 (line num in coconut source)
max_time: float = 20.0  # Maximum time to cut off  #36 (line num in coconut source)

## Setup
env_id: str = f"gace:{ace_id}-{ace_backend}-v{ace_variant}"  #39 (line num in coconut source)
time_stamp: str = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")  #40 (line num in coconut source)
model_dir: str = f"./models/{time_stamp}-{env_id}-{algorithm}"  #41 (line num in coconut source)
model_path: str = f"{model_dir}/checkpoint.pt"  #42 (line num in coconut source)
log_dir: str = f"./runs/{time_stamp}-{env_id}-{algorithm}/"  #43 (line num in coconut source)
os.makedirs(model_dir, exist_ok=True)  #44 (line num in coconut source)

## Setup Globals
device = pt.device("cuda:1") if pt.cuda.is_available() else pt.device("cpu")  #47 (line num in coconut source)
writer = SummaryWriter(log_dir=log_dir, flush_secs=30)  #48 (line num in coconut source)
_ = (pt.manual_seed)(rng_seed)  #49 (line num in coconut source)

## Utility
def write_performance(env: gym.Env, step: int):  #52 (line num in coconut source)
    target = env.target  #53 (line num in coconut source)
    performance = (ac.current_performance)((env).ace)  #54 (line num in coconut source)
    for k in target.keys():  #55 (line num in coconut source)
        writer.add_scalars(k, {"performance": performance[k], "target": target[k]}, step)  #56 (line num in coconut source)

## Replay Buffer
    return performance  #61 (line num in coconut source)

class ReplayBuffer(_coconut.typing.NamedTuple("ReplayBuffer", [("state", pt.Tensor), ("action", pt.Tensor), ("reward", pt.Tensor), ("next_state", pt.Tensor), ("done", pt.Tensor)])):  #61 (line num in coconut source)
    _coconut_is_data = True  #61 (line num in coconut source)
    __slots__ = ()  #61 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #61 (line num in coconut source)
    def __eq__(self, other):  #61 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #61 (line num in coconut source)
    def __hash__(self):  #61 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #61 (line num in coconut source)
    __match_args__ = ('state', 'action', 'reward', 'next_state', 'done')  #61 (line num in coconut source)
    @_coconut_tco  #61 (line num in coconut source)
    def __len__(self):  #61 (line num in coconut source)
        return _coconut_tail_call((int), ((self.state).shape)[0])  #63 (line num in coconut source)

    def __add__(self, other: ReplayBuffer):  #64 (line num in coconut source)
        buf = (ReplayBuffer)(*(map)(_coconut_forward_compose(pt.cat, _coconut.operator.itemgetter((_coconut.slice(-buffer_size, None)))), (_coconut_partial(zip, {0: self}, 2))((map)(_coconut.operator.methodcaller("to", device), other))))  #65 (line num in coconut source)

        return buf  #68 (line num in coconut source)

def sample_buffer(buffer: ReplayBuffer, batch_size: int):  #68 (line num in coconut source)
    idx = (((pt.randperm)((len)(buffer)))[_coconut.slice(None, batch_size)]).tolist()  #69 (line num in coconut source)
    smpl = (ReplayBuffer)(*(map)(_coconut.operator.itemgetter((idx, _coconut.slice(None, None))), buffer))  #70 (line num in coconut source)


## Neural Networks
    return smpl  #74 (line num in coconut source)

class HStack(_coconut.collections.namedtuple("HStack", ()), pt.nn.Module):  #74 (line num in coconut source)
    ...
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #74 (line num in coconut source)
        return _coconut_tail_call((pt.hstack), X)  #75 (line num in coconut source)


class VStack(_coconut.collections.namedtuple("VStack", ()), pt.nn.Module):  #77 (line num in coconut source)
    ...
    __match_args__ = ()  #77 (line num in coconut source)
    @_coconut_tco  #77 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #77 (line num in coconut source)
        return _coconut_tail_call((pt.vstack), X)  #78 (line num in coconut source)

## Value Network

class ValueNet(_coconut.typing.NamedTuple("ValueNet", [("obs_dim", int)]), pt.nn.Module):  #81 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim',)  #81 (line num in coconut source)
    def __init__(self, obs_dim: int):  #82 (line num in coconut source)
        super(ValueNet, self).__init__()  #83 (line num in coconut source)
        self.v_net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #84 (line num in coconut source)

    @_coconut_tco  #85 (line num in coconut source)
    def forward(self, state: pt.Tensor):  #85 (line num in coconut source)
        return _coconut_tail_call((self.v_net), state)  #88 (line num in coconut source)

## Critic Network

class CriticNet(_coconut.typing.NamedTuple("CriticNet", [("obs_dim", int), ("act_dim", int)]), pt.nn.Module):  #91 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim', 'act_dim')  #91 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #92 (line num in coconut source)
        super(CriticNet, self).__init__()  #93 (line num in coconut source)
        dim = obs_dim + act_dim  #94 (line num in coconut source)
        self.c_net = pt.nn.Sequential(HStack(), pt.nn.Linear(dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #95 (line num in coconut source)

    @_coconut_tco  #96 (line num in coconut source)
    def forward(self, state: pt.Tensor, action: pt.Tensor):  #96 (line num in coconut source)
        return _coconut_tail_call((self.c_net), (state, action))  #100 (line num in coconut source)

## Actor Network

class ActorNet(_coconut.typing.NamedTuple("ActorNet", [("obs_dim", int), ("act_dim", int), ("σ_min", float), ("σ_max", float)]), pt.nn.Module):  #103 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim', 'act_dim', 'σ_min', 'σ_max')  #103 (line num in coconut source)
    def __new__(_coconut_cls, obs_dim, act_dim, σ_min=-2.0, σ_max=20.0):  #103 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (obs_dim, act_dim, σ_min, σ_max))  #103 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #105 (line num in coconut source)
        super(ActorNet, self).__init__()  #106 (line num in coconut source)
        self.lin_1 = pt.nn.Linear(obs_dim, 256)  #107 (line num in coconut source)
        self.lin_2 = pt.nn.Linear(256, 128)  #108 (line num in coconut source)
        self.lin_3 = pt.nn.Linear(128, 64)  #109 (line num in coconut source)
        self.lin_μ = pt.nn.Linear(64, act_dim)  #110 (line num in coconut source)
        self.lin_σ = pt.nn.Linear(64, act_dim)  #111 (line num in coconut source)
        self.relu = pt.nn.functional.relu  #112 (line num in coconut source)
        self.min_σ = σ_min  #113 (line num in coconut source)
        self.max_σ = σ_max  #114 (line num in coconut source)
        self.lin_σ.weight.data.uniform_(-3e-3, 3e-3)  #115 (line num in coconut source)
        self.lin_σ.bias.data.uniform_(-3e-3, 3e-3)  #116 (line num in coconut source)

    def forward(self, state: pt.Tensor):  #117 (line num in coconut source)
        x = (self.lin_3)((self.relu)((self.lin_2)((self.relu)((self.lin_1)(state)))))  #118 (line num in coconut source)
        μ = (self.lin_μ)(x)  #119 (line num in coconut source)
        σ = (_coconut_partial(pt.clamp, {1: self.min_σ, 2: self.max_σ}, 3))((self.lin_σ)(x))  #120 (line num in coconut source)

        return (μ, σ)  #122 (line num in coconut source)

class Model(_coconut.typing.NamedTuple("Model", [("actor", pt.nn.Module), ("critic_1", pt.nn.Module), ("critic_2", pt.nn.Module), ("value_online", pt.nn.Module), ("value_target", pt.nn.Module), ("actor_opt", pt.optim.Optimizer), ("critic_1_opt", pt.optim.Optimizer), ("critic_2_opt", pt.optim.Optimizer), ("value_opt", pt.optim.Optimizer), ("critic_loss", _coconut.typing.Any), ("value_loss", _coconut.typing.Any)])):  #122 (line num in coconut source)
    ...
    __match_args__ = ('actor', 'critic_1', 'critic_2', 'value_online', 'value_target', 'actor_opt', 'critic_1_opt', 'critic_2_opt', 'value_opt', 'critic_loss', 'value_loss')  #122 (line num in coconut source)
    def soft_sync(self, other=None):  #122 (line num in coconut source)
        source = other if other else self.value_online  #128 (line num in coconut source)
        target = self.value_target  #129 (line num in coconut source)
        update = lambda t, o: (t.copy_)((t * (1.0 - τ_soft) + o * τ_soft))  #130 (line num in coconut source)
        _ = (list)((_coconut_partial(starmap, {0: update}, 2))((zip)(*(fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("parameters"), list, _coconut.functools.partial(fmap, _coconut.operator.attrgetter("data"))), [target, source]))))  #131 (line num in coconut source)
        return self.value_target  #132 (line num in coconut source)

    def act(self, state: pt.Tensor):  #132 (line num in coconut source)
        μ, σ_ = (self.actor)(state)  #134 (line num in coconut source)
        σ = (pt.exp)(σ_)  #135 (line num in coconut source)
        a = ((pt.tanh)(((pt.distributions.Normal)(*(μ, σ))).sample())).detach()  #136 (line num in coconut source)
        return a  #137 (line num in coconut source)

    def evaluate(self, state: pt.Tensor, ε: float=1e-6):  #137 (line num in coconut source)
        μ, σ_ = (self.actor)(state)  #139 (line num in coconut source)
        σ = (pt.exp)(σ_)  #140 (line num in coconut source)
        n = pt.distributions.Normal(μ, σ)  #141 (line num in coconut source)
        z = n.sample()  #142 (line num in coconut source)
        a = (pt.tanh)(z)  #143 (line num in coconut source)
        ε_ = pt.log(1 - a.pow(2) + ε)  #144 (line num in coconut source)
        p = ((_coconut_partial((_coconut_minus), {1: ε_}, 2))((n).log_prob(z))).sum(-1, keepdim=True)  #145 (line num in coconut source)

        return (a, p, z, μ, σ)  #147 (line num in coconut source)

def make_model(act_dim: int, obs_dim: int):  #147 (line num in coconut source)
    def init_weights(m: pt.nn.Module, init: float):  #148 (line num in coconut source)
        if isinstance(m, pt.nn.Linear) and (m.out_features == 1 or m.out_features == act_dim):  #149 (line num in coconut source)
            m.weight.data.uniform_(-init, init)  #150 (line num in coconut source)
            m.bias.data.uniform_(-init, init)  #151 (line num in coconut source)

    critic_loss = pt.nn.MSELoss()  #152 (line num in coconut source)
    value_loss = pt.nn.MSELoss()  #153 (line num in coconut source)
    actor = (ActorNet(obs_dim, act_dim)).to(device)  #154 (line num in coconut source)
    critic_1 = (CriticNet(obs_dim, act_dim)).to(device)  #155 (line num in coconut source)
    critic_2 = (CriticNet(obs_dim, act_dim)).to(device)  #156 (line num in coconut source)
    value_online = (ValueNet(obs_dim)).to(device)  #157 (line num in coconut source)
    _ = [(n.apply)(_coconut_partial(init_weights, {1: 3e-3}, 2)) for n in [actor, critic_1, critic_2, value_online]]  #158 (line num in coconut source)
    value_target = (ValueNet(obs_dim)).to(device)  #160 (line num in coconut source)
    params = (zip)(*(fmap)(_coconut.operator.methodcaller("parameters"), [value_target, value_online]))  #161 (line num in coconut source)
    _ = [(tp.data.copy_)((op).data) for tp, op in params]  #162 (line num in coconut source)
    actor_opt = pt.optim.Adam(actor.parameters(), lr=α_actor, betas=βs)  #163 (line num in coconut source)
    critic_1_opt = pt.optim.Adam(critic_1.parameters(), lr=α_critic, betas=βs)  #165 (line num in coconut source)
    critic_2_opt = pt.optim.Adam(critic_2.parameters(), lr=α_critic, betas=βs)  #167 (line num in coconut source)
    value_opt = pt.optim.Adam(value_online.parameters(), lr=α_value, betas=βs)  #169 (line num in coconut source)
    model = Model(actor, critic_1, critic_2, value_online, value_target, actor_opt, critic_1_opt, critic_2_opt, value_opt, critic_loss, value_loss)  #171 (line num in coconut source)

    return model  #175 (line num in coconut source)

def scale_rewards(reward: pt.Tensor, ρ: float=1e-3):  #175 (line num in coconut source)
    scaled_reward = (reward - pt.mean(reward)) / (pt.std(reward) + ρ)  #176 (line num in coconut source)

## Update
    return scaled_reward  #179 (line num in coconut source)

def soft_q_update(epoch: int, model: Model, buffer: ReplayBuffer, batch_size: int, μλ: float=1e-3, σλ: float=1e-3, zλ: float=0.0, γ: float=0.99):  #179 (line num in coconut source)
    states, actions, rewards_, next_states, dones = (map)(_coconut.operator.methodcaller("to", device), (_coconut_partial(sample_buffer, {1: batch_size}, 2))(buffer))  #182 (line num in coconut source)
    rewards = (scale_rewards)(rewards_)  #184 (line num in coconut source)
#rewards     = rewards_
    v_expected = (model.value_online)(states)  #186 (line num in coconut source)
    q_exp_1 = (model.critic_1)(*(states, actions))  #187 (line num in coconut source)
    q_exp_2 = (model.critic_2)(*(states, actions))  #188 (line num in coconut source)
    q_expected = (pt.min)(*(q_exp_1, q_exp_2))  #189 (line num in coconut source)
    new_actions, log_prob, z, μ, σ = (model.evaluate)(states)  #190 (line num in coconut source)
    v_target = (model.value_target)(next_states)  #192 (line num in coconut source)
    q_next = ((rewards + (1 - dones) * γ * v_target)).detach()  #193 (line num in coconut source)
    c1_loss = (model.critic_loss)(*(q_exp_1, q_next))  #194 (line num in coconut source)
    c2_loss = (model.critic_loss)(*(q_exp_2, q_next))  #195 (line num in coconut source)
    q_exp_1_nxt = (model.critic_1)(*(states, new_actions))  #196 (line num in coconut source)
    q_exp_2_nxt = (model.critic_2)(*(states, new_actions))  #197 (line num in coconut source)
    q_exp_nxt = (pt.min)(*(q_exp_1_nxt, q_exp_2_nxt))  #198 (line num in coconut source)
    v_next = ((q_exp_nxt - log_prob)).detach()  #199 (line num in coconut source)
    v_loss = (model.value_loss)(*(v_expected, v_next))  #200 (line num in coconut source)
    l_target = q_exp_nxt - v_expected  #201 (line num in coconut source)
    l_loss = (((log_prob * (log_prob - l_target))).detach()).mean()  #202 (line num in coconut source)
    μ_loss = (_coconut_partial((_coconut.operator.mul), {0: μλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(μ)))  #203 (line num in coconut source)
    σ_loss = (_coconut_partial((_coconut.operator.mul), {0: σλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(σ)))  #204 (line num in coconut source)
    z_loss = (_coconut_partial((_coconut.operator.mul), {0: zλ}, 2))((pt.mean)(((_coconut_partial(pt.pow, {1: 2}, 2))(z)).sum(1)))  #205 (line num in coconut source)
    a_loss = l_loss + μ_loss + σ_loss + z_loss  #206 (line num in coconut source)
    _ = model.critic_1_opt.zero_grad()  #207 (line num in coconut source)
    _ = c1_loss.backward()  #208 (line num in coconut source)
    _ = model.critic_1_opt.step()  #209 (line num in coconut source)
    _ = model.critic_2_opt.zero_grad()  #210 (line num in coconut source)
    _ = c2_loss.backward()  #211 (line num in coconut source)
    _ = model.critic_2_opt.step()  #212 (line num in coconut source)
    _ = model.value_opt.zero_grad()  #213 (line num in coconut source)
    _ = v_loss.backward()  #214 (line num in coconut source)
    _ = model.value_opt.step()  #215 (line num in coconut source)
    _ = model.actor_opt.zero_grad()  #216 (line num in coconut source)
    _ = a_loss.backward()  #217 (line num in coconut source)
    _ = model.actor_opt.step()  #218 (line num in coconut source)
    _ = model.soft_sync()  #219 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Actor", a_loss, epoch)  #220 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic_1", c1_loss, epoch)  #221 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic_2", c2_loss, epoch)  #222 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Value", v_loss, epoch)  #223 (line num in coconut source)
    if verbose:  #224 (line num in coconut source)
        print(f"\tL Loss: {l_loss}")  #225 (line num in coconut source)
        print(f"\tμ Loss: {μ_loss}")  #226 (line num in coconut source)
        print(f"\tσ Loss: {σ_loss}")  #227 (line num in coconut source)
        print(f"\tz Loss: {z_loss}")  #228 (line num in coconut source)
        print(f"\ta Loss: {a_loss}")  #229 (line num in coconut source)

## Exploration
    return (v_loss, c1_loss, c2_loss, a_loss)  #232 (line num in coconut source)

def postprocess(observations, keys: dict[str, list[str]]):  #232 (line num in coconut source)
    pf = lambda k: ":" not in k and "/" not in k and (k[0]).islower()  #233 (line num in coconut source)
    lf = lambda k: k in ["ugbw", "cof", "sr_f", "sr_r"] or k.endswith("fug")  #234 (line num in coconut source)
    p_idx = (list)((fmap)(keys["observations"].index, (filter)(pf, keys["observations"])))  #235 (line num in coconut source)
    i_idx = [(keys["observations"].index)(k) for k in keys["actions"]]  #238 (line num in coconut source)
    idx = (sorted)((p_idx + i_idx))  #239 (line num in coconut source)
    l_msk = ((pt.Tensor)((list)((_coconut_partial(map, {0: lf}, 2))(keys["observations"])))).to(device)  #240 (line num in coconut source)
    of = lambda o: (pt.where(o > 0, pt.log10(o), o) * l_msk) + (o * (1 - l_msk))  #241 (line num in coconut source)
    states = (pt.vstack)([((of)(((pt.from_numpy)(obs)).to(device)))[idx] for obs in observations])  #242 (line num in coconut source)
#states  = (pt.log10(states_) *  l_msk) + (states_ * (~l_msk))
#states = [ obs |> pt.from_numpy for obs in observations
#         ] |> pt.vstack |> .to(device)

## Checkpoint Saving
    return states  #249 (line num in coconut source)

def save_checkpoint(model: Model, checkpoint_file: str):  #249 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [model.actor, model.critic_1, model.critic_2, model.value_online, model.value_target, model.actor_opt, model.critic_1_opt, model.critic_2_opt, model.value_opt])  #250 (line num in coconut source)
    keys = ["actor", "critic_1", "critic_2", "value_online", "value_target", "actor_opt", "critic_1_opt", "critic_2_opt", "value_opt"]  #254 (line num in coconut source)
    save_dict = (dict)((zip)(*(keys, state_dicts)))  #257 (line num in coconut source)
    res = (pt.save)(*(save_dict, checkpoint_file))  #258 (line num in coconut source)

## Training Loop
    return res  #261 (line num in coconut source)

@_coconut_mark_as_match  #261 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #261 (line num in coconut source)
    _coconut_match_check_0 = False  #261 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #261 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #261 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #261 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #261 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) == 7) and ("model" not in _coconut_match_kwargs) and ("episode" not in _coconut_match_kwargs) and ("buffer" not in _coconut_match_kwargs) and (_coconut_match_args[4] is True):  #261 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #261 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #261 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #261 (line num in coconut source)
        if not _coconut_match_kwargs:  #261 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_0  #261 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_1  #261 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_2  #261 (line num in coconut source)
            _coconut_match_check_0 = True  #261 (line num in coconut source)
    if _coconut_match_check_0:  #261 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #261 (line num in coconut source)
            model = _coconut_match_temp_0  #261 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #261 (line num in coconut source)
            episode = _coconut_match_temp_1  #261 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #261 (line num in coconut source)
            buffer = _coconut_match_temp_2  #261 (line num in coconut source)
    if not _coconut_match_check_0:  #261 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, model, episode, _, True, buffer, _) = model where:', _coconut_match_args)  #261 (line num in coconut source)

    total = ((buffer.reward).sum()).item()  #262 (line num in coconut source)
    _ = writer.add_scalar(f"_Reward_Total", total, episode)  #263 (line num in coconut source)
    if verbose:  #264 (line num in coconut source)
        (print)(f"Episode {episode:03} Finished | Total Reward: {total}")  #265 (line num in coconut source)
    return model  #266 (line num in coconut source)

@_coconut_addpattern(run_episode)  #266 (line num in coconut source)
@_coconut_tco  #266 (line num in coconut source)
@_coconut_mark_as_match  #266 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #266 (line num in coconut source)
    _coconut_match_check_1 = False  #266 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #266 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #266 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #266 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #266 (line num in coconut source)
    _coconut_match_set_name_finish = _coconut_sentinel  #266 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #266 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #266 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #266 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) <= 7) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "envs" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "model" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "episode" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "finish" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 5, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "states" in _coconut_match_kwargs)) == 1):  #266 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("envs")  #266 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #266 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #266 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("iteration")  #266 (line num in coconut source)
        _coconut_match_temp_7 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("finish")  #266 (line num in coconut source)
        _coconut_match_temp_8 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #266 (line num in coconut source)
        _coconut_match_temp_9 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("states")  #266 (line num in coconut source)
        if not _coconut_match_kwargs:  #266 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_3  #266 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_4  #266 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_5  #266 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_6  #266 (line num in coconut source)
            _coconut_match_set_name_finish = _coconut_match_temp_7  #266 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_8  #266 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_9  #266 (line num in coconut source)
            _coconut_match_check_1 = True  #266 (line num in coconut source)
    if _coconut_match_check_1:  #266 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #266 (line num in coconut source)
            envs = _coconut_match_temp_3  #266 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #266 (line num in coconut source)
            model = _coconut_match_temp_4  #266 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #266 (line num in coconut source)
            episode = _coconut_match_temp_5  #266 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #266 (line num in coconut source)
            iteration = _coconut_match_temp_6  #266 (line num in coconut source)
        if _coconut_match_set_name_finish is not _coconut_sentinel:  #266 (line num in coconut source)
            finish = _coconut_match_temp_7  #266 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #266 (line num in coconut source)
            buffer = _coconut_match_temp_8  #266 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #266 (line num in coconut source)
            states = _coconut_match_temp_9  #266 (line num in coconut source)
    if not _coconut_match_check_1:  #266 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode( envs, model, episode, iteration, finish, buffer, states\n) = run_episode( envs, model, episode, iteration_\n, finish_, buffer_, states_ ) where:', _coconut_match_args)  #266 (line num in coconut source)

    t0 = time.time()  #269 (line num in coconut source)
    actions = (model.act)(states)  #270 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #271 (line num in coconut source)
    states_ = postprocess(observations, envs.info[0])  #275 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #276 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #277 (line num in coconut source)
    buffer_ = buffer + ReplayBuffer(states, actions, rewards, states_, dones)  #278 (line num in coconut source)
    t1 = time.time()  #279 (line num in coconut source)
    dt = t1 - t0  #280 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #281 (line num in coconut source)
    if len(buffer_) > batch_size:  #282 (line num in coconut source)
        soft_q_update(iteration, model, buffer_, batch_size)  #283 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(dones)  #284 (line num in coconut source)
    stop_ = (((pt.mean)(rewards)).item()) < early_stop  #285 (line num in coconut source)
    finish_ = done_ or stop_ or (iteration >= num_steps)  # or (dt > max_time)  #286 (line num in coconut source)
    iteration_ = iteration + 1  #287 (line num in coconut source)
    if verbose:  #288 (line num in coconut source)
        (print)(f"Iteration {iteration:03} took {dt:.3f}s | Average Reward: {rewards.mean():.3f}")  #289 (line num in coconut source)
    _ = writer.add_scalar("_Reward_Mean", rewards.mean(), iteration)  #290 (line num in coconut source)
    _ = save_checkpoint(model, model_path)  #291 (line num in coconut source)

## Episode Loop
    return _coconut_tail_call(run_episode, envs, model, episode, iteration_, finish_, buffer_, states_)  #294 (line num in coconut source)

def run_episodes(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #294 (line num in coconut source)
    obs = envs.reset()  #296 (line num in coconut source)
    states = postprocess(obs, envs.info[0])  #297 (line num in coconut source)
    buffer = (ReplayBuffer)(*(tuple)(_coconut_iter_getitem((repeat)((pt.empty(0)).to(device)), (_coconut.slice(None, 5)))))  #298 (line num in coconut source)
    model = run_episode(envs, model, episode, 0, False, buffer, states)  #299 (line num in coconut source)

    return model  #300 (line num in coconut source)
