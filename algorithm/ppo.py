...
num_envs: int = 50  #16 (line num in coconut source)
num_episodes: int = 42  # Number of episodes to play  #17 (line num in coconut source)
num_steps: int = 5  # How many steps to take -> num_steps × num_envs = n_points ∈  data_set  #18 (line num in coconut source)
num_epochs: int = 50  # How many time steps to update policy  #19 (line num in coconut source)
max_iter: int = 100  # Maximum number of iterations during episode  #20 (line num in coconut source)
early_stop: float = -50.0  # Early stop criterion  #21 (line num in coconut source)
batch_size: int = 10  # size of the batches during epoch  #22 (line num in coconut source)
act_std: float = 0.25  # standard deviation action distribution  #23 (line num in coconut source)
rng_seed: int = 666  # Random seed for reproducability  #24 (line num in coconut source)
algorithm: str = "ppo"  # Name of used algorithm ∈  ./algorithm  #25 (line num in coconut source)
ace_id: str = "op2"  # ACE Identifier of the Environment  #26 (line num in coconut source)
ace_backend: str = "xh035"  # PDK/Technology backend of the ACE Environment  #27 (line num in coconut source)
ace_variant: int = 0  # ACE Environment variant  #28 (line num in coconut source)
ε: float = 0.2  # Factor for clipping  #29 (line num in coconut source)
η: float = 0.001  # Factor in loss function  #30 (line num in coconut source)
γ: float = 0.99  # Discount Factor  #31 (line num in coconut source)
τ: float = 0.95  # Avantage Factor  #32 (line num in coconut source)
α: float = 1e-3  # Learning Rate  #33 (line num in coconut source)
β1: float = 0.9  #35 (line num in coconut source)
β2: float = 0.999  #36 (line num in coconut source)

## Setup
βs: tuple[float] = (β1, β2)  #39 (line num in coconut source)
...
## Environment setup
envs: gace.envs.vec.VecACE = gace.vector_make_same(env_id, num_envs)  #52 (line num in coconut source)
#obs_dim: int = envs.observation_space[0].shape[0]
obs_dim: int = ((_coconut.operator.mul))(3, (len)(_coconut_iter_getitem(envs, 0).target))  #54 (line num in coconut source)
#obs_dim: int = envs$[0].target |> len |> (*)$(2)
act_dim: int = envs.action_space[0].shape[0]  #56 (line num in coconut source)

## Replay Memory
class Memory(_coconut.typing.NamedTuple("Memory", [("states", pt.Tensor), ("actions", pt.Tensor), ("logprobs", pt.Tensor), ("rewards", pt.Tensor), ("values", pt.Tensor), ("masks", pt.Tensor)])):  #59 (line num in coconut source)
    ...
    __match_args__ = ('states', 'actions', 'logprobs', 'rewards', 'values', 'masks')  #59 (line num in coconut source)
    @_coconut_tco  #59 (line num in coconut source)
    def __add__(self, other):  #59 (line num in coconut source)
        return _coconut_tail_call((Memory), *(fmap)(pt.cat, (zip)(*(self, other))))  #61 (line num in coconut source)


def data_loader(memory: Memory, batch_size: int=batch_size):  #63 (line num in coconut source)
    states, actions, logprobs, rewards, values, masks = (Memory)(*(fmap)(_coconut.operator.itemgetter((_coconut.slice(None, -1))), memory))  #64 (line num in coconut source)
    values_ = memory.values[1:]  #66 (line num in coconut source)
    returns = gae(rewards, values, masks, values_)  #67 (line num in coconut source)
    advantages = returns - values  #68 (line num in coconut source)
    loader = (_coconut_partial(DataLoader, {}, 1, batch_size=batch_size, shuffle=True))((TensorDataset)(*(states, actions, logprobs, returns, advantages)))  #69 (line num in coconut source)

## Neural Networks for actor and critic
    return loader  #74 (line num in coconut source)

def act_net(obs_dim: int, act_dim: int):  #74 (line num in coconut source)
    net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 128), pt.nn.Tanh(), pt.nn.Linear(128, 256), pt.nn.Tanh(), pt.nn.Linear(256, 128), pt.nn.Tanh(), pt.nn.Linear(128, 64), pt.nn.Tanh(), pt.nn.Linear(64, act_dim), pt.nn.Tanh())  #75 (line num in coconut source)

    return net  #81 (line num in coconut source)

def crt_net(obs_dim: int):  #81 (line num in coconut source)
    net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 128), pt.nn.Tanh(), pt.nn.Linear(128, 256), pt.nn.Tanh(), pt.nn.Linear(256, 128), pt.nn.Tanh(), pt.nn.Linear(128, 64), pt.nn.Tanh(), pt.nn.Linear(64, 1))  #82 (line num in coconut source)

    return net  #88 (line num in coconut source)

def std_deviation(act_dim: int, act_std: float=0.0):  #88 (line num in coconut source)
    std = (_coconut_partial((_coconut.operator.mul), {1: act_std}, 2))((_coconut_partial(pt.ones, {0: 1}, 2))(act_dim))  #89 (line num in coconut source)

    return std  #91 (line num in coconut source)

def continuous_actor_critic(actor: pt.nn.Module, critic: pt.nn.Module, std: float, states: pt.Tensor):  #91 (line num in coconut source)
    μ = actor(states)  #94 (line num in coconut source)
#σ = std |> .expand_as(μ) |> pt.diag_embed |> .to(device)
    σ = ((_coconut_partial((_coconut.operator.mul), {1: std}, 2))((pt.eye)(((μ).shape)[1]))).to(device)  #96 (line num in coconut source)
    π = pt.distributions.MultivariateNormal(μ, σ)  #97 (line num in coconut source)
    v = (critic(states)).squeeze()  #98 (line num in coconut source)

    return (π, v)  #100 (line num in coconut source)

def discrete_actor_critic(actor, critic, states):  #100 (line num in coconut source)
    μ = actor(states)  #101 (line num in coconut source)
    π = pt.distributions.Categorical(μ)  #102 (line num in coconut source)
    v = (critic(states)).squeeze()  #103 (line num in coconut source)

    return (π, v)  #105 (line num in coconut source)

def update_policy(actor_critic: function, optimizer: pt.optim, states: pt.Tensor, actions: pt.Tensor, logprobs: pt.Tensor, returns: pt.Tensor, advantages: pt.Tensor):  #105 (line num in coconut source)
    dist, values = actor_critic(states)  #109 (line num in coconut source)
    entropy = ((dist).entropy()).mean()  #110 (line num in coconut source)
    logprobs_ = dist.log_prob(actions)  #111 (line num in coconut source)
    ratios = (pt.exp)((logprobs_ - logprobs))  #112 (line num in coconut source)
    surr_1 = ratios * advantages  #113 (line num in coconut source)
    surr_2 = pt.clamp(ratios, 1.0 - ε, 1.0 + ε) * advantages  #114 (line num in coconut source)
    loss_act = ((_coconut_minus))((pt.min)(*(surr_1, surr_2)))  #115 (line num in coconut source)
    loss_crt = (pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))((returns - values)))  #116 (line num in coconut source)
    losses = 0.5 * loss_crt + loss_act - η * entropy  #117 (line num in coconut source)
    _ = (optimizer).zero_grad()  #118 (line num in coconut source)
    _ = ((pt.mean)(losses)).backward()  #119 (line num in coconut source)
    _ = (optimizer).step()  #120 (line num in coconut source)

    return losses  #122 (line num in coconut source)

def subset(obs, inf, pre):  #122 (line num in coconut source)
    sub = (((pt.vstack)((list)((starmap)(_coconut_forward_compose((lambda i, o: o[i]), pt.from_numpy), (_coconut_partial(zip, {1: obs}, 2))([[i["output-parameters"].index(p) for p in i["output-parameters"] if p.startswith(pre)] for i in inf]))))).to(device)).detach()  #123 (line num in coconut source)

    return sub  #129 (line num in coconut source)

def postprocess(observations, infos):  #129 (line num in coconut source)
    states = (((pt.vstack)((list)((starmap)(_coconut_forward_compose((lambda i, o: o[i]), pt.from_numpy), (_coconut_partial(zip, {1: observations}, 2))([[i["output-parameters"].index(p) for p in i["output-parameters"] if (any)((_coconut_partial(fmap, {0: p.startswith}, 2))(["performance", "target", "distance"]))] for i in infos]))))).to(device)).detach()  #134 (line num in coconut source)
#states = observations |> fmap$pt.from_numpy |> pt.vstack |> .to(device) |> .detach() 

    return states  #141 (line num in coconut source)

def trajectories(actor_critic: function, envs: gace.envs.vec.VecACE, states: pt.Tensor):  #141 (line num in coconut source)
    dist, values = actor_critic(states)  #143 (line num in coconut source)
    actions = (dist.sample()).detach()  #144 (line num in coconut source)
    logprobs = (dist.log_prob)(actions)  #145 (line num in coconut source)
    observations, rewards_, dones, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #146 (line num in coconut source)
    next_states = postprocess(observations, infos)  #150 (line num in coconut source)
    masks = (_coconut_partial((_coconut_minus), {0: 1}, 2))(((pt.Tensor)(dones)).to(device))  #151 (line num in coconut source)
    rewards = ((pt.Tensor)(rewards_)).to(device)  #152 (line num in coconut source)
    memories = Memory(states, actions, logprobs, rewards, values, masks)  #153 (line num in coconut source)

    return (memories, next_states)  #155 (line num in coconut source)

def gae(r: pt.Tensor, v: pt.Tensor, m: pt.Tensor, v_: pt.Tensor, γ: float=0.99, τ: float=0.95):  #155 (line num in coconut source)
    δ = r + γ * v_ * m - v  #157 (line num in coconut source)
    l = (reversed)((range)(((δ).shape)[0]))  #158 (line num in coconut source)
    i = pt.Tensor([0,]).to(device)  #159 (line num in coconut source)
    g = (reduce(lambda g_, i_: (pt.hstack)((_coconut_partial((_coconut_comma_op), {1: g_}, 2))(δ[i_] + γ * τ * m[i_] * g_[0])), l, i))[_coconut.slice(None, -1)]  #160 (line num in coconut source)
    a = v + g  #162 (line num in coconut source)

    return a  #164 (line num in coconut source)

def save_checkpoint(actor: pt.nn.Module, critic: pt.nn.Module, optimizer: pt.optim, checkpoint_file: str):  #164 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [actor, critic, optimizer])  #167 (line num in coconut source)
    save_dict = (dict)((zip)(*(_coconut_partial((_coconut_comma_op), {0: ["actor", "critic", "optimizer"]}, 2))(state_dicts)))  #168 (line num in coconut source)
    res = pt.save(save_dict, checkpoint_file)  #170 (line num in coconut source)

    return res  #172 (line num in coconut source)

def write_performance(env: gym.Env, step: int, episode: int):  #172 (line num in coconut source)
    target = env.target  #173 (line num in coconut source)
    performance = (ac.current_performance)((env).ace)  #174 (line num in coconut source)
    for k in target.keys():  #175 (line num in coconut source)
        writer.add_scalars(k, {f"Episode_{episode}": performance[k]}, step)  #176 (line num in coconut source)

    return performance  #179 (line num in coconut source)

@_coconut_mark_as_match  #179 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #179 (line num in coconut source)
    _coconut_match_check_0 = False  #179 (line num in coconut source)
    _coconut_match_set_name_loss = _coconut_sentinel  #179 (line num in coconut source)
    _coconut_match_set_name_reward = _coconut_sentinel  #179 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #179 (line num in coconut source)
    if (3 <= _coconut.len(_coconut_match_args) <= 5) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "loss" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "reward" in _coconut_match_kwargs)) == 1) and (_coconut_match_args[2] is True):  #179 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("loss")  #179 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("reward")  #179 (line num in coconut source)
        if not _coconut_match_kwargs:  #179 (line num in coconut source)
            _coconut_match_set_name_loss = _coconut_match_temp_0  #179 (line num in coconut source)
            _coconut_match_set_name_reward = _coconut_match_temp_1  #179 (line num in coconut source)
            _coconut_match_check_0 = True  #179 (line num in coconut source)
    if _coconut_match_check_0:  #179 (line num in coconut source)
        if _coconut_match_set_name_loss is not _coconut_sentinel:  #179 (line num in coconut source)
            loss = _coconut_match_temp_0  #179 (line num in coconut source)
        if _coconut_match_set_name_reward is not _coconut_sentinel:  #179 (line num in coconut source)
            reward = _coconut_match_temp_1  #179 (line num in coconut source)
    if not _coconut_match_check_0:  #179 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, _, True, loss, reward) = (loss, reward) where:', _coconut_match_args)  #179 (line num in coconut source)

    if verbose:  #180 (line num in coconut source)
        print(f"All Environments Done")  #181 (line num in coconut source)
    return (loss, reward)  #182 (line num in coconut source)

@_coconut_addpattern(run_episode)  #182 (line num in coconut source)
@_coconut_tco  #182 (line num in coconut source)
@_coconut_mark_as_match  #182 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #182 (line num in coconut source)
    _coconut_match_check_2 = False  #182 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #182 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #182 (line num in coconut source)
    _coconut_match_set_name_loss = _coconut_sentinel  #182 (line num in coconut source)
    _coconut_match_set_name_reward = _coconut_sentinel  #182 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #182 (line num in coconut source)
    if (3 <= _coconut.len(_coconut_match_args) <= 5) and ("iteration" not in _coconut_match_kwargs) and ("states" not in _coconut_match_kwargs) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "loss" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "reward" in _coconut_match_kwargs)) == 1):  #182 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("iteration")  #182 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("states")  #182 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("loss")  #182 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("reward")  #182 (line num in coconut source)
        if not _coconut_match_kwargs:  #182 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_3  #182 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_4  #182 (line num in coconut source)
            _coconut_match_set_name_loss = _coconut_match_temp_5  #182 (line num in coconut source)
            _coconut_match_set_name_reward = _coconut_match_temp_6  #182 (line num in coconut source)
            _coconut_match_check_2 = True  #182 (line num in coconut source)
    if _coconut_match_check_2:  #182 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #182 (line num in coconut source)
            iteration = _coconut_match_temp_3  #182 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #182 (line num in coconut source)
            states = _coconut_match_temp_4  #182 (line num in coconut source)
        if _coconut_match_set_name_loss is not _coconut_sentinel:  #182 (line num in coconut source)
            loss = _coconut_match_temp_5  #182 (line num in coconut source)
        if _coconut_match_set_name_reward is not _coconut_sentinel:  #182 (line num in coconut source)
            reward = _coconut_match_temp_6  #182 (line num in coconut source)
    if not _coconut_match_check_2:  #182 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode( iteration, states, _, loss, reward\n) = run_episode( iteration + 1, next_states, done, loss_, reward_\n)  where:', _coconut_match_args)  #182 (line num in coconut source)

    @_coconut_mark_as_match  #182 (line num in coconut source)
    def traject(*_coconut_match_args, **_coconut_match_kwargs):  #182 (line num in coconut source)
        _coconut_match_check_1 = False  #182 (line num in coconut source)
        _coconut_match_set_name_memories = _coconut_sentinel  #182 (line num in coconut source)
        _coconut_match_set_name_states = _coconut_sentinel  #182 (line num in coconut source)
        _coconut_match_set_name_step = _coconut_sentinel  #182 (line num in coconut source)
        _coconut_FunctionMatchError = _coconut_get_function_match_error()  #182 (line num in coconut source)
        if (1 <= _coconut.len(_coconut_match_args) <= 2) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "step" in _coconut_match_kwargs)) == 1) and (_coconut.isinstance(_coconut_match_args[0], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_args[0]) == 2):  #182 (line num in coconut source)
            _coconut_match_temp_2 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("step")  #182 (line num in coconut source)
            _coconut_match_set_name_memories = _coconut_match_args[0][0]  #182 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_args[0][1]  #182 (line num in coconut source)
            if not _coconut_match_kwargs:  #182 (line num in coconut source)
                _coconut_match_set_name_step = _coconut_match_temp_2  #182 (line num in coconut source)
                _coconut_match_check_1 = True  #182 (line num in coconut source)
        if _coconut_match_check_1:  #182 (line num in coconut source)
            if _coconut_match_set_name_memories is not _coconut_sentinel:  #182 (line num in coconut source)
                memories = _coconut_match_args[0][0]  #182 (line num in coconut source)
            if _coconut_match_set_name_states is not _coconut_sentinel:  #182 (line num in coconut source)
                states = _coconut_match_args[0][1]  #182 (line num in coconut source)
            if _coconut_match_set_name_step is not _coconut_sentinel:  #182 (line num in coconut source)
                step = _coconut_match_temp_2  #182 (line num in coconut source)
        if not _coconut_match_check_1:  #182 (line num in coconut source)
            raise _coconut_FunctionMatchError('def traject((memories,states), step) = (memories_, states_) where:', _coconut_match_args)  #182 (line num in coconut source)

        t0 = time.time()  #186 (line num in coconut source)
        memory, states_ = trajectories(actor_critic, envs, states)  #187 (line num in coconut source)
        memories_ = memory + memories  #188 (line num in coconut source)
        t1 = time.time()  #189 (line num in coconut source)
        if verbose:  #190 (line num in coconut source)
            r = memory.rewards.mean()  #191 (line num in coconut source)
            print(f"Step {step:03}/{num_steps} took {(t1 - t0):.3f}s | Average Reward: {r:.3f}")  #192 (line num in coconut source)
        writer.add_scalars("_Mean_Step_Reward", {f"Iter_{iteration}": memory.rewards.mean()}, step)  #193 (line num in coconut source)
        _ = (_coconut_partial(write_performance, {1: step, 2: iteration}, 3))(((envs).gace_envs)[0])  #194 (line num in coconut source)
        return (memories_, states_)  #195 (line num in coconut source)

    @_coconut_tco  #195 (line num in coconut source)
    def train(epoch, loader):  #195 (line num in coconut source)
        t0 = time.time()  #196 (line num in coconut source)
        loss = (pt.cat)((list)(starmap(update, loader)))  #197 (line num in coconut source)
        t1 = time.time()  #198 (line num in coconut source)
#if verbose:
#    print(f"Epoch {epoch:03}/{num_epochs} took {(t1 - t0):.3f}s | Average Loss: {loss.mean():.3f}")
        writer.add_scalars("_Mean_Epoch_Loss", {f"Iter_{iteration}": loss.mean().item()}, epoch)  #201 (line num in coconut source)
        writer.add_scalars("_Sum_Epoch_Loss", {f"Iter_{iteration}": loss.sum().item()}, epoch)  #202 (line num in coconut source)
        return _coconut_tail_call((pt.mean), loss)  #203 (line num in coconut source)

    _ = policy.eval()  #203 (line num in coconut source)
    with pt.no_grad():  #204 (line num in coconut source)
        memories_, next_states = reduce(traject, range(num_steps), ((Memory)(*_coconut_iter_getitem((repeat)((pt.empty(0)).to(device)), (_coconut.slice(None, 6)))), states))  #205 (line num in coconut source)
    _ = policy.train()  #209 (line num in coconut source)
    loader = data_loader(memories_)  #210 (line num in coconut source)
    losses = ((pt.stack)([train(epoch, loader) for epoch in (range)(num_epochs)])).detach()  #211 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))((1 - memories_.masks))  #213 (line num in coconut source)
    stop_ = (((pt.mean)(memories_.rewards)).item()) < early_stop  #214 (line num in coconut source)
    done = done_ or stop_ or (iteration >= max_iter)  #215 (line num in coconut source)
    loss_ = pt.cat((loss, losses))  #216 (line num in coconut source)
    reward_ = pt.cat((reward, memories_.rewards))  #217 (line num in coconut source)
    if verbose:  #218 (line num in coconut source)
        r = memories_.rewards.mean().item()  #219 (line num in coconut source)
        print(f"Iteration {iteration:03} | Average Reward {r:.3f}")  #220 (line num in coconut source)
    _ = save_checkpoint(actor, critic, optimizer, model_path)  #221 (line num in coconut source)
    writer.add_scalar(f"_Epsiode_Loss", loss_.mean().item(), iteration)  #222 (line num in coconut source)
    writer.add_scalar(f"_Episode_Reward", reward_.sum().item(), iteration)  #223 (line num in coconut source)

    return _coconut_tail_call(run_episode, iteration + 1, next_states, done, loss_, reward_)  #225 (line num in coconut source)

def run_episodes(episode: int):  #225 (line num in coconut source)
    _ = actor.to(device)  #226 (line num in coconut source)
    _ = critic.to(device)  #227 (line num in coconut source)
    if verbose:  #228 (line num in coconut source)
        print(f"Starting Episode {episode:03}")  #229 (line num in coconut source)
    states = ((pt.vstack)((fmap)(_coconut_forward_compose(pt.from_numpy, _coconut.operator.itemgetter((_coconut.slice(None, obs_dim)))), envs.reset()))).to(device)  #230 (line num in coconut source)
    l = r = (pt.empty(0)).to(device=device)  #231 (line num in coconut source)
    losses, rewards = run_episode(0, states, False, l, r)  #232 (line num in coconut source)
    writer.add_scalar(f"_Mean_Epsiode_Loss", losses.mean().item(), episode)  #233 (line num in coconut source)
    writer.add_scalar(f"_Total_Episode_Reward", rewards.sum().item(), episode)  #234 (line num in coconut source)
    for i in (range)((len)(losses)):  #235 (line num in coconut source)
        writer.add_scalars(f"Epsiode_Loss", {f"Episode_{episode}": losses[i]}, i)  #236 (line num in coconut source)
        writer.add_scalars(f"Epsiode_Reward", {f"Episode_{episode}": rewards[i]}, i)  #237 (line num in coconut source)
    if verbose:  #238 (line num in coconut source)
        print(f"Episode {episode:03} | Total Reward: {rewards.sum():.3f}")  #239 (line num in coconut source)
        print(f"Episode {episode:03} | Average Loss: {losses.mean():.3f}")  #240 (line num in coconut source)
        print(f"Finished Episode {episode:03}")  #241 (line num in coconut source)
    act = (_coconut_forward_compose(_coconut.operator.methodcaller("eval"), _coconut.operator.methodcaller("cpu")))(actor)  #242 (line num in coconut source)
    crt = (_coconut_forward_compose(_coconut.operator.methodcaller("eval"), _coconut.operator.methodcaller("cpu")))(critic)  #243 (line num in coconut source)

## Actor Critic Model
    return (act, crt)  #246 (line num in coconut source)

actor: pt.nn.Module = (act_net(obs_dim, act_dim)).to(device)  #246 (line num in coconut source)
critic: pt.nn.Module = (crt_net(obs_dim)).to(device)  #247 (line num in coconut source)
#deviation: pt.nn.Parameter = std_deviation(act_dim, act_std) |> .to(device)
policy: pt.nn.ModuleList = pt.nn.ModuleList([actor, critic])  #249 (line num in coconut source)

actor_critic: function = _coconut.functools.partial(continuous_actor_critic, actor, critic, act_std)  #251 (line num in coconut source)
...
optimizer: pt.optim = pt.optim.Adam(policy.parameters(), lr=α, betas=βs)  #255 (line num in coconut source)
...
update: function = _coconut.functools.partial(update_policy, actor_critic, optimizer)  #260 (line num in coconut source)
