...
class ValueNet(_coconut.typing.NamedTuple("ValueNet", [("obs_dim", int)]), pt.nn.Module):  #86 (line num in coconut source)
    _coconut_is_data = True  #86 (line num in coconut source)
    __slots__ = ()  #86 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #86 (line num in coconut source)
    def __eq__(self, other):  #86 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #86 (line num in coconut source)
    def __hash__(self):  #86 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #86 (line num in coconut source)
    __match_args__ = ('obs_dim',)  #86 (line num in coconut source)
    def __init__(self, obs_dim: int):  #87 (line num in coconut source)
        super(ValueNet, self).__init__()  #88 (line num in coconut source)
        self.v_net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #89 (line num in coconut source)

    @_coconut_tco  #90 (line num in coconut source)
    def forward(self, state: pt.Tensor):  #90 (line num in coconut source)
        return _coconut_tail_call((self.v_net), state)  #93 (line num in coconut source)

## Critic Network

class CriticNet(_coconut.typing.NamedTuple("CriticNet", [("obs_dim", int), ("act_dim", int)]), pt.nn.Module):  #96 (line num in coconut source)
    _coconut_is_data = True  #96 (line num in coconut source)
    __slots__ = ()  #96 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #96 (line num in coconut source)
    def __eq__(self, other):  #96 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #96 (line num in coconut source)
    def __hash__(self):  #96 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #96 (line num in coconut source)
    __match_args__ = ('obs_dim', 'act_dim')  #96 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #97 (line num in coconut source)
        super(CriticNet, self).__init__()  #98 (line num in coconut source)
        dim = obs_dim + act_dim  #99 (line num in coconut source)
        self.c_net = pt.nn.Sequential(HStack(), pt.nn.Linear(dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #100 (line num in coconut source)

    @_coconut_tco  #101 (line num in coconut source)
    def forward(self, state: pt.Tensor, action: pt.Tensor):  #101 (line num in coconut source)
        return _coconut_tail_call((self.c_net), (state, action))  #105 (line num in coconut source)

## Actor Network

class ActorNet(_coconut.typing.NamedTuple("ActorNet", [("obs_dim", int), ("act_dim", int), ("σ_min", float), ("σ_max", float)]), pt.nn.Module):  #108 (line num in coconut source)
    _coconut_is_data = True  #108 (line num in coconut source)
    __slots__ = ()  #108 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #108 (line num in coconut source)
    def __eq__(self, other):  #108 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #108 (line num in coconut source)
    def __hash__(self):  #108 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #108 (line num in coconut source)
    __match_args__ = ('obs_dim', 'act_dim', 'σ_min', 'σ_max')  #108 (line num in coconut source)
    def __new__(_coconut_cls, obs_dim, act_dim, σ_min=-2.0, σ_max=20.0):  #108 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (obs_dim, act_dim, σ_min, σ_max))  #108 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #110 (line num in coconut source)
        super(ActorNet, self).__init__()  #111 (line num in coconut source)
        self.lin_1 = pt.nn.Linear(obs_dim, 256)  #112 (line num in coconut source)
        self.lin_2 = pt.nn.Linear(256, 128)  #113 (line num in coconut source)
        self.lin_3 = pt.nn.Linear(128, 64)  #114 (line num in coconut source)
        self.lin_μ = pt.nn.Linear(64, act_dim)  #115 (line num in coconut source)
        self.lin_σ = pt.nn.Linear(64, act_dim)  #116 (line num in coconut source)
        self.relu = pt.nn.functional.relu  #117 (line num in coconut source)
        self.min_σ = σ_min  #118 (line num in coconut source)
        self.max_σ = σ_max  #119 (line num in coconut source)

    def forward(self, state: pt.Tensor):  #120 (line num in coconut source)
        x = (self.lin_3)((self.relu)((self.lin_2)((self.relu)((self.lin_1)(state)))))  #121 (line num in coconut source)
        μ = (self.lin_μ)(x)  #122 (line num in coconut source)
        σ = (_coconut_partial(pt.clamp, {1: self.min_σ, 2: self.max_σ}, 3))((self.lin_σ)(x))  #123 (line num in coconut source)

        return (μ, σ)  #125 (line num in coconut source)

def evaluate(actor: ActorNet, state: pt.Tensor, ε: float=1e-6):  #125 (line num in coconut source)
    μ, σ_ = (actor)(state)  #127 (line num in coconut source)
    σ = (pt.exp)(σ_)  #128 (line num in coconut source)
    n = pt.distributions.Normal(μ, σ)  #129 (line num in coconut source)
    z = n.sample()  #130 (line num in coconut source)
    a = (pt.tanh)(z)  #131 (line num in coconut source)
    ε_ = pt.log(1 - a.pow(2) + ε)  #132 (line num in coconut source)
    p = ((_coconut_partial((_coconut_minus), {1: ε_}, 2))((n).log_prob(z))).sum(-1, keepdim=True)  #133 (line num in coconut source)

    return (a, p, z, μ, σ)  #135 (line num in coconut source)

def act(actor: ActorNet, state: pt.Tensor):  #135 (line num in coconut source)
    μ, σ_ = (actor)(state)  #136 (line num in coconut source)
    σ = (pt.exp)(σ_)  #137 (line num in coconut source)
    a = ((pt.tanh)(((pt.distributions.Normal)(*(μ, σ))).sample())).detach()  #138 (line num in coconut source)

    return a  #140 (line num in coconut source)

def sync_params(target, online):  #140 (line num in coconut source)
    _ = (list)((_coconut_partial(starmap, {0: lambda t, o: (t.copy_)((t * (1.0 - τ_soft) + o * τ_soft))}, 2))((zip)(*(fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("parameters"), list, _coconut.functools.partial(fmap, _coconut.operator.attrgetter("data"))), [target, online]))))  #141 (line num in coconut source)

## Agent Setup
    return target  #145 (line num in coconut source)

q_criterion = pt.nn.MSELoss()  #145 (line num in coconut source)
v_criterion = pt.nn.MSELoss()  #146 (line num in coconut source)

value_online = (ValueNet(obs_dim)).to(device)  #148 (line num in coconut source)
value_target = (ValueNet(obs_dim)).to(device)  #149 (line num in coconut source)

for tp, op in (zip)(*(fmap)(_coconut.operator.methodcaller("parameters"), [value_target, value_online])):  #151 (line num in coconut source)
    (tp.data.copy_)((op).data)  #152 (line num in coconut source)

critic_online = (CriticNet(obs_dim, act_dim)).to(device)  #154 (line num in coconut source)
actor_online = (ActorNet(obs_dim, act_dim)).to(device)  #155 (line num in coconut source)

v_optim = pt.optim.Adam(value_online.parameters(), lr=α, betas=βs)  #157 (line num in coconut source)
q_optim = pt.optim.Adam(critic_online.parameters(), lr=α, betas=βs)  #158 (line num in coconut source)
p_optim = pt.optim.Adam(actor_online.parameters(), lr=α, betas=βs)  #159 (line num in coconut source)

## Update
def soft_q_update(epoch: int, buffer: ReplayBuffer, batch_size: int, γ: float=0.99, μλ: float=1e-3, σλ: float=1e-3, zλ: float=0.0, τ_soft: float=1e-2):  #162 (line num in coconut source)
    states, actions, rewards, next_states, dones = (fmap)(_coconut.operator.methodcaller("to", device), buffer.sample(batch_size))  #166 (line num in coconut source)
    v_expected = value_online(states)  #168 (line num in coconut source)
    q_expected = critic_online(states, actions)  #169 (line num in coconut source)
    new_actions, log_prob, z, μ, σ = evaluate(actor_online, states)  #170 (line num in coconut source)
    v_target = value_target(next_states)  #172 (line num in coconut source)
    q_next = ((rewards + (1 - dones) * γ * v_target)).detach()  #173 (line num in coconut source)
    q_loss = q_criterion(q_expected, q_next)  #174 (line num in coconut source)
    q_exp_nxt = critic_online(states, new_actions)  #175 (line num in coconut source)
    v_next = ((q_exp_nxt - log_prob)).detach()  #176 (line num in coconut source)
    v_loss = v_criterion(v_expected, v_next)  #177 (line num in coconut source)
    lp_target = q_exp_nxt - v_expected  #178 (line num in coconut source)
    l_loss = (((log_prob * (log_prob - lp_target))).detach()).mean()  #179 (line num in coconut source)
    μ_loss = (_coconut_partial((_coconut.operator.mul), {0: μλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(μ)))  #180 (line num in coconut source)
    σ_loss = (_coconut_partial((_coconut.operator.mul), {0: σλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(σ)))  #181 (line num in coconut source)
    z_loss = (_coconut_partial((_coconut.operator.mul), {0: zλ}, 2))((pt.mean)(((_coconut_partial(pt.pow, {1: 2}, 2))(z)).sum(1)))  #182 (line num in coconut source)
    p_loss = l_loss + μ_loss + σ_loss + z_loss  #183 (line num in coconut source)
    _ = q_optim.zero_grad()  #184 (line num in coconut source)
    _ = q_loss.backward()  #185 (line num in coconut source)
    _ = q_optim.step()  #186 (line num in coconut source)
    _ = v_optim.zero_grad()  #187 (line num in coconut source)
    _ = v_loss.backward()  #188 (line num in coconut source)
    _ = v_optim.step()  #189 (line num in coconut source)
    _ = p_optim.zero_grad()  #190 (line num in coconut source)
    _ = p_loss.backward()  #191 (line num in coconut source)
    _ = p_optim.step()  #192 (line num in coconut source)
    _ = sync_params(value_target, value_online)  #193 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Value", v_loss, epoch)  #194 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic", q_loss, epoch)  #195 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Actor", p_loss, epoch)  #196 (line num in coconut source)

## Exploration
    return (v_loss, q_loss, p_loss)  #199 (line num in coconut source)

def subset(obs, inf, pre):  #199 (line num in coconut source)
    sub = (((pt.vstack)((list)((starmap)(_coconut_forward_compose((lambda i, o: o[i]), pt.from_numpy), (_coconut_partial(zip, {1: obs}, 2))([[i["output-parameters"].index(p) for p in i["output-parameters"] if p.startswith(pre)] for i in inf]))))).to(device)).detach()  #200 (line num in coconut source)

    return sub  #206 (line num in coconut source)

def postprocess(observations, infos):  #206 (line num in coconut source)
    states = (((pt.vstack)((list)((starmap)(_coconut_forward_compose((lambda i, o: o[i]), pt.from_numpy), (_coconut_partial(zip, {1: observations}, 2))([[i["output-parameters"].index(p) for p in i["output-parameters"] if (any)((_coconut_partial(fmap, {0: p.startswith}, 2))(["performance", "target", "distance"]))] for i in infos]))))).to(device)).detach()  #211 (line num in coconut source)
#states = observations |> fmap$pt.from_numpy |> pt.vstack |> .to(device) |> .detach() 

    return states  #218 (line num in coconut source)

@_coconut_mark_as_match  #218 (line num in coconut source)
def train(*_coconut_match_args, **_coconut_match_kwargs):  #218 (line num in coconut source)
    _coconut_match_check_0 = False  #218 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #218 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) == 5) and (_coconut_match_args[2] is True):  #218 (line num in coconut source)
        if not _coconut_match_kwargs:  #218 (line num in coconut source)
            _coconut_match_check_0 = True  #218 (line num in coconut source)
    if not _coconut_match_check_0:  #218 (line num in coconut source)
        raise _coconut_FunctionMatchError('def train(_, _, True, _, _) = (value_target, critic_online, actor_online)', _coconut_match_args)  #218 (line num in coconut source)

    return (value_target, critic_online, actor_online)  #218 (line num in coconut source)

@_coconut_addpattern(train)  #219 (line num in coconut source)
@_coconut_tco  #219 (line num in coconut source)
@_coconut_mark_as_match  #219 (line num in coconut source)
def train(*_coconut_match_args, **_coconut_match_kwargs):  #219 (line num in coconut source)
    _coconut_match_check_1 = False  #219 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #219 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #219 (line num in coconut source)
    _coconut_match_set_name_finish = _coconut_sentinel  #219 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #219 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #219 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #219 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) <= 5) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "episode" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "finish" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "states" in _coconut_match_kwargs)) == 1):  #219 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("episode")  #219 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("iteration")  #219 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("finish")  #219 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("buffer")  #219 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("states")  #219 (line num in coconut source)
        if not _coconut_match_kwargs:  #219 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_0  #219 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_1  #219 (line num in coconut source)
            _coconut_match_set_name_finish = _coconut_match_temp_2  #219 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_3  #219 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_4  #219 (line num in coconut source)
            _coconut_match_check_1 = True  #219 (line num in coconut source)
    if _coconut_match_check_1:  #219 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #219 (line num in coconut source)
            episode = _coconut_match_temp_0  #219 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #219 (line num in coconut source)
            iteration = _coconut_match_temp_1  #219 (line num in coconut source)
        if _coconut_match_set_name_finish is not _coconut_sentinel:  #219 (line num in coconut source)
            finish = _coconut_match_temp_2  #219 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #219 (line num in coconut source)
            buffer = _coconut_match_temp_3  #219 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #219 (line num in coconut source)
            states = _coconut_match_temp_4  #219 (line num in coconut source)
    if not _coconut_match_check_1:  #219 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def train( episode, iteration, finish, buffer, states\n    ) = train( episode, iteration_, finish_, buffer_, states_\n    ) where:', _coconut_match_args)  #219 (line num in coconut source)

    t0 = time.time()  #222 (line num in coconut source)
    actions = act(actor_online, states)  #223 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #224 (line num in coconut source)
    states_ = postprocess(observations, infos)  #228 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #229 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #230 (line num in coconut source)
    buffer_ = (buffer.push)((ReplayBuffer)(*(states, actions, rewards, states_, dones)))  #231 (line num in coconut source)
    t1 = time.time()  #233 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #234 (line num in coconut source)
#_           = (if buffer_.size() > batch_size
#                  then soft_q_update(iteration, buffer_, batch_size)
#                  else None)
    _ = soft_q_update(iteration, buffer_, batch_size)  #238 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(dones)  #239 (line num in coconut source)
    stop_ = (((pt.mean)(rewards)).item()) < early_stop  #240 (line num in coconut source)
    finish_ = done_ or stop_ or (iteration >= num_steps)  #241 (line num in coconut source)
    iteration_ = iteration + 1  #242 (line num in coconut source)
    if verbose:  #243 (line num in coconut source)
        print(f"Iteration {iteration_:03} took {(t1 - t0):.3f}s | Average Reward: {rewards.mean():.3f}")  #244 (line num in coconut source)

    return _coconut_tail_call(train, episode, iteration_, finish_, buffer_, states_)  #246 (line num in coconut source)

def run_episode(episode: int):  #246 (line num in coconut source)
    states = ((pt.vstack)((fmap)(_coconut_forward_compose(pt.from_numpy, _coconut.operator.itemgetter((_coconut.slice(None, obs_dim)))), envs.reset()))).to(device)  #247 (line num in coconut source)
    buffer = (ReplayBuffer)(*(tuple)(_coconut_iter_getitem((repeat)(pt.empty(0)), (_coconut.slice(None, 5)))))  #248 (line num in coconut source)
    v, c, a = train(episode, 0, False, buffer, states)  #249 (line num in coconut source)

    return (v, c, a)  #250 (line num in coconut source)
