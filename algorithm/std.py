...
num_episodes: int = 666  # Number of episodes to play  #11 (line num in coconut source)
num_steps: int = 150  # How many steps to take per episode  #12 (line num in coconut source)
early_stop: float = -500.0  # Early stop criterion  #13 (line num in coconut source)
batch_size: int = 200  # size of the batches during epoch  #14 (line num in coconut source)
rng_seed: int = 666  # Random seed for reproducability  #15 (line num in coconut source)
max_time: float = 20.0  # Maximum time to cut off  #16 (line num in coconut source)

## Environment Parameters
ace_id: str = "op2"  # ACE Identifier of the Environment  #19 (line num in coconut source)
ace_backend: str = "xh035"  # PDK/Technology backend of the ACE Environment  #20 (line num in coconut source)
ace_variant: int = 0  # ACE Environment variant  #21 (line num in coconut source)

## Algorithm Hyper Parameters
algorithm: str = "std"  # Name of used algorithm ∈  ./algorithm  #24 (line num in coconut source)
...
σ_min: float = -20  # Lower Clipping  #27 (line num in coconut source)
σ_max: float = 2  # Upper Clipping  #28 (line num in coconut source)

## NN Hyper Parameters
weights_init: float = 3e-3  # Weight Initializer  #31 (line num in coconut source)
α_actor: float = 2e-4  # Learning Rate for Actor  #32 (line num in coconut source)
α_critic: float = 1e-4  # Learning Rate for Critic  #33 (line num in coconut source)
α_value: float = 2e-4  # Learning Rate for Value  #34 (line num in coconut source)
β1: float = 0.9  # 0.9  #36 (line num in coconut source)
β2: float = 0.999  # 0.999  #37 (line num in coconut source)

## Memory Hyper Parameters
βs: tuple[float] = (β1, β2)  # Maximum size of replay buffer  #40 (line num in coconut source)
buffer_size: int = (int)(1e7)  # Maximum size of replay buffer  #40 (line num in coconut source)
α_per: float = 0.6  #41 (line num in coconut source)
β_start: float = 0.4  #42 (line num in coconut source)
β_frames: int = 10000  #43 (line num in coconut source)

## Setup
env_id: str = f"gace:{ace_id}-{ace_backend}-v{ace_variant}"  #46 (line num in coconut source)
time_stamp: str = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")  #47 (line num in coconut source)
model_dir: str = f"./models/{time_stamp}-{env_id}-{algorithm}"  #48 (line num in coconut source)
model_path: str = f"{model_dir}/checkpoint.pt"  #49 (line num in coconut source)
log_dir: str = f"./runs/{time_stamp}-{env_id}-{algorithm}/"  #50 (line num in coconut source)
os.makedirs(model_dir, exist_ok=True)  #51 (line num in coconut source)

device = pt.device("cuda:1") if pt.cuda.is_available() else pt.device("cpu")  #53 (line num in coconut source)
writer = SummaryWriter(log_dir=log_dir, flush_secs=30)  #54 (line num in coconut source)
_ = (pt.manual_seed)(rng_seed)  #55 (line num in coconut source)

## Utility
def write_performance(env: gym.Env, step: int):  #58 (line num in coconut source)
    target = env.target  #59 (line num in coconut source)
    performance = (ac.current_performance)((env).ace)  #60 (line num in coconut source)
    for k in target.keys():  #61 (line num in coconut source)
        writer.add_scalars(k, {"performance": performance[k], "target": target[k]}, step)  #62 (line num in coconut source)

    return performance  #66 (line num in coconut source)

def save_checkpoint(model: Model, checkpoint_file: str):  #66 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [model.actor, model.critic_1, model.critic_2, model.value_online, model.value_target, model.actor_opt, model.critic_1_opt, model.critic_2_opt, model.value_opt])  #67 (line num in coconut source)
    keys = ["actor", "critic_1", "critic_2", "value_online", "value_target", "actor_opt", "critic_1_opt", "critic_2_opt", "value_opt"]  #71 (line num in coconut source)
    save_dict = (dict)((zip)(*(keys, state_dicts)))  #74 (line num in coconut source)
    res = (pt.save)(*(save_dict, checkpoint_file))  #75 (line num in coconut source)

## Prioritized Experience Replay Buffer
    return res  #78 (line num in coconut source)

class PERBuffer(_coconut.typing.NamedTuple("PERBuffer", [("priority", pt.Tensor), ("state", pt.Tensor), ("action", pt.Tensor), ("reward", pt.Tensor), ("next_state", pt.Tensor), ("done", pt.Tensor)])):  #78 (line num in coconut source)
    _coconut_is_data = True  #78 (line num in coconut source)
    __slots__ = ()  #78 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #78 (line num in coconut source)
    def __eq__(self, other):  #78 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #78 (line num in coconut source)
    def __hash__(self):  #78 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #78 (line num in coconut source)
    __match_args__ = ('priority', 'state', 'action', 'reward', 'next_state', 'done')  #78 (line num in coconut source)
    @_coconut_tco  #78 (line num in coconut source)
    def __len__(self):  #78 (line num in coconut source)
        return _coconut_tail_call((int), ((self.state).shape)[0])  #80 (line num in coconut source)
...
def push_per_buffer(buffer: PERBuffer, other: PERBuffer):  #89 (line num in coconut source)
    ...
    buffer_ = (PERBuffer)(*(map)(_coconut_forward_compose(pt.cat, _coconut.operator.itemgetter((_coconut.slice(None, buffer_size)))), (_coconut_partial(zip, {0: buffer}, 2))((PERBuffer)(prio, *(map)(_coconut.operator.methodcaller("to", device), other[1:])))))  #94 (line num in coconut source)

    return buffer_  #98 (line num in coconut source)

def sample_per_buffer(buffer: PERBuffer, batch_size: int, β: float=β_start):  #98 (line num in coconut source)
    N = (len)(buffer)  #100 (line num in coconut source)
    p = (pt.squeeze)(buffer.priority)  #101 (line num in coconut source)
    p_ = pt.pow(p, α_per)  #102 (line num in coconut source)
    P = p_ / pt.sum(p_)  #103 (line num in coconut source)
    i = ((_coconut.functools.partial(_coconut_iter_getitem, pt.arange(0, N)))(P.multinomial(num_samples=batch_size, replacement=False))).tolist()  #104 (line num in coconut source)
    samples = (PERBuffer)(*(map)(_coconut.operator.itemgetter((i)), buffer))  #106 (line num in coconut source)
    w_ = (_coconut_partial(pt.pow, {1: (-β)}, 2))((N * P[i]))  #107 (line num in coconut source)
    w = w_ / pt.max(w_)  #108 (line num in coconut source)

    return (samples, i, w)  #110 (line num in coconut source)

def update_per_buffer(buffer: PERBuffer, idx: list[int], priority: pt.Tensor):  #110 (line num in coconut source)
    prio = buffer[0]  #112 (line num in coconut source)
    prio[idx] = priority.reshape(-1, 1)  #113 (line num in coconut source)
    buffer_ = PERBuffer(prio, *buffer[1:])  #114 (line num in coconut source)

    return buffer_  #116 (line num in coconut source)

def β_by_frame(f_idx: pt.Tensor, β_start: float=0.4, β_frames: int=1000):  #116 (line num in coconut source)
    β_ = (β_start + f_idx * (1.0 - β_start) / β_frames)  #118 (line num in coconut source)
    f_β = (_coconut_partial(pt.min, {1: β_}, 2))((pt.ones_like)(f_idx))  #119 (line num in coconut source)

## Neural Networks
    return f_β  #122 (line num in coconut source)

class HStack(_coconut.collections.namedtuple("HStack", ()), pt.nn.Module):  #122 (line num in coconut source)
    ...
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #122 (line num in coconut source)
        return _coconut_tail_call((pt.hstack), X)  #123 (line num in coconut source)


class VStack(_coconut.collections.namedtuple("VStack", ()), pt.nn.Module):  #125 (line num in coconut source)
    _coconut_is_data = True  #125 (line num in coconut source)
    __slots__ = ()  #125 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #125 (line num in coconut source)
    def __eq__(self, other):  #125 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #125 (line num in coconut source)
    def __hash__(self):  #125 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #125 (line num in coconut source)
    __match_args__ = ()  #125 (line num in coconut source)
    @_coconut_tco  #125 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #125 (line num in coconut source)
        return _coconut_tail_call((pt.vstack), X)  #126 (line num in coconut source)

## Actor Network

class ActorNet(_coconut.typing.NamedTuple("ActorNet", [("obs_dim", int), ("act_dim", int), ("σ_min", float), ("σ_max", float)]), pt.nn.Module):  #129 (line num in coconut source)
    ...
    __match_args__ = ('obs_dim', 'act_dim', 'σ_min', 'σ_max')  #129 (line num in coconut source)
    def __new__(_coconut_cls, obs_dim, act_dim, σ_min=-2.0, σ_max=20.0):  #129 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (obs_dim, act_dim, σ_min, σ_max))  #129 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #131 (line num in coconut source)
        super(ActorNet, self).__init__()  #132 (line num in coconut source)
        self.lin_1 = pt.nn.Linear(obs_dim, 256)  #133 (line num in coconut source)
        self.lin_2 = pt.nn.Linear(256, 128)  #134 (line num in coconut source)
        self.lin_3 = pt.nn.Linear(128, 64)  #135 (line num in coconut source)
        self.lin_μ = pt.nn.Linear(64, act_dim)  #136 (line num in coconut source)
        self.lin_σ = pt.nn.Linear(64, act_dim)  #137 (line num in coconut source)
        self.relu = pt.nn.functional.relu  #138 (line num in coconut source)
        self.min_σ = σ_min  #139 (line num in coconut source)
        self.max_σ = σ_max  #140 (line num in coconut source)

    def forward(self, state: pt.Tensor):  #141 (line num in coconut source)
        x = (self.lin_3)((self.relu)((self.lin_2)((self.relu)((self.lin_1)(state)))))  #142 (line num in coconut source)
        μ = (self.lin_μ)(x)  #143 (line num in coconut source)
        σ = (_coconut_partial(pt.clamp, {1: self.min_σ, 2: self.max_σ}, 3))((self.lin_σ)(x))  #144 (line num in coconut source)

## Critic Network
        return (μ, σ)  #147 (line num in coconut source)

class CriticNet(_coconut.typing.NamedTuple("CriticNet", [("obs_dim", int), ("act_dim", int)]), pt.nn.Module):  #147 (line num in coconut source)
    _coconut_is_data = True  #147 (line num in coconut source)
    __slots__ = ()  #147 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #147 (line num in coconut source)
    def __eq__(self, other):  #147 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #147 (line num in coconut source)
    def __hash__(self):  #147 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #147 (line num in coconut source)
    __match_args__ = ('obs_dim', 'act_dim')  #147 (line num in coconut source)
    def __init__(self, obs_dim: int, act_dim: int):  #148 (line num in coconut source)
        super(CriticNet, self).__init__()  #149 (line num in coconut source)
        dim = obs_dim + act_dim  #150 (line num in coconut source)
        self.c_net = pt.nn.Sequential(HStack(), pt.nn.Linear(dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #151 (line num in coconut source)

    @_coconut_tco  #152 (line num in coconut source)
    def forward(self, state: pt.Tensor, action: pt.Tensor):  #152 (line num in coconut source)
        return _coconut_tail_call((self.c_net), (state, action))  #156 (line num in coconut source)

## Value Network

class ValueNet(_coconut.typing.NamedTuple("ValueNet", [("obs_dim", int)]), pt.nn.Module):  #159 (line num in coconut source)
    _coconut_is_data = True  #159 (line num in coconut source)
    __slots__ = ()  #159 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #159 (line num in coconut source)
    def __eq__(self, other):  #159 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #159 (line num in coconut source)
    def __hash__(self):  #159 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #159 (line num in coconut source)
    __match_args__ = ('obs_dim',)  #159 (line num in coconut source)
    def __init__(self, obs_dim: int):  #160 (line num in coconut source)
        super(ValueNet, self).__init__()  #161 (line num in coconut source)
        self.v_net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #162 (line num in coconut source)

    @_coconut_tco  #163 (line num in coconut source)
    def forward(self, state: pt.Tensor):  #163 (line num in coconut source)
        return _coconut_tail_call((self.v_net), state)  #166 (line num in coconut source)

## SAC Model

class Model(_coconut.typing.NamedTuple("Model", [("actor", pt.nn.Module), ("critic_1", pt.nn.Module), ("critic_2", pt.nn.Module), ("value_online", pt.nn.Module), ("value_target", pt.nn.Module), ("actor_opt", pt.optim.Optimizer), ("critic_1_opt", pt.optim.Optimizer), ("critic_2_opt", pt.optim.Optimizer), ("value_opt", pt.optim.Optimizer), ("critic_loss", _coconut.typing.Any), ("value_loss", _coconut.typing.Any)])):  #169 (line num in coconut source)
    _coconut_is_data = True  #169 (line num in coconut source)
    __slots__ = ()  #169 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #169 (line num in coconut source)
    def __eq__(self, other):  #169 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #169 (line num in coconut source)
    def __hash__(self):  #169 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #169 (line num in coconut source)
    __match_args__ = ('actor', 'critic_1', 'critic_2', 'value_online', 'value_target', 'actor_opt', 'critic_1_opt', 'critic_2_opt', 'value_opt', 'critic_loss', 'value_loss')  #169 (line num in coconut source)
    def soft_sync(self, other=None):  #169 (line num in coconut source)
        source = other if other else self.value_online  #175 (line num in coconut source)
        target = self.value_target  #176 (line num in coconut source)
        update = lambda t, o: (t.copy_)((t * (1.0 - τ_soft) + o * τ_soft))  #177 (line num in coconut source)
        _ = (list)((_coconut_partial(starmap, {0: update}, 2))((zip)(*(fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("parameters"), list, _coconut.functools.partial(fmap, _coconut.operator.attrgetter("data"))), [target, source]))))  #178 (line num in coconut source)
        return self.value_target  #179 (line num in coconut source)

    def act(self, state: pt.Tensor):  #179 (line num in coconut source)
        μ, log_σ = (self.actor)(state)  #181 (line num in coconut source)
        σ = (pt.exp)(log_σ)  #182 (line num in coconut source)
        a = ((pt.tanh)(((pt.distributions.Normal)(*(μ, σ))).sample())).detach()  #183 (line num in coconut source)
        return a  #184 (line num in coconut source)

    def evaluate(self, state: pt.Tensor, ε: float=1e-6):  #184 (line num in coconut source)
        μ, log_σ = (self.actor)(state)  #186 (line num in coconut source)
        σ = (pt.exp)(log_σ)  #187 (line num in coconut source)
        n = pt.distributions.Normal(μ, σ)  #188 (line num in coconut source)
        z = n.sample()  #189 (line num in coconut source)
        a = (pt.tanh)(z)  #190 (line num in coconut source)
        ε_ = pt.log(1 - a.pow(2) + ε)  #191 (line num in coconut source)
        p = ((_coconut_partial((_coconut_minus), {1: ε_}, 2))((n).log_prob(z))).sum(-1, keepdim=True)  #192 (line num in coconut source)

        return (a, p, z, μ, log_σ)  #194 (line num in coconut source)

def make_model(act_dim: int, obs_dim: int):  #194 (line num in coconut source)
    def init_weights(m: pt.nn.Module, init: float):  #195 (line num in coconut source)
        if isinstance(m, pt.nn.Linear) and (m.out_features == 1 or m.out_features == act_dim):  #196 (line num in coconut source)
            m.weight.data.uniform_(-init, init)  #197 (line num in coconut source)
            m.bias.data.uniform_(-init, init)  #198 (line num in coconut source)
#critic_loss   = pt.nn.MSELoss()

    critic_loss = lambda qe, qt, w: (_coconut_partial((_coconut.operator.mul), {0: 0.5}, 2))((pt.mean)((_coconut_partial((_coconut.operator.mul), {1: w}, 2))((_coconut_partial(pt.pow, {1: 2}, 2))((qe - qt)))))  #200 (line num in coconut source)
    value_loss = pt.nn.MSELoss()  #202 (line num in coconut source)
    actor = (ActorNet(obs_dim, act_dim)).to(device)  #203 (line num in coconut source)
    critic_1 = (CriticNet(obs_dim, act_dim)).to(device)  #204 (line num in coconut source)
    critic_2 = (CriticNet(obs_dim, act_dim)).to(device)  #205 (line num in coconut source)
    value_online = (ValueNet(obs_dim)).to(device)  #206 (line num in coconut source)
    _ = [(n.apply)(_coconut_partial(init_weights, {1: weights_init}, 2)) for n in [actor, critic_1, critic_2, value_online]]  #207 (line num in coconut source)
    value_target = (ValueNet(obs_dim)).to(device)  #209 (line num in coconut source)
    params = (zip)(*(fmap)(_coconut.operator.methodcaller("parameters"), [value_target, value_online]))  #210 (line num in coconut source)
    _ = [(tp.data.copy_)((op).data) for tp, op in params]  #211 (line num in coconut source)
    actor_opt = pt.optim.Adam(actor.parameters(), lr=α_actor, betas=βs)  #212 (line num in coconut source)
    critic_1_opt = pt.optim.Adam(critic_1.parameters(), lr=α_critic, betas=βs)  #214 (line num in coconut source)
    critic_2_opt = pt.optim.Adam(critic_2.parameters(), lr=α_critic, betas=βs)  #216 (line num in coconut source)
    value_opt = pt.optim.Adam(value_online.parameters(), lr=α_value, betas=βs)  #218 (line num in coconut source)
    model = Model(actor, critic_1, critic_2, value_online, value_target, actor_opt, critic_1_opt, critic_2_opt, value_opt, critic_loss, value_loss)  #220 (line num in coconut source)

## Environment Step Post Processing
    return model  #225 (line num in coconut source)

def postprocess(observations, keys: dict[str, list[str]]):  #225 (line num in coconut source)
    pf = lambda k: ":" not in k and "/" not in k and (k[0]).islower()  #226 (line num in coconut source)
    lf = lambda k: k in ["ugbw", "cof", "sr_f", "sr_r"] or k.endswith(":fug")  #227 (line num in coconut source)
    p_idx = (list)((fmap)(keys["observations"].index, (filter)(pf, keys["observations"])))  #228 (line num in coconut source)
    a_idx = [(keys["observations"].index)(k) for k in keys["actions"]]  #231 (line num in coconut source)
    idx = (sorted)((p_idx + a_idx))  #232 (line num in coconut source)
    l_msk = ((pt.Tensor)((list)((_coconut_partial(map, {0: lf}, 2))(keys["observations"])))).to(device)  #233 (line num in coconut source)
    of = lambda o: (pt.where(o > 0, pt.log10(o), o) * l_msk) + (o * (1 - l_msk))  #234 (line num in coconut source)
    states = (pt.vstack)([((of)(((pt.from_numpy)(obs)).to(device)))[idx] for obs in observations])  #235 (line num in coconut source)
#states  = (pt.log10(states_) *  l_msk) + (states_ * (~l_msk))
#states = [ obs |> pt.from_numpy for obs in observations
#         ] |> pt.vstack |> .to(device)

## Reward Scaling
    return states  #242 (line num in coconut source)

def scale_rewards(reward: pt.Tensor, ρ: float=1e-3):  #242 (line num in coconut source)
    scaled_reward = (reward - pt.mean(reward)) / (pt.std(reward) + ρ)  #243 (line num in coconut source)

## Update
    return scaled_reward  #246 (line num in coconut source)

def soft_q_update(epoch: int, model: Model, buffer: PERBuffer, batch_size: int, μλ: float=1e-3, σλ: float=1e-3, zλ: float=0.0, γ: float=0.99):  #246 (line num in coconut source)
    β = ((β_by_frame)((pt.tensor)(epoch))).item()  #249 (line num in coconut source)
    buffer_sample, idx, weights = (_coconut_partial(sample_per_buffer, {1: batch_size, 2: β}, 3))(buffer)  #250 (line num in coconut source)
    _, states, actions, rewards_, next_states, dones = (map)(_coconut.operator.methodcaller("to", device), buffer_sample)  #252 (line num in coconut source)
    rewards = (scale_rewards)(rewards_)  #254 (line num in coconut source)
#rewards     = rewards_
    new_actions, log_prob, z, μ, log_σ = (model.evaluate)(states)  #256 (line num in coconut source)
    q_exp_1 = (model.critic_1)(*(states, actions))  #258 (line num in coconut source)
    q_exp_2 = (model.critic_2)(*(states, actions))  #259 (line num in coconut source)
    v_expected = (model.value_online)(states)  #260 (line num in coconut source)
    v_target = (model.value_target)(next_states)  #261 (line num in coconut source)
    q_next = ((rewards + (1 - dones) * γ * v_target)).detach()  #262 (line num in coconut source)
#c1_loss     = (q_exp_1, q_next) |*> model.critic_loss
#c2_loss     = (q_exp_2, q_next) |*> model.critic_loss
    c1_loss = (model.critic_loss)(*(q_exp_1, q_next, weights))  #265 (line num in coconut source)
    c2_loss = (model.critic_loss)(*(q_exp_2, q_next, weights))  #266 (line num in coconut source)
    q_exp_1_nxt = (model.critic_1)(*(states, new_actions))  #267 (line num in coconut source)
    q_exp_2_nxt = (model.critic_2)(*(states, new_actions))  #268 (line num in coconut source)
    q_exp_nxt = (pt.min)(*(q_exp_1_nxt, q_exp_2_nxt))  #269 (line num in coconut source)
    v_next = ((q_exp_nxt - log_prob)).detach()  #270 (line num in coconut source)
    v_loss = (model.value_loss)(*(v_expected, v_next))  #271 (line num in coconut source)
    l_target = q_exp_nxt - v_expected  #272 (line num in coconut source)
    l_loss = (pt.mean)(((log_prob * (log_prob - l_target))).detach())  #273 (line num in coconut source)
    μ_loss = (_coconut_partial((_coconut.operator.mul), {0: μλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(μ)))  #274 (line num in coconut source)
    σ_loss = (_coconut_partial((_coconut.operator.mul), {0: σλ}, 2))((pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))(log_σ)))  #275 (line num in coconut source)
    z_loss = (_coconut_partial((_coconut.operator.mul), {0: zλ}, 2))((pt.mean)(((_coconut_partial(pt.pow, {1: 2}, 2))(z)).sum(1)))  #276 (line num in coconut source)
    a_loss = l_loss + μ_loss + σ_loss + z_loss  #277 (line num in coconut source)
    _ = model.critic_1_opt.zero_grad()  #278 (line num in coconut source)
    _ = c1_loss.backward()  #279 (line num in coconut source)
    _ = model.critic_1_opt.step()  #280 (line num in coconut source)
    _ = model.critic_2_opt.zero_grad()  #281 (line num in coconut source)
    _ = c2_loss.backward()  #282 (line num in coconut source)
    _ = model.critic_2_opt.step()  #283 (line num in coconut source)
    _ = model.value_opt.zero_grad()  #284 (line num in coconut source)
    _ = v_loss.backward()  #285 (line num in coconut source)
    _ = model.value_opt.step()  #286 (line num in coconut source)
    _ = model.actor_opt.zero_grad()  #287 (line num in coconut source)
    _ = a_loss.backward()  #288 (line num in coconut source)
    _ = model.actor_opt.step()  #289 (line num in coconut source)
    _ = model.soft_sync()  #290 (line num in coconut source)
    td_1_err = ((q_next - q_exp_1)).detach()  #291 (line num in coconut source)
    td_2_err = ((q_next - q_exp_2)).detach()  #292 (line num in coconut source)
    priorities = (pt.squeeze)((pt.abs)((((td_1_err + td_2_err) / 2.0) + 1e-5)))  #293 (line num in coconut source)
    buffer_ = (_coconut_partial(update_per_buffer, {1: idx, 2: priorities}, 3))(buffer)  #294 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Actor", a_loss, epoch)  #295 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic_1", c1_loss, epoch)  #296 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Critic_2", c2_loss, epoch)  #297 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Value", v_loss, epoch)  #298 (line num in coconut source)

## Exploration and Training Loop
    return buffer_  #301 (line num in coconut source)

@_coconut_mark_as_match  #301 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #301 (line num in coconut source)
    _coconut_match_check_0 = False  #301 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #301 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #301 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #301 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #301 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) == 7) and ("model" not in _coconut_match_kwargs) and ("episode" not in _coconut_match_kwargs) and ("buffer" not in _coconut_match_kwargs) and (_coconut_match_args[4] is True):  #301 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #301 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #301 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #301 (line num in coconut source)
        if not _coconut_match_kwargs:  #301 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_0  #301 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_1  #301 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_2  #301 (line num in coconut source)
            _coconut_match_check_0 = True  #301 (line num in coconut source)
    if _coconut_match_check_0:  #301 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #301 (line num in coconut source)
            model = _coconut_match_temp_0  #301 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #301 (line num in coconut source)
            episode = _coconut_match_temp_1  #301 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #301 (line num in coconut source)
            buffer = _coconut_match_temp_2  #301 (line num in coconut source)
    if not _coconut_match_check_0:  #301 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, model, episode, _, True, buffer,_) = model where:', _coconut_match_args)  #301 (line num in coconut source)

    total = ((buffer.reward).sum()).item()  #302 (line num in coconut source)
    _ = writer.add_scalar(f"_Reward_Total", total, episode)  #303 (line num in coconut source)
    if verbose:  #304 (line num in coconut source)
        (print)(f"Episode {episode:03} Finished | Total Reward: {total}")  #305 (line num in coconut source)
    return model  #306 (line num in coconut source)

@_coconut_addpattern(run_episode)  #306 (line num in coconut source)
@_coconut_tco  #306 (line num in coconut source)
@_coconut_mark_as_match  #306 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #306 (line num in coconut source)
    _coconut_match_check_1 = False  #306 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #306 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #306 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #306 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #306 (line num in coconut source)
    _coconut_match_set_name_finish = _coconut_sentinel  #306 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #306 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #306 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #306 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) <= 7) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "envs" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "model" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "episode" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "finish" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 5, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "states" in _coconut_match_kwargs)) == 1):  #306 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("envs")  #306 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #306 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #306 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("iteration")  #306 (line num in coconut source)
        _coconut_match_temp_7 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("finish")  #306 (line num in coconut source)
        _coconut_match_temp_8 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #306 (line num in coconut source)
        _coconut_match_temp_9 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("states")  #306 (line num in coconut source)
        if not _coconut_match_kwargs:  #306 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_3  #306 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_4  #306 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_5  #306 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_6  #306 (line num in coconut source)
            _coconut_match_set_name_finish = _coconut_match_temp_7  #306 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_8  #306 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_9  #306 (line num in coconut source)
            _coconut_match_check_1 = True  #306 (line num in coconut source)
    if _coconut_match_check_1:  #306 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #306 (line num in coconut source)
            envs = _coconut_match_temp_3  #306 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #306 (line num in coconut source)
            model = _coconut_match_temp_4  #306 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #306 (line num in coconut source)
            episode = _coconut_match_temp_5  #306 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #306 (line num in coconut source)
            iteration = _coconut_match_temp_6  #306 (line num in coconut source)
        if _coconut_match_set_name_finish is not _coconut_sentinel:  #306 (line num in coconut source)
            finish = _coconut_match_temp_7  #306 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #306 (line num in coconut source)
            buffer = _coconut_match_temp_8  #306 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #306 (line num in coconut source)
            states = _coconut_match_temp_9  #306 (line num in coconut source)
    if not _coconut_match_check_1:  #306 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode( envs, model, episode, iteration, finish, buffer, states\n) = run_episode( envs, model, episode, iteration_\n, finish_, buffer_, states_ ) where:', _coconut_match_args)  #306 (line num in coconut source)

    t0 = time.time()  #309 (line num in coconut source)
    actions = (model.act)(states)  #310 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #311 (line num in coconut source)
    states_ = postprocess(observations, envs.info[0])  #315 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #316 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #317 (line num in coconut source)
    new_buffer = (_coconut_partial(push_per_buffer, {0: buffer}, 2))((make_per_buffer)(*(states, actions, rewards, states_, dones)))  #318 (line num in coconut source)
    t1 = time.time()  #320 (line num in coconut source)
    dt = t1 - t0  #321 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #322 (line num in coconut source)
    buffer_ = (soft_q_update(iteration, model, new_buffer, batch_size) if len(new_buffer) > batch_size else new_buffer)  #323 (line num in coconut source)
    done_ = (_coconut_forward_compose(pt.squeeze, _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(dones)  #326 (line num in coconut source)
    stop_ = (((pt.mean)(rewards)).item()) < early_stop  #327 (line num in coconut source)
    finish_ = done_ or stop_ or (iteration >= num_steps)  # or (dt > max_time)  #328 (line num in coconut source)
    iteration_ = iteration + 1  #329 (line num in coconut source)
    if verbose:  #330 (line num in coconut source)
        (print)(f"Iteration {iteration:03} took {dt:.3f}s | Average Reward: {rewards.mean():.3f}")  #331 (line num in coconut source)
    _ = writer.add_scalar("_Reward_Mean", rewards.mean(), iteration)  #332 (line num in coconut source)
    _ = save_checkpoint(model, model_path)  #333 (line num in coconut source)

## Episode Loop
    return _coconut_tail_call(run_episode, envs, model, episode, iteration_, finish_, buffer_, states_)  #336 (line num in coconut source)

def run_episodes(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #336 (line num in coconut source)
    obs = envs.reset()  #338 (line num in coconut source)
    states = postprocess(obs, envs.info[0])  #339 (line num in coconut source)
    buffer = make_per_buffer()  #340 (line num in coconut source)
    model = run_episode(envs, model, episode, 0, False, buffer, states)  #341 (line num in coconut source)

    return model  #342 (line num in coconut source)
