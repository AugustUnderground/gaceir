...
num_episodes: int = 42  # Number of episodes to play  #19 (line num in coconut source)
num_steps: int = 2  # How many steps to take  #20 (line num in coconut source)
num_iters: int = 100  # Iterations per Epoch  #21 (line num in coconut source)
num_epochs: int = 100  # How many time steps to update policy  #22 (line num in coconut source)
early_stop: float = -50.0  # Early stop criterion  #23 (line num in coconut source)
batch_size: int = 100  # size of the batches during epoch  #24 (line num in coconut source)
act_std: float = 0.25  # standard deviation action distribution  #25 (line num in coconut source)
rng_seed: int = 666  # Random seed for reproducability  #26 (line num in coconut source)
algorithm: str = "td3"  # Name of used algorithm ∈  ./algorithm  #27 (line num in coconut source)
ace_id: str = "op2"  # ACE Identifier of the Environment  #28 (line num in coconut source)
ace_backend: str = "xh035"  # PDK/Technology backend of the ACE Environment  #29 (line num in coconut source)
ace_variant: int = 0  # ACE Environment variant  #30 (line num in coconut source)
γ: float = 0.99  # Discount Factor  #31 (line num in coconut source)
τ_soft: float = 1e-2  # Avantage Factor  #32 (line num in coconut source)
α: float = 1e-4  # Learning Rate  #33 (line num in coconut source)
β1: float = 0.9  #35 (line num in coconut source)
β2: float = 0.999  #36 (line num in coconut source)
βs: tuple[float] = (β1, β2)  # Exploration Noise  #37 (line num in coconut source)
exp_noise: float = 0.15  # Exploration Noise  #37 (line num in coconut source)
pol_noise: float = 0.2  # Noise added to replay actions  #38 (line num in coconut source)
clp_noise: float = 0.5  # Noise clipping  #39 (line num in coconut source)
update_interval: int = 2  # Update every n epochs  #40 (line num in coconut source)
buffer_size: int = (int)(1e7)  # Maximum size of replay buffer  #41 (line num in coconut source)
warmup_periode: int = 100  # Collect experience during warmup  #42 (line num in coconut source)

## Setup
env_id: str = f"gace:{ace_id}-{ace_backend}-v{ace_variant}"  #45 (line num in coconut source)
time_stamp: str = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")  #46 (line num in coconut source)
model_dir: str = f"./models/{time_stamp}-{env_id}-{algorithm}"  #47 (line num in coconut source)
model_path: str = f"{model_dir}/checkpoint.pt"  #48 (line num in coconut source)
log_dir: str = f"./runs/{time_stamp}-{env_id}-{algorithm}/"  #49 (line num in coconut source)
os.makedirs(model_dir, exist_ok=True)  #50 (line num in coconut source)
...
## Replay Buffer
    return performance  #67 (line num in coconut source)

class ReplayBuffer(_coconut.typing.NamedTuple("ReplayBuffer", [("state", pt.Tensor), ("action", pt.Tensor), ("reward", pt.Tensor), ("next_state", pt.Tensor), ("done", pt.Tensor)])):  #67 (line num in coconut source)
    _coconut_is_data = True  #67 (line num in coconut source)
    __slots__ = ()  #67 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #67 (line num in coconut source)
    def __eq__(self, other):  #67 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #67 (line num in coconut source)
    def __hash__(self):  #67 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #67 (line num in coconut source)
    __match_args__ = ('state', 'action', 'reward', 'next_state', 'done')  #67 (line num in coconut source)
    def push(self, other):  #67 (line num in coconut source)
        new = ((fmap)(_coconut.operator.itemgetter((_coconut.slice(-buffer_size, None))), (ReplayBuffer)(*other)) if len(self.state) <= 0 else (ReplayBuffer)(*(fmap)(_coconut_forward_compose(pt.vstack, _coconut.operator.itemgetter((_coconut.slice(-buffer_size, None)))), zip(self, other))))  #70 (line num in coconut source)
        return new  #71 (line num in coconut source)

    def sample(self, batch_size: int):  #71 (line num in coconut source)
        idx = (((pt.randperm)(((self.state).shape)[0]))[_coconut.slice(None, batch_size)]).tolist()  #74 (line num in coconut source)
        smpl = (ReplayBuffer)(*(e[idx, :] for e in self))  #75 (line num in coconut source)
        return smpl  #76 (line num in coconut source)

    @_coconut_tco  #76 (line num in coconut source)
    def size(self):  #76 (line num in coconut source)
        return _coconut_tail_call((float), ((self.state).shape)[0])  #76 (line num in coconut source)

## Neural Networks

class HStack(_coconut.collections.namedtuple("HStack", ()), pt.nn.Module):  #79 (line num in coconut source)
    ...
    __match_args__ = ()  #79 (line num in coconut source)
    @_coconut_tco  #79 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #79 (line num in coconut source)
        return _coconut_tail_call((pt.hstack), X)  #80 (line num in coconut source)


class VStack(_coconut.collections.namedtuple("VStack", ()), pt.nn.Module):  #82 (line num in coconut source)
    _coconut_is_data = True  #82 (line num in coconut source)
    __slots__ = ()  #82 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #82 (line num in coconut source)
    def __eq__(self, other):  #82 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #82 (line num in coconut source)
    def __hash__(self):  #82 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #82 (line num in coconut source)
    __match_args__ = ()  #82 (line num in coconut source)
    @_coconut_tco  #82 (line num in coconut source)
    def forward(self, X: tuple[pt.Tensor, pt.Tensor]):  #82 (line num in coconut source)
        return _coconut_tail_call((pt.vstack), X)  #83 (line num in coconut source)

## Critic

def critic_net(obs_dim: int, act_dim: int):  #86 (line num in coconut source)
    dim = obs_dim + act_dim  #87 (line num in coconut source)
    crt_net = pt.nn.Sequential(HStack(), pt.nn.Linear(dim, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, 1))  #88 (line num in coconut source)

## Actor
    return crt_net  #95 (line num in coconut source)

def actor_net(obs_dim: int, act_dim: int):  #95 (line num in coconut source)
    act_net = pt.nn.Sequential(pt.nn.Linear(obs_dim, 128), pt.nn.ReLU(), pt.nn.Linear(128, 256), pt.nn.ReLU(), pt.nn.Linear(256, 128), pt.nn.ReLU(), pt.nn.Linear(128, 64), pt.nn.ReLU(), pt.nn.Linear(64, act_dim), pt.nn.Tanh())  #96 (line num in coconut source)

    return act_net  #102 (line num in coconut source)

def soft_update(source: pt.nn.Module, target: pt.nn.Module, τ: float=1e-2):  #102 (line num in coconut source)
    for param, target_param in zip(source.parameters(), target.parameters()):  #104 (line num in coconut source)
        target_param.data.copy_(τ * param.data + (1 - τ) * target_param.data)  #105 (line num in coconut source)

    return target  #107 (line num in coconut source)

class Model(_coconut.typing.NamedTuple("Model", [("Q1", pt.nn.Module), ("Q2", pt.nn.Module), ("π", pt.nn.Module), ("θ1", pt.nn.Module), ("θ2", pt.nn.Module), ("φ", pt.nn.Module), ("Q1_optim", pt.optim.Optimizer), ("Q2_optim", pt.optim.Optimizer), ("π_optim", pt.optim.Optimizer), ("Q_loss", _coconut.typing.Any)])):  #107 (line num in coconut source)
    ...
    __match_args__ = ('Q1', 'Q2', 'π', 'θ1', 'θ2', 'φ', 'Q1_optim', 'Q2_optim', 'π_optim', 'Q_loss')  #107 (line num in coconut source)
    def select_action(self, state: pt.Tensor, step: int):  #107 (line num in coconut source)
        with pt.no_grad():  #112 (line num in coconut source)
            action = ((_coconut_partial(add_noise, {}, 1, t=step))((self.π)(state))).detach()  #113 (line num in coconut source)
        if verbose:  #114 (line num in coconut source)
            print(f"Selecting Agent Action.")  #115 (line num in coconut source)
        return action  #116 (line num in coconut source)

    def eval_mode(self):  #116 (line num in coconut source)
        (fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("eval"), _coconut.operator.methodcaller("to", device)), [self.π, self.Q1, self.Q2, self.φ, self.θ1, self.θ2])  #117 (line num in coconut source)
        return self  #118 (line num in coconut source)

    def train_mode(self):  #118 (line num in coconut source)
        (fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("train"), _coconut.operator.methodcaller("to", device)), [self.π, self.Q1, self.Q2, self.φ, self.θ1, self.θ2])  #120 (line num in coconut source)

        return self  #123 (line num in coconut source)

def make_model(act_dim: int, obs_dim: int):  #123 (line num in coconut source)
    Q1 = (critic_net(obs_dim, act_dim)).to(device)  #124 (line num in coconut source)
    Q2 = (critic_net(obs_dim, act_dim)).to(device)  #125 (line num in coconut source)
    π = (actor_net(obs_dim, act_dim)).to(device)  #126 (line num in coconut source)
    θ1 = (critic_net(obs_dim, act_dim)).to(device)  #127 (line num in coconut source)
    θ2 = (critic_net(obs_dim, act_dim)).to(device)  #128 (line num in coconut source)
    φ = (actor_net(obs_dim, act_dim)).to(device)  #129 (line num in coconut source)
    _ = soft_update(Q1, θ1, τ=1.0)  #130 (line num in coconut source)
    _ = soft_update(Q2, θ2, τ=1.0)  #131 (line num in coconut source)
    _ = soft_update(π, φ, τ=1.0)  #132 (line num in coconut source)
    Q1_optim = pt.optim.Adam(Q1.parameters(), lr=α, betas=βs)  #133 (line num in coconut source)
    Q2_optim = pt.optim.Adam(Q2.parameters(), lr=α, betas=βs)  #134 (line num in coconut source)
    π_optim = pt.optim.Adam(π.parameters(), lr=α, betas=βs)  #135 (line num in coconut source)
    Q_loss = pt.nn.MSELoss()  #136 (line num in coconut source)
    model = Model(Q1, Q2, π, θ1, θ2, φ, Q1_optim, Q2_optim, π_optim, Q_loss)  #137 (line num in coconut source)

## Exploitation
    return model  #140 (line num in coconut source)

def add_noise(a, t=0, lo=-1.0, hi=1.0, σ_max=1.0, σ_min=1.0, d=1000000):  #140 (line num in coconut source)
    σ = σ_max - (σ_max - σ_min) * min(1.0, t / d)  #142 (line num in coconut source)
    a_ = (_coconut_partial(pt.clip, {}, 1, min=lo, max=hi))((a + pt.randn_like(a) * σ))  #143 (line num in coconut source)

    return a_  #145 (line num in coconut source)

def random_action(envs):  #145 (line num in coconut source)
    action = (((pt.vstack)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("sample"), pt.from_numpy), envs.action_space))).detach()).to(device)  #146 (line num in coconut source)
    if verbose:  #148 (line num in coconut source)
        print(f"WARMUP: Selecting Random Action.")  #149 (line num in coconut source)

    return action  #151 (line num in coconut source)

def policy_update(epoch, model, states):  #151 (line num in coconut source)
    π_loss = ((_coconut_minus))(((model.Q1)((_coconut_partial((_coconut_comma_op), {0: states}, 2))((model.π)(states)))).mean())  #152 (line num in coconut source)
    _ = model.π_optim.zero_grad()  #153 (line num in coconut source)
    _ = π_loss.backward()  #154 (line num in coconut source)
    _ = model.π_optim.step()  #155 (line num in coconut source)
    _ = soft_update(model.Q1, model.θ1, τ=τ_soft)  #156 (line num in coconut source)
    _ = soft_update(model.Q2, model.θ2, τ=τ_soft)  #157 (line num in coconut source)
    _ = soft_update(model.π, model.φ, τ=τ_soft)  #158 (line num in coconut source)
    _ = writer.add_scalar("_Loss_π", π_loss, epoch)  #159 (line num in coconut source)

    return π_loss  #161 (line num in coconut source)

def update(epoch, model, buffer):  #161 (line num in coconut source)
    states, actions, rewards, next_states, dones = (fmap)(_coconut.operator.methodcaller("to", device), buffer.sample(batch_size))  #162 (line num in coconut source)
    noise = ((_coconut_partial(pt.clamp, {1: -clp_noise, 2: clp_noise}, 3))((_coconut_partial(pt.normal, {1: pol_noise}, 2))((pt.zeros_like)(actions)))).to(device)  #164 (line num in coconut source)
    next_actions = (_coconut_partial(pt.clamp, {1: -1.0, 2: 1.0}, 3))((_coconut_partial((_coconut.operator.add), {1: noise}, 2))((model.φ)(states)))  #166 (line num in coconut source)
    q1_target = (model.θ1)((next_states, next_actions))  #167 (line num in coconut source)
    q2_target = (model.θ2)((next_states, next_actions))  #168 (line num in coconut source)
    q_target = pt.min(q1_target, q2_target)  #169 (line num in coconut source)
    q_expected = ((rewards + (1.0 - dones) * γ * q_target)).detach()  #170 (line num in coconut source)
    q1 = (model.Q1)((states, actions))  #171 (line num in coconut source)
    q2 = (model.Q2)((states, actions))  #172 (line num in coconut source)
    q1_loss = model.Q_loss(q1, q_expected)  #173 (line num in coconut source)
    q2_loss = model.Q_loss(q2, q_expected)  #174 (line num in coconut source)
    _ = model.Q1_optim.zero_grad()  #175 (line num in coconut source)
    _ = q1_loss.backward()  #176 (line num in coconut source)
    _ = model.Q1_optim.step()  #177 (line num in coconut source)
    _ = model.Q2_optim.zero_grad()  #178 (line num in coconut source)
    _ = q2_loss.backward()  #179 (line num in coconut source)
    _ = model.Q2_optim.step()  #180 (line num in coconut source)
    π_loss = (policy_update(epoch, model, states) if epoch % 2 == 0 else ((pt.tensor)(pt.nan)).to(device))  # epoch in count(0, update_interval)  #181 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Q1", q1_loss, epoch)  #184 (line num in coconut source)
    _ = writer.add_scalar("_Loss_Q2", q2_loss, epoch)  #185 (line num in coconut source)
#if verbose:
#    print(f"Epoch {epoch:03} | Q Loss: {q1_loss:3f}, {q2_loss:3f}")

## Exploration
    return (q1_loss, q2_loss, π_loss)  #190 (line num in coconut source)

def postprocess(observations, keys):  #190 (line num in coconut source)
    idx = [(list)((map)(lambda p: (key.index)(p), (_coconut_partial(filter, {0: lambda k: ":" not in k}, 2))(key))) for key in keys]  #191 (line num in coconut source)
    states = ((pt.vstack)((fmap)(pt.from_numpy, [o[i] for o, i in zip(observations, idx)]))).to(device)  #193 (line num in coconut source)

    return states  #196 (line num in coconut source)

def explore(envs, model, step, states, buffer):  #196 (line num in coconut source)
    actions = (random_action(envs) if (step * len(envs)) < warmup_periode else model.select_action(states, step))  #198 (line num in coconut source)
    t0 = time.time()  #201 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #202 (line num in coconut source)
    next_states = postprocess(observations, [i["output-parameters"] for i in infos])  #206 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #207 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #208 (line num in coconut source)
    new_buffer = (buffer.push)((ReplayBuffer)(*(states, actions, rewards, next_states, dones)))  #209 (line num in coconut source)
    t1 = time.time()  #211 (line num in coconut source)
    _ = writer.add_scalar("_Reward_Mean", rewards.mean().item(), step)  #212 (line num in coconut source)
    if verbose:  #213 (line num in coconut source)
        print(f"Step {step:03} took {(t1 - t0):.3f}s  | Average Reward: {rewards.mean():.3f}")  #214 (line num in coconut source)

## Checkpoint Saving
    return (next_states, new_buffer)  #217 (line num in coconut source)

def save_checkpoint(model: Model, checkpoint_file: str):  #217 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [model.Q1, model.Q2, model.π, model.θ1, model.θ2, model.φ, model.Q1_optim, model.Q2_optim])  #218 (line num in coconut source)
    keys = ["Q1", "Q2", "pi", "T1", "T2", "phi", "Q1_opt", "Q2_opt", "pi_opt"]  #220 (line num in coconut source)
    save_dict = (dict)((zip)(*(keys, state_dicts)))  #222 (line num in coconut source)
    res = pt.save(save_dict, checkpoint_file)  #223 (line num in coconut source)

## Run Episode until done
    return res  #226 (line num in coconut source)
...
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #226 (line num in coconut source)
    _coconut_match_check_0 = False  #226 (line num in coconut source)
    ...
    if (_coconut.len(_coconut_match_args) == 7) and ("model" not in _coconut_match_kwargs) and ("episode" not in _coconut_match_kwargs) and ("buffer" not in _coconut_match_kwargs) and (_coconut_match_args[6] is True):  #226 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #226 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #226 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #226 (line num in coconut source)
        if not _coconut_match_kwargs:  #226 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_0  #226 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_1  #226 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_2  #226 (line num in coconut source)
            _coconut_match_check_0 = True  #226 (line num in coconut source)
    if _coconut_match_check_0:  #226 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #226 (line num in coconut source)
            model = _coconut_match_temp_0  #226 (line num in coconut source)
        ...
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #226 (line num in coconut source)
            buffer = _coconut_match_temp_2  #226 (line num in coconut source)
    if not _coconut_match_check_0:  #226 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, model, episode, _, _, buffer, True) = model where:', _coconut_match_args)  #226 (line num in coconut source)

    total = ((buffer.reward).sum()).item()  #227 (line num in coconut source)
    _ = writer.add_scalar(f"_Reward_Total", total, episode)  #228 (line num in coconut source)
    if verbose:  #229 (line num in coconut source)
        (print)(f"Episode {episode:03} Finished | Total Reward: {total}")  #230 (line num in coconut source)
    return model  #231 (line num in coconut source)

@_coconut_addpattern(run_episode)  #231 (line num in coconut source)
@_coconut_tco  #231 (line num in coconut source)
@_coconut_mark_as_match  #231 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #231 (line num in coconut source)
    _coconut_match_check_1 = False  #231 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #231 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #231 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #231 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #231 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #231 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #231 (line num in coconut source)
    _coconut_match_set_name_done = _coconut_sentinel  #231 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #231 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) <= 7) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "envs" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "model" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "episode" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "states" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 5, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "done" in _coconut_match_kwargs)) == 1):  #231 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("envs")  #231 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #231 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #231 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("iteration")  #231 (line num in coconut source)
        _coconut_match_temp_7 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("states")  #231 (line num in coconut source)
        _coconut_match_temp_8 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #231 (line num in coconut source)
        _coconut_match_temp_9 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("done")  #231 (line num in coconut source)
        if not _coconut_match_kwargs:  #231 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_3  #231 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_4  #231 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_5  #231 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_6  #231 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_7  #231 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_8  #231 (line num in coconut source)
            _coconut_match_set_name_done = _coconut_match_temp_9  #231 (line num in coconut source)
            _coconut_match_check_1 = True  #231 (line num in coconut source)
    if _coconut_match_check_1:  #231 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #231 (line num in coconut source)
            envs = _coconut_match_temp_3  #231 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #231 (line num in coconut source)
            model = _coconut_match_temp_4  #231 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #231 (line num in coconut source)
            episode = _coconut_match_temp_5  #231 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #231 (line num in coconut source)
            iteration = _coconut_match_temp_6  #231 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #231 (line num in coconut source)
            states = _coconut_match_temp_7  #231 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #231 (line num in coconut source)
            buffer = _coconut_match_temp_8  #231 (line num in coconut source)
        if _coconut_match_set_name_done is not _coconut_sentinel:  #231 (line num in coconut source)
            done = _coconut_match_temp_9  #231 (line num in coconut source)
    if not _coconut_match_check_1:  #231 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode( envs, model, episode, iteration, states, buffer, done\n) = run_episode( envs, model, episode, iteration + 1\n, next_states, new_buffer, finish\n) where:', _coconut_match_args)  #231 (line num in coconut source)

    _ = model.eval_mode()  #235 (line num in coconut source)
    expl = lambda sb, e: (_coconut_partial(explore, {0: envs, 1: model, 2: e}, 4))(*sb)  #236 (line num in coconut source)
    steps = range(iteration, iteration + num_steps)  #237 (line num in coconut source)
    next_states, new_buffer = reduce(expl, steps, (states, buffer))  #238 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.attrgetter("done"), _coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(new_buffer)  #240 (line num in coconut source)
    last = len(envs) + num_steps  #241 (line num in coconut source)
    stop_ = new_buffer.reward[-last:].mean().item() < early_stop  #242 (line num in coconut source)
    finish = done_ or stop_ or (iteration >= num_iters)  #243 (line num in coconut source)
    _ = model.train_mode()  #244 (line num in coconut source)
    q1_loss, q2_loss, π_loss_ = (tuple)((_coconut.functools.partial(_coconut.functools.partial, fmap)(pt.hstack))((zip)(*[update(epoch, model, new_buffer) for epoch in (range)(num_epochs)])))  #245 (line num in coconut source)
    π_loss = π_loss_[~π_loss_.isnan()]  #249 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #250 (line num in coconut source)

    return _coconut_tail_call(run_episode, envs, model, episode, iteration + 1, next_states, new_buffer, finish)  #252 (line num in coconut source)

def run_episodes(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #252 (line num in coconut source)
    obs = envs.reset()  #254 (line num in coconut source)
    states = postprocess(obs, envs.observation_keys)  #255 (line num in coconut source)
    buffer = (ReplayBuffer)(*(tuple)(_coconut_iter_getitem((repeat)(pt.empty(0)), (_coconut.slice(None, 5)))))  #256 (line num in coconut source)
    memory = run_episode(envs, model, episode, 0, states, buffer, False)  #257 (line num in coconut source)
    _ = model.eval_mode()  #258 (line num in coconut source)
