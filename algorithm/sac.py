...
def postprocess(observations, keys: dict[str, list[str]]):  #202 (line num in coconut source)
    ...
    p_idx = (list)((fmap)(keys["observations"].index, (filter)(pf, keys["observations"])))  #204 (line num in coconut source)
    i_idx = [(keys["observations"].index)(k) for k in keys["actions"]]  #207 (line num in coconut source)
    idx = p_idx + i_idx  #208 (line num in coconut source)
    log_idx = (list)((fmap)(keys["observations"].index, filter(lambda k: k in ["ugbw", "sr_f", "sr_r"], keys)))  #209 (line num in coconut source)
    states = ((pt.vstack)([(pt.from_numpy)(obs[idx]) for obs in observations])).to(device)  #211 (line num in coconut source)
    states[:, log_idx] = (pt.log)(states[:, log_idx])  #213 (line num in coconut source)
#states = [ obs |> pt.from_numpy for obs in observations
#         ] |> pt.vstack |> .to(device)

## Checkpoint Saving
    return states  #218 (line num in coconut source)

def save_checkpoint(model: Model, checkpoint_file: str):  #218 (line num in coconut source)
    state_dicts = (fmap)(_coconut.operator.methodcaller("state_dict"), [model.actor_online, model.critic_online, model.value_online, model.value_target, model.actor_optim, model.critic_optim, model.value_optim])  #219 (line num in coconut source)
    keys = ["actor_online", "critic_online", "value_online", "value_target", "actor_optim", "critic_optim", "value_optim"]  #222 (line num in coconut source)
    save_dict = (dict)((zip)(*(keys, state_dicts)))  #224 (line num in coconut source)
    res = pt.save(save_dict, checkpoint_file)  #225 (line num in coconut source)

## Training Loop
    return res  #228 (line num in coconut source)

@_coconut_mark_as_match  #228 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #228 (line num in coconut source)
    _coconut_match_check_0 = False  #228 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #228 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #228 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #228 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #228 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) == 7) and ("model" not in _coconut_match_kwargs) and ("episode" not in _coconut_match_kwargs) and ("buffer" not in _coconut_match_kwargs) and (_coconut_match_args[4] is True):  #228 (line num in coconut source)
        _coconut_match_temp_0 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #228 (line num in coconut source)
        _coconut_match_temp_1 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #228 (line num in coconut source)
        _coconut_match_temp_2 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #228 (line num in coconut source)
        if not _coconut_match_kwargs:  #228 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_0  #228 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_1  #228 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_2  #228 (line num in coconut source)
            _coconut_match_check_0 = True  #228 (line num in coconut source)
    if _coconut_match_check_0:  #228 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #228 (line num in coconut source)
            model = _coconut_match_temp_0  #228 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #228 (line num in coconut source)
            episode = _coconut_match_temp_1  #228 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #228 (line num in coconut source)
            buffer = _coconut_match_temp_2  #228 (line num in coconut source)
    if not _coconut_match_check_0:  #228 (line num in coconut source)
        raise _coconut_FunctionMatchError('def run_episode(_, model, episode, _, True, buffer, _) = model where:', _coconut_match_args)  #228 (line num in coconut source)

    total = ((buffer.reward).sum()).item()  #229 (line num in coconut source)
    _ = writer.add_scalar(f"_Reward_Total", total, episode)  #230 (line num in coconut source)
    if verbose:  #231 (line num in coconut source)
        (print)(f"Episode {episode:03} Finished | Total Reward: {total}")  #232 (line num in coconut source)
    return model  #233 (line num in coconut source)

@_coconut_addpattern(run_episode)  #233 (line num in coconut source)
@_coconut_tco  #233 (line num in coconut source)
@_coconut_mark_as_match  #233 (line num in coconut source)
def run_episode(*_coconut_match_args, **_coconut_match_kwargs):  #233 (line num in coconut source)
    _coconut_match_check_1 = False  #233 (line num in coconut source)
    _coconut_match_set_name_envs = _coconut_sentinel  #233 (line num in coconut source)
    _coconut_match_set_name_model = _coconut_sentinel  #233 (line num in coconut source)
    _coconut_match_set_name_episode = _coconut_sentinel  #233 (line num in coconut source)
    _coconut_match_set_name_iteration = _coconut_sentinel  #233 (line num in coconut source)
    _coconut_match_set_name_finish = _coconut_sentinel  #233 (line num in coconut source)
    _coconut_match_set_name_buffer = _coconut_sentinel  #233 (line num in coconut source)
    _coconut_match_set_name_states = _coconut_sentinel  #233 (line num in coconut source)
    _coconut_FunctionMatchError = _coconut_get_function_match_error()  #233 (line num in coconut source)
    if (_coconut.len(_coconut_match_args) <= 7) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "envs" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "model" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "episode" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "finish" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 5, "buffer" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "states" in _coconut_match_kwargs)) == 1):  #233 (line num in coconut source)
        _coconut_match_temp_3 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("envs")  #233 (line num in coconut source)
        _coconut_match_temp_4 = _coconut_match_args[1] if _coconut.len(_coconut_match_args) > 1 else _coconut_match_kwargs.pop("model")  #233 (line num in coconut source)
        _coconut_match_temp_5 = _coconut_match_args[2] if _coconut.len(_coconut_match_args) > 2 else _coconut_match_kwargs.pop("episode")  #233 (line num in coconut source)
        _coconut_match_temp_6 = _coconut_match_args[3] if _coconut.len(_coconut_match_args) > 3 else _coconut_match_kwargs.pop("iteration")  #233 (line num in coconut source)
        _coconut_match_temp_7 = _coconut_match_args[4] if _coconut.len(_coconut_match_args) > 4 else _coconut_match_kwargs.pop("finish")  #233 (line num in coconut source)
        _coconut_match_temp_8 = _coconut_match_args[5] if _coconut.len(_coconut_match_args) > 5 else _coconut_match_kwargs.pop("buffer")  #233 (line num in coconut source)
        _coconut_match_temp_9 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("states")  #233 (line num in coconut source)
        if not _coconut_match_kwargs:  #233 (line num in coconut source)
            _coconut_match_set_name_envs = _coconut_match_temp_3  #233 (line num in coconut source)
            _coconut_match_set_name_model = _coconut_match_temp_4  #233 (line num in coconut source)
            _coconut_match_set_name_episode = _coconut_match_temp_5  #233 (line num in coconut source)
            _coconut_match_set_name_iteration = _coconut_match_temp_6  #233 (line num in coconut source)
            _coconut_match_set_name_finish = _coconut_match_temp_7  #233 (line num in coconut source)
            _coconut_match_set_name_buffer = _coconut_match_temp_8  #233 (line num in coconut source)
            _coconut_match_set_name_states = _coconut_match_temp_9  #233 (line num in coconut source)
            _coconut_match_check_1 = True  #233 (line num in coconut source)
    if _coconut_match_check_1:  #233 (line num in coconut source)
        if _coconut_match_set_name_envs is not _coconut_sentinel:  #233 (line num in coconut source)
            envs = _coconut_match_temp_3  #233 (line num in coconut source)
        if _coconut_match_set_name_model is not _coconut_sentinel:  #233 (line num in coconut source)
            model = _coconut_match_temp_4  #233 (line num in coconut source)
        if _coconut_match_set_name_episode is not _coconut_sentinel:  #233 (line num in coconut source)
            episode = _coconut_match_temp_5  #233 (line num in coconut source)
        if _coconut_match_set_name_iteration is not _coconut_sentinel:  #233 (line num in coconut source)
            iteration = _coconut_match_temp_6  #233 (line num in coconut source)
        if _coconut_match_set_name_finish is not _coconut_sentinel:  #233 (line num in coconut source)
            finish = _coconut_match_temp_7  #233 (line num in coconut source)
        if _coconut_match_set_name_buffer is not _coconut_sentinel:  #233 (line num in coconut source)
            buffer = _coconut_match_temp_8  #233 (line num in coconut source)
        if _coconut_match_set_name_states is not _coconut_sentinel:  #233 (line num in coconut source)
            states = _coconut_match_temp_9  #233 (line num in coconut source)
    if not _coconut_match_check_1:  #233 (line num in coconut source)
        raise _coconut_FunctionMatchError('addpattern def run_episode( envs, model, episode, iteration, finish, buffer, states\n) = run_episode( envs, model, episode, iteration_\n, finish_, buffer_, states_ ) where:', _coconut_match_args)  #233 (line num in coconut source)

    t0 = time.time()  #236 (line num in coconut source)
    actions = (model.act)(states)  #237 (line num in coconut source)
    observations, rewards_, dones_, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #238 (line num in coconut source)
    states_ = postprocess(observations, envs.info[0])  #242 (line num in coconut source)
    rewards = (((pt.Tensor)(rewards_)).to(device)).reshape(-1, 1)  #243 (line num in coconut source)
    dones = (((pt.Tensor)(dones_)).to(device)).reshape(-1, 1)  #244 (line num in coconut source)
    buffer_ = (buffer.push)((ReplayBuffer)(*(states, actions, rewards, states_, dones)))  #245 (line num in coconut source)
    t1 = time.time()  #247 (line num in coconut source)
    dt = t1 - t0  #248 (line num in coconut source)
    _ = (_coconut_partial(write_performance, {1: iteration}, 2))(_coconut_iter_getitem(envs, 0))  #249 (line num in coconut source)
    _ = soft_q_update(iteration, model, buffer_, batch_size)  #250 (line num in coconut source)
    done_ = (_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("bool"), pt.all, _coconut.operator.methodcaller("item")))(dones)  #251 (line num in coconut source)
    stop_ = (((pt.mean)(rewards)).item()) < early_stop  #252 (line num in coconut source)
    finish_ = done_ or stop_ or (iteration >= num_steps) or (dt > max_time)  #253 (line num in coconut source)
    iteration_ = iteration + 1  #254 (line num in coconut source)
    if verbose:  #255 (line num in coconut source)
        (print)(f"Iteration {iteration:03} took {dt:.3f}s | Average Reward: {rewards.mean():.3f}")  #256 (line num in coconut source)
    _ = writer.add_scalar("_Reward_Mean", rewards.mean(), iteration)  #257 (line num in coconut source)
    _ = save_checkpoint(model, model_path)  #258 (line num in coconut source)

## Episode Loop
    return _coconut_tail_call(run_episode, envs, model, episode, iteration_, finish_, buffer_, states_)  #261 (line num in coconut source)

def run_episodes(model: Model, envs: gace.envs.vec.VecACE, episode: int):  #261 (line num in coconut source)
    obs = envs.reset()  #263 (line num in coconut source)
    states = postprocess(obs, envs.info[0])  #264 (line num in coconut source)
    buffer = (ReplayBuffer)(*(tuple)(_coconut_iter_getitem((repeat)(pt.empty(0)), (_coconut.slice(None, 5)))))  #265 (line num in coconut source)
    model = run_episode(envs, model, episode, 0, False, buffer, states)  #266 (line num in coconut source)

    return model  #267 (line num in coconut source)
