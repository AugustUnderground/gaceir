...
η_q: float = 2e-4  # Learning Rate for Critic  #47 (line num in coconut source)
η_α: float = 2e-4  # Learning Rate for Critic  #48 (line num in coconut source)
...
def scale_rewards(reward: pt.Tensor, ρ: float=1e-3):  #135 (line num in coconut source)
    scaled_reward = (reward - pt.mean(reward)) / (pt.std(reward) + ρ)  #136 (line num in coconut source)
...
class PERBuffer(_coconut.typing.NamedTuple("PERBuffer", [("buffer", ReplayBuffer), ("priorities", pt.Tensor), ("α", float), ("β_start", float), ("β_frames", int)])):  #150 (line num in coconut source)
    ...
    __match_args__ = ('buffer', 'priorities', 'α', 'β_start', 'β_frames')  #150 (line num in coconut source)
    def __new__(_coconut_cls, buffer, priorities, α=α_start, β_start=β_start, β_frames=β_frames):  #150 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (buffer, priorities, α, β_start, β_frames))  #150 (line num in coconut source)
...
def mk_per_buffer(α: float=0.6, β_start: float=0.4, β_frames: int=int(1e5)):  #154 (line num in coconut source)
    ...
    buffer = PERBuffer(buf, prio, α, β_start, β_frames)  #158 (line num in coconut source)

    return buffer  #160 (line num in coconut source)

def β_by_frame(buffer: PERBuffer, frame_idx: int):  #160 (line num in coconut source)
    β = (_coconut_partial(min, {0: 1.0}, 2))((buffer.β_start + frame_idx * (1.0 - buffer.β_start) / buffer.β_frames))  #161 (line num in coconut source)

    return β  #164 (line num in coconut source)
...
def per_push(pb: PERBuffer, state: pt.Tensor, action: pt.Tensor, reward: pt.Tensor, next_state: pt.Tensor, done: pt.Tensor):  #164 (line num in coconut source)
    b1, p1, α, β_start, β_frames = pb  #166 (line num in coconut source)
    ...
    buffer = PERBuffer(buf, prio, α, β_start, β_frames)  #173 (line num in coconut source)
...
def per_sample(buffer: PERBuffer, frame_idx: int, batch_size: int=batch_size):  #175 (line num in coconut source)
    ...
    p = ((_coconut_partial(pt.pow, {1: buffer.α}, 2))(buffer.priorities)).squeeze()  #178 (line num in coconut source)
    ...
    β = β_by_frame(buffer, frame_idx)  #183 (line num in coconut source)
    W = (_coconut_partial(pt.pow, {1: -β}, 2))((N * P[i]))  #184 (line num in coconut source)
...
def per_update(buffer: PERBuffer, indices: pt.Tensor, priorities: pt.Tensor):  #187 (line num in coconut source)
    b1, p1, α, β_start, β_frames = buffer  #189 (line num in coconut source)
    ...
    buffer_ = PERBuffer(b1, p2, α, β_start, β_frames)  #193 (line num in coconut source)
...
class Actor(_coconut.typing.NamedTuple("Actor", [("obs_dim", int), ("act_dim", int)]), pt.nn.Module):  #197 (line num in coconut source)
    ...
    def __init__(self, obs_dim: int, act_dim: int, σ_min: float=σ_min, σ_max: float=σ_max):  #198 (line num in coconut source)
        super(Actor, self).__init__()  #200 (line num in coconut source)
        self.σ_min = σ_min  #201 (line num in coconut source)
        self.σ_max = σ_max  #202 (line num in coconut source)
        ...
        self.lin_μ = pt.nn.Linear(256, act_dim)  #205 (line num in coconut source)
        self.lin_σ = pt.nn.Linear(256, act_dim)  #206 (line num in coconut source)
        ...
        weights_init([self.lin_μ, self.lin_σ], limits=(-w_init, w_init))  #209 (line num in coconut source)
    ...
    def forward(self, state):  #210 (line num in coconut source)
        ...
        μ = (self.lin_μ)(s)  #212 (line num in coconut source)
        σ = (_coconut_partial(pt.clamp, {1: self.σ_min, 2: self.σ_max}, 3))((self.lin_σ)(s))  #213 (line num in coconut source)

## Critic
        return (μ, σ)  #216 (line num in coconut source)
...
class Agent(_coconut.typing.NamedTuple("Agent", [("π_online", pt.nn.Module), ("π_optim", pt.optim.Optimizer), ("q1_online", pt.nn.Module), ("q1_optim", pt.optim.Optimizer), ("q2_online", pt.nn.Module), ("q2_optim", pt.optim.Optimizer), ("q1_target", pt.nn.Module), ("q2_target", pt.nn.Module), ("H_target", float), ("π_prior", Callable), ("α_log", pt.Tensor), ("α_optim", pt.optim.Optimizer)])):  #231 (line num in coconut source)
    ...
    __match_args__ = ('π_online', 'π_optim', 'q1_online', 'q1_optim', 'q2_online', 'q2_optim', 'q1_target', 'q2_target', 'H_target', 'π_prior', 'α_log', 'α_optim')  #231 (line num in coconut source)
    def save_state(self, checkpoint_file: str):  #231 (line num in coconut source)
        nets = [self.π_online, self.q1_online, self.q2_online, self.q1_target, self.q2_target]  #238 (line num in coconut source)
        opts = [self.π_optim, self.q1_optim, self.q2_optim, self.α_optim]  #240 (line num in coconut source)
        tens = [self.α_log,]  #241 (line num in coconut source)
...
def act(agent: Agent, state: pt.Tensor):  #255 (line num in coconut source)
    ...
    a = ((pt.tanh)((μ + σ * ε))).detach()  #260 (line num in coconut source)

    return a  #262 (line num in coconut source)

def evaluate(agent: Agent, state: pt.Tensor, ε_n: float=ε_noise):  #262 (line num in coconut source)
    μ, σ_log = (agent.π_online)(state)  #264 (line num in coconut source)
    σ = (pt.exp)(σ_log)  #265 (line num in coconut source)
    N = pt.distributions.Normal(0, 1)  #266 (line num in coconut source)
    ε = (N).sample()  #267 (line num in coconut source)
    a = (pt.tanh)((μ + σ * ε))  #268 (line num in coconut source)
    l1 = ((pt.distributions.Normal)(*(μ, σ))).log_prob(μ + σ * ε)  #269 (line num in coconut source)
    l2 = (pt.log)((1 - pt.pow(a, 2) + ε_n))  #270 (line num in coconut source)
...
def make_agent(act_dim: int, obs_dim: int, prior: str="Uniform"):  #273 (line num in coconut source)
    π_online = (Actor(obs_dim, act_dim)).to(device)  #275 (line num in coconut source)
    ...
    π_optim = pt.optim.Adam(π_online.parameters(), lr=η_π, betas=βs)  #282 (line num in coconut source)
    q1_optim = pt.optim.Adam(q1_online.parameters(), lr=η_q, betas=βs)  #284 (line num in coconut source)
    q2_optim = pt.optim.Adam(q2_online.parameters(), lr=η_q, betas=βs)  #286 (line num in coconut source)
    H_target = -act_dim  #288 (line num in coconut source)
    α_log = pt.tensor([0.0,], requires_grad=True, device=device)  #289 (line num in coconut source)
    α_optim = pt.optim.Adam([α_log,], lr=η_α, betas=βs)  #290 (line num in coconut source)
    π_prior = _coconut_forward_compose((uniform_prior if prior == "Uniform" else gaussian_prior), _coconut.operator.methodcaller("to", device))  #291 (line num in coconut source)
    agent = Agent(π_online, π_optim, q1_online, q1_optim, q2_online, q2_optim, q1_target, q2_target, H_target, π_prior, α_log, α_optim)  #294 (line num in coconut source)
...
def update_step(*_coconut_match_args, **_coconut_match_kwargs):  #301 (line num in coconut source)
    ...
    _coconut_match_set_name_γ = _coconut_sentinel  #301 (line num in coconut source)
    ...
    if (_coconut.len(_coconut_match_args) <= 8) and (_coconut.sum((_coconut.len(_coconut_match_args) > 0, "iteration" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 1, "epoch" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 2, "agent" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 3, "experiences" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 4, "weights" in _coconut_match_kwargs)) == 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 5, "prios" in _coconut_match_kwargs)) <= 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 6, "γ" in _coconut_match_kwargs)) <= 1) and (_coconut.sum((_coconut.len(_coconut_match_args) > 7, "d" in _coconut_match_kwargs)) <= 1):  #301 (line num in coconut source)
        ...
        _coconut_match_temp_10 = _coconut_match_args[6] if _coconut.len(_coconut_match_args) > 6 else _coconut_match_kwargs.pop("γ") if "γ" in _coconut_match_kwargs else 0.99  #301 (line num in coconut source)
        ...
        if not _coconut_match_kwargs:  #301 (line num in coconut source)
            ...
            _coconut_match_set_name_γ = _coconut_match_temp_10  #301 (line num in coconut source)
    ...
    if _coconut_match_check_1:  #301 (line num in coconut source)
        ...
        if _coconut_match_set_name_γ is not _coconut_sentinel:  #301 (line num in coconut source)
            γ = _coconut_match_temp_10  #301 (line num in coconut source)
    ...
    a_t1, log_π_t1 = evaluate(agent, s_t1)  #306 (line num in coconut source)
    log_π_t1_ = (_coconut_partial(pt.mean, {}, 1, dim=1, keepdims=True))(log_π_t1)  #307 (line num in coconut source)
    α = (pt.exp)(agent.α_log)  #308 (line num in coconut source)
    α_ = (pt.tensor(0.0, device=device) if iteration == 0 else α.detach())  #309 (line num in coconut source)
...
#r      = r_t0_
    ...
    Q_t1 = (pt.clone)(((r + (γ * (1 - d_t0) * (Q_t1_ - α_ * log_π_t1_)))).detach())  #319 (line num in coconut source)
    ...
    δ1 = pt.pow(Q1_t0 - Q_t1, 2) * w  #323 (line num in coconut source)
    δ2 = pt.pow(Q2_t0 - Q_t1, 2) * w  #324 (line num in coconut source)
    J_Q1 = (pt.clone)((pt.mean)((0.5 * δ1)))  #325 (line num in coconut source)
    J_Q2 = (pt.clone)((pt.mean)((0.5 * δ2)))  #326 (line num in coconut source)
    ...
    if iteration in count(0, d):  #333 (line num in coconut source)
        a_t0_, log_π_t0_ = evaluate(agent, s_t0)  #334 (line num in coconut source)
        log_π_t0 = (pt.clone)((log_π_t0_).detach())  #335 (line num in coconut source)
        J_α = (pt.mean)((-α * log_π_t0 - α * agent.H_target))  #336 (line num in coconut source)
        _ = agent.α_optim.zero_grad(set_to_none=True)  #337 (line num in coconut source)
        _ = J_α.backward()  #338 (line num in coconut source)
        _ = agent.α_optim.step()  #339 (line num in coconut source)
        ...
        J_π = (pt.mean)(((α_ * log_π_t0_) - Q_t0_))  #344 (line num in coconut source)
        _ = agent.π_optim.zero_grad(set_to_none=True)  #345 (line num in coconut source)
        _ = J_π.backward()  #346 (line num in coconut source)
        _ = agent.π_optim.step()  #347 (line num in coconut source)
        ...
        _ = writer.add_scalar("_Loss_π", J_π, iteration)  #350 (line num in coconut source)
        _ = writer.add_scalar("_Loss_α", J_α, iteration)  #351 (line num in coconut source)
    prios_ = ((pt.abs)((0.5 * (δ1 + δ2) + ε_const))).detach()  #352 (line num in coconut source)
    ...
    return _coconut_tail_call(update_step, iteration, epoch_, agent, experiences, weights, prios_, γ, d)  #357 (line num in coconut source)
