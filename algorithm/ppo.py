...
device = pt.device("cuda:1") if pt.cuda.is_available() else pt.device("cpu")  #8 (line num in coconut source)

def set_device(dev: pt.device):  #10 (line num in coconut source)
    global device  #11 (line num in coconut source)
    device = dev  #12 (line num in coconut source)

## Algorithm parameter defaults
    return device  # Factor for clipping  #15 (line num in coconut source)
# Factor for clipping
ε: float = 0.2  # Factor for clipping  #15 (line num in coconut source)
η: float = 0.001  # Factor in loss function  #16 (line num in coconut source)
γ: float = 0.99  # Discount Factor  #17 (line num in coconut source)
τ: float = 0.95  # Avantage Factor  #18 (line num in coconut source)
...
def op_actor(obs_dim: int, act_dim: int):  #39 (line num in coconut source)
    actor = pt.nn.Sequential(pt.nn.Linear(obs_dim, 128), pt.nn.Tanh(), pt.nn.Linear(128, 256), pt.nn.Tanh(), pt.nn.Linear(256, 128), pt.nn.Tanh(), pt.nn.Linear(128, 64), pt.nn.Tanh(), pt.nn.Linear(64, act_dim), pt.nn.Tanh())  #40 (line num in coconut source)

    return actor  #46 (line num in coconut source)

def op_critic(obs_dim: int):  #46 (line num in coconut source)
    critic = pt.nn.Sequential(pt.nn.Linear(obs_dim, 128), pt.nn.Tanh(), pt.nn.Linear(128, 256), pt.nn.Tanh(), pt.nn.Linear(256, 128), pt.nn.Tanh(), pt.nn.Linear(128, 64), pt.nn.Tanh(), pt.nn.Linear(64, 1))  #47 (line num in coconut source)

    return critic  #53 (line num in coconut source)

def st1_actor(obs_dim: int=12, act_dim: int=6):  #53 (line num in coconut source)
    actor = pt.nn.Sequential(pt.nn.Linear(obs_dim, 64), pt.nn.Tanh(), pt.nn.Linear(64, 64), pt.nn.Tanh(), pt.nn.Linear(64, act_dim), pt.nn.Tanh())  #54 (line num in coconut source)

    return actor  #58 (line num in coconut source)

def st1_critic(obs_dim: int=12):  #58 (line num in coconut source)
    critic = pt.nn.Sequential(pt.nn.Linear(obs_dim, 64), pt.nn.Tanh(), pt.nn.Linear(64, 64), pt.nn.Tanh(), pt.nn.Linear(64, 1))  #59 (line num in coconut source)

    return critic  #63 (line num in coconut source)

def nand4_actor(obs_dim: int=12, act_dim: int=5):  #63 (line num in coconut source)
    actor = pt.nn.Sequential(pt.nn.Linear(obs_dim, 64), pt.nn.Tanh(), pt.nn.Linear(64, 64), pt.nn.Tanh(), pt.nn.Linear(64, act_dim), pt.nn.Tanh())  #64 (line num in coconut source)

    return actor  #68 (line num in coconut source)

def nand4_critic(obs_dim: int=12):  #68 (line num in coconut source)
    critic = pt.nn.Sequential(pt.nn.Linear(obs_dim, 64), pt.nn.Tanh(), pt.nn.Linear(64, 64), pt.nn.Tanh(), pt.nn.Linear(64, 1))  #69 (line num in coconut source)

    return critic  #73 (line num in coconut source)

def std_deviation(act_dim: int, act_std: float=0.0):  #73 (line num in coconut source)
    std = (_coconut_partial((_coconut.operator.mul), {1: act_std}, 2))((_coconut_partial(pt.ones, {0: 1}, 2))(act_dim))  #74 (line num in coconut source)

    return std  #76 (line num in coconut source)

def continuous_actor_critic(actor: pt.nn.Module, critic: pt.nn.Module, std: float, states: pt.Tensor):  #76 (line num in coconut source)
    μ = actor(states)  #79 (line num in coconut source)
#σ = std |> .expand_as(μ) |> pt.diag_embed |> .to(device)
    σ = ((_coconut_partial((_coconut.operator.mul), {1: std}, 2))((pt.eye)(((μ).shape)[1]))).to(device)  #81 (line num in coconut source)
    π = pt.distributions.MultivariateNormal(μ, σ)  #82 (line num in coconut source)
    v = (critic(states)).squeeze()  #83 (line num in coconut source)

    return (π, v)  #85 (line num in coconut source)

def discrete_actor_critic(actor, critic, states):  #85 (line num in coconut source)
    μ = actor(states)  #86 (line num in coconut source)
    π = pt.distributions.Categorical(μ)  #87 (line num in coconut source)
    v = (critic(states)).squeeze()  #88 (line num in coconut source)

    return (π, v)  # optim_act: pt.optim, optim_crt: pt.optim  #90 (line num in coconut source)
# optim_act: pt.optim, optim_crt: pt.optim
@_coconut_tco  # optim_act: pt.optim, optim_crt: pt.optim  #90 (line num in coconut source)
def update_policy(actor_critic: function, optimizer: pt.optim, states: pt.Tensor, actions: pt.Tensor, logprobs: pt.Tensor, returns: pt.Tensor, advantages: pt.Tensor):  # optim_act: pt.optim, optim_crt: pt.optim  #90 (line num in coconut source)
    dist, values = actor_critic(states)  #94 (line num in coconut source)
    entropy = ((dist).entropy()).mean()  #95 (line num in coconut source)
    logprobs_ = dist.log_prob(actions)  #96 (line num in coconut source)
    ratios = (pt.exp)((logprobs_ - logprobs))  #97 (line num in coconut source)
    surr_1 = ratios * advantages  #98 (line num in coconut source)
    surr_2 = pt.clamp(ratios, 1.0 - ε, 1.0 + ε) * advantages  #99 (line num in coconut source)
    loss_act = ((_coconut_minus))((pt.min)(*(surr_1, surr_2)))  #100 (line num in coconut source)
    loss_crt = (pt.mean)((_coconut_partial(pt.pow, {1: 2}, 2))((returns - values)))  #101 (line num in coconut source)
    losses = 0.5 * loss_crt + loss_act - η * entropy  #102 (line num in coconut source)
    _ = (optimizer).zero_grad()  #103 (line num in coconut source)
    _ = ((pt.mean)(losses)).backward()  #104 (line num in coconut source)
    _ = (optimizer).step()  #105 (line num in coconut source)
...
#_           = optim_crt |> .step()
    ...
    return _coconut_tail_call(losses.detach)  #113 (line num in coconut source)

def subset(obs, inf, pre):  #113 (line num in coconut source)
    sub = (((pt.vstack)((list)((starmap)(_coconut_forward_compose((lambda i, o: o[i]), pt.from_numpy), (_coconut_partial(zip, {1: obs}, 2))([[i["output-parameters"].index(p) for p in i["output-parameters"] if p.startswith(pre)] for i in inf]))))).to(device)).detach()  #114 (line num in coconut source)

    return sub  #120 (line num in coconut source)

def postprocess(observations, infos):  #120 (line num in coconut source)
    states = (((pt.vstack)((list)((starmap)(_coconut_forward_compose((lambda i, o: o[i]), pt.from_numpy), (_coconut_partial(zip, {1: observations}, 2))([[i["output-parameters"].index(p) for p in i["output-parameters"] if (any)((_coconut_partial(fmap, {0: p.startswith}, 2))(["performance", "target", "distance"]))] for i in infos]))))).to(device)).detach()  #125 (line num in coconut source)
#states = observations |> fmap$pt.from_numpy |> pt.vstack |> .to(device) |> .detach() 

    return states  #132 (line num in coconut source)

def trajectories(actor_critic: function, envs: gace.envs.vec.VecACE, states: pt.Tensor):  #132 (line num in coconut source)
    dist, values = actor_critic(states)  #134 (line num in coconut source)
    actions = ((dist).sample()).detach()  #135 (line num in coconut source)
    logprobs = (dist.log_prob)(actions)  #136 (line num in coconut source)
    observations, rewards_, dones, infos = (envs.step)((list)((fmap)(_coconut_forward_compose(_coconut.operator.methodcaller("squeeze"), _coconut.operator.methodcaller("cpu"), _coconut.operator.methodcaller("numpy")), (_coconut_partial(pt.split, {1: 1}, 2))(actions))))  #137 (line num in coconut source)
    next_states = postprocess(observations, infos)  #141 (line num in coconut source)
    masks = (_coconut_partial((_coconut_minus), {0: 1}, 2))(((pt.Tensor)(dones)).to(device))  #142 (line num in coconut source)
    rewards = ((pt.Tensor)(rewards_)).to(device)  #143 (line num in coconut source)
    memories = (states, actions, logprobs, rewards, values, masks)  #144 (line num in coconut source)

    return (memories, next_states)  #146 (line num in coconut source)

def gae(r: pt.Tensor, v: pt.Tensor, m: pt.Tensor, v_: pt.Tensor, γ: float=0.99, τ: float=0.95):  #146 (line num in coconut source)
    δ = r + γ * v_ * m - v  #148 (line num in coconut source)
    l = (reversed)((range)(((δ).shape)[0]))  #149 (line num in coconut source)
    i = pt.Tensor([0,]).to(device)  #150 (line num in coconut source)
    g = (reduce(lambda g_, i_: (pt.hstack)((_coconut_partial((_coconut_comma_op), {1: g_}, 2))(δ[i_] + γ * τ * m[i_] * g_[0])), l, i))[_coconut.slice(None, -1)]  #151 (line num in coconut source)
    a = v + g  #153 (line num in coconut source)

    return a  #155 (line num in coconut source)

def save_checkpoint(actor: pt.nn.Module, critic: pt.nn.Module, deviation: pt.nn.Module, optimizer: pt.optim, checkpoint_file: str):  #155 (line num in coconut source)
    state_dicts = (_coconut_partial((_coconut.operator.add), {1: [deviation,]}, 2))((fmap)(_coconut.operator.methodcaller("state_dict"), [actor, critic, optimizer]))  #158 (line num in coconut source)
    save_dict = (dict)((_coconut_partial(zip, {0: ["actor", "critic", "optimizer", "deviation"]}, 2))(state_dicts))  #160 (line num in coconut source)
    pt.save(save_dict, checkpoint_file)  #163 (line num in coconut source)

    return True  #164 (line num in coconut source)
